# Chapter: Loop-invariant code motion LICM
...
**Topic:** Loop-invariant code motion LICM
...
```cpp
#include <iostream>
// Research-level implementation template for: Loop-invariant code motion LICM
...
int main() {
    std::cout << "Topic: Loop-invariant code motion LICM" << std::endl;
    return 0;
}
```
...
```cpp
#include <iostream>
// Alternative perspective / comparison for: Loop-invariant code motion LICM
...
**In one paragraph:** This chapter covers **Loop-invariant code motion LICM** — a research-level topic in Compiler Internals.
