# Chapter: Min cost flow with negative cycles pro
...
**Topic:** Min cost flow with negative cycles pro
...
**Correctness proof sketch for Min cost flow with negative cycles pro:**
...
```
ALGORITHM MIN_COST_FLOW_WITH_NEGATIVE_CYCLES_PRO(input):
...
#include <iostream>
#include <vector>
// Production implementation template for: Min cost flow with negative cycles pro
// Full compilable C++17 — pedagogical version
int main() {
    std::cout << "Implementation: Min cost flow with negative cycles pro" << std::endl;
    return 0;
}
```
...
#include <iostream>
#include <vector>
// Competitive programming version for: Min cost flow with negative cycles pro
