# Chapter: Stock III two transactions
...
## 1. Problem Statement
Master the topic: **Stock III two transactions**.
...
This problem exercises **Stock III two transactions** — a core technique in Dynamic Programming.
...
#include <iostream>
#include <vector>
// Brute-force template for: Stock III two transactions
int solve_stock_iii_two_transactions(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_stock_iii_two_transactions(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Stock III two transactions
int solve_stock_iii_two_transactions(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_stock_iii_two_transactions(v); }
```
...
- **Core idea:** Stock III two transactions — Dynamic Programming.
