# Chapter: Diameter of binary tree DP
...
## 1. Problem Statement
Master the topic: **Diameter of binary tree DP**.
...
This problem exercises **Diameter of binary tree DP** — a core technique in Dynamic Programming.
...
#include <iostream>
#include <vector>
// Brute-force template for: Diameter of binary tree DP
int solve_diameter_of_binary_tree_dp(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_diameter_of_binary_tree_dp(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Diameter of binary tree DP
int solve_diameter_of_binary_tree_dp(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_diameter_of_binary_tree_dp(v); }
```
...
- **Core idea:** Diameter of binary tree DP — Dynamic Programming.
