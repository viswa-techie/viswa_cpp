# Chapter: Number of paths in k moves
...
## 1. Problem Statement
Master the topic: **Number of paths in k moves**.
...
This problem exercises **Number of paths in k moves** — a core technique in Dynamic Programming.
...
#include <iostream>
#include <vector>
// Brute-force template for: Number of paths in k moves
int solve_number_of_paths_in_k_moves(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_number_of_paths_in_k_moves(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Number of paths in k moves
int solve_number_of_paths_in_k_moves(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_number_of_paths_in_k_moves(v); }
```
...
- **Core idea:** Number of paths in k moves — Dynamic Programming.
