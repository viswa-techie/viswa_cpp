# Chapter: Minimum operations to make array increasing
...
## 1. Problem Statement
Master the topic: **Minimum operations to make array increasing**.
...
This problem exercises **Minimum operations to make array increasing** — a core technique in Dynamic Programming.
...
#include <iostream>
#include <vector>
// Brute-force template for: Minimum operations to make array increasing
int solve_minimum_operations_to_make_array_in(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_minimum_operations_to_make_array_in(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Minimum operations to make array increasing
int solve_minimum_operations_to_make_array_in(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_minimum_operations_to_make_array_in(v); }
```
...
- **Core idea:** Minimum operations to make array increasing — Dynamic Programming.
