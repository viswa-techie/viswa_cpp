# Chapter: DP on DAG shortest longest path
...
## 1. Problem Statement
Master the topic: **DP on DAG shortest longest path**.
...
This problem exercises **DP on DAG shortest longest path** — a core technique in Dynamic Programming.
...
#include <iostream>
#include <vector>
// Brute-force template for: DP on DAG shortest longest path
int solve_dp_on_dag_shortest_longest_path(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_dp_on_dag_shortest_longest_path(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: DP on DAG shortest longest path
int solve_dp_on_dag_shortest_longest_path(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_dp_on_dag_shortest_longest_path(v); }
```
...
- **Core idea:** DP on DAG shortest longest path — Dynamic Programming.
