# Chapter: Coin change II count ways
...
## 1. Problem Statement
Master the topic: **Coin change II count ways**.
...
This problem exercises **Coin change II count ways** — a core technique in Dynamic Programming.
...
#include <iostream>
#include <vector>
// Brute-force template for: Coin change II count ways
int solve_coin_change_ii_count_ways(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_coin_change_ii_count_ways(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Coin change II count ways
int solve_coin_change_ii_count_ways(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_coin_change_ii_count_ways(v); }
```
...
- **Core idea:** Coin change II count ways — Dynamic Programming.
