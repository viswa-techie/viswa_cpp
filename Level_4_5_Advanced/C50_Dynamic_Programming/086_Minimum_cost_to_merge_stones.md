# Chapter: Minimum cost to merge stones
...
## 1. Problem Statement
Master the topic: **Minimum cost to merge stones**.
...
This problem exercises **Minimum cost to merge stones** — a core technique in Dynamic Programming.
...
#include <iostream>
#include <vector>
// Brute-force template for: Minimum cost to merge stones
int solve_minimum_cost_to_merge_stones(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_minimum_cost_to_merge_stones(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Minimum cost to merge stones
int solve_minimum_cost_to_merge_stones(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_minimum_cost_to_merge_stones(v); }
```
...
- **Core idea:** Minimum cost to merge stones — Dynamic Programming.
