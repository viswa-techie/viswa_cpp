...
## 1. Problem Statement
Master the topic: **Optimal binary search tree**.
...
This problem exercises **Optimal binary search tree** — a core technique in Dynamic Programming.
...
#include <iostream>
#include <vector>
// Brute-force template for: Optimal binary search tree
int solve_optimal_binary_search_tree(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_optimal_binary_search_tree(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Optimal binary search tree
int solve_optimal_binary_search_tree(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_optimal_binary_search_tree(v); }
```
...
- **Core idea:** Optimal binary search tree — Dynamic Programming.
