# Chapter: Stock with transaction fee
...
## 1. Problem Statement
Master the topic: **Stock with transaction fee**.
...
This problem exercises **Stock with transaction fee** — a core technique in Dynamic Programming.
...
#include <iostream>
#include <vector>
// Brute-force template for: Stock with transaction fee
int solve_stock_with_transaction_fee(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_stock_with_transaction_fee(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Stock with transaction fee
int solve_stock_with_transaction_fee(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_stock_with_transaction_fee(v); }
```
...
- **Core idea:** Stock with transaction fee — Dynamic Programming.
