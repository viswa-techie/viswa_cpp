# Chapter: Partition to K equal sum subsets
...
## 1. Problem Statement
Master the topic: **Partition to K equal sum subsets**.
...
This problem exercises **Partition to K equal sum subsets** — a core technique in Dynamic Programming.
...
#include <iostream>
#include <vector>
// Brute-force template for: Partition to K equal sum subsets
int solve_partition_to_k_equal_sum_subsets(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_partition_to_k_equal_sum_subsets(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Partition to K equal sum subsets
int solve_partition_to_k_equal_sum_subsets(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_partition_to_k_equal_sum_subsets(v); }
```
...
- **Core idea:** Partition to K equal sum subsets — Dynamic Programming.
