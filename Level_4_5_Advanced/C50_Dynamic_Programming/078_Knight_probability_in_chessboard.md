# Chapter: Knight probability in chessboard
...
## 1. Problem Statement
Master the topic: **Knight probability in chessboard**.
...
This problem exercises **Knight probability in chessboard** — a core technique in Dynamic Programming.
...
#include <iostream>
#include <vector>
// Brute-force template for: Knight probability in chessboard
int solve_knight_probability_in_chessboard(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_knight_probability_in_chessboard(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Knight probability in chessboard
int solve_knight_probability_in_chessboard(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_knight_probability_in_chessboard(v); }
```
...
- **Core idea:** Knight probability in chessboard — Dynamic Programming.
