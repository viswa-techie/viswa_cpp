# Chapter: Minimum subset sum difference
...
## 1. Problem Statement
Master the topic: **Minimum subset sum difference**.
...
This problem exercises **Minimum subset sum difference** — a core technique in Dynamic Programming.
...
#include <iostream>
#include <vector>
// Brute-force template for: Minimum subset sum difference
int solve_minimum_subset_sum_difference(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_minimum_subset_sum_difference(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Minimum subset sum difference
int solve_minimum_subset_sum_difference(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_minimum_subset_sum_difference(v); }
```
...
- **Core idea:** Minimum subset sum difference — Dynamic Programming.
