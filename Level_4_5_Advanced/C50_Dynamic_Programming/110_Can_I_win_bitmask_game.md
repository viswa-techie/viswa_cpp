# Chapter: Can I win bitmask game
...
## 1. Problem Statement
Master the topic: **Can I win bitmask game**.
...
This problem exercises **Can I win bitmask game** — a core technique in Dynamic Programming.
...
#include <iostream>
#include <vector>
// Brute-force template for: Can I win bitmask game
int solve_can_i_win_bitmask_game(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_can_i_win_bitmask_game(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Can I win bitmask game
int solve_can_i_win_bitmask_game(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_can_i_win_bitmask_game(v); }
```
...
- **Core idea:** Can I win bitmask game — Dynamic Programming.
