# Chapter: Max profit from jobs DP plus binary search
...
## 1. Problem Statement
Master the topic: **Max profit from jobs DP plus binary search**.
...
This problem exercises **Max profit from jobs DP plus binary search** — a core technique in Dynamic Programming.
...
#include <iostream>
#include <vector>
// Brute-force template for: Max profit from jobs DP plus binary search
int solve_max_profit_from_jobs_dp_plus_binary(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_max_profit_from_jobs_dp_plus_binary(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Max profit from jobs DP plus binary search
int solve_max_profit_from_jobs_dp_plus_binary(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_max_profit_from_jobs_dp_plus_binary(v); }
```
...
- **Core idea:** Max profit from jobs DP plus binary search — Dynamic Programming.
