# Chapter: Count subsets with sum k
...
## 1. Problem Statement
Master the topic: **Count subsets with sum k**.
...
This problem exercises **Count subsets with sum k** — a core technique in Dynamic Programming.
...
#include <iostream>
#include <vector>
// Brute-force template for: Count subsets with sum k
int solve_count_subsets_with_sum_k(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_count_subsets_with_sum_k(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Count subsets with sum k
int solve_count_subsets_with_sum_k(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_count_subsets_with_sum_k(v); }
```
...
- **Core idea:** Count subsets with sum k — Dynamic Programming.
