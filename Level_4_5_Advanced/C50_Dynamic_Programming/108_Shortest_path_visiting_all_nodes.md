# Chapter: Shortest path visiting all nodes
...
## 1. Problem Statement
Master the topic: **Shortest path visiting all nodes**.
...
This problem exercises **Shortest path visiting all nodes** — a core technique in Dynamic Programming.
...
#include <iostream>
#include <vector>
// Brute-force template for: Shortest path visiting all nodes
int solve_shortest_path_visiting_all_nodes(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_shortest_path_visiting_all_nodes(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Shortest path visiting all nodes
int solve_shortest_path_visiting_all_nodes(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_shortest_path_visiting_all_nodes(v); }
```
...
- **Core idea:** Shortest path visiting all nodes — Dynamic Programming.
