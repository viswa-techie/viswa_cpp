# Chapter: Longest subarray with constraints DP plus deque
...
## 1. Problem Statement
Master the topic: **Longest subarray with constraints DP plus deque**.
...
This problem exercises **Longest subarray with constraints DP plus deque** — a core technique in Dynamic Programming.
...
#include <iostream>
#include <vector>
// Brute-force template for: Longest subarray with constraints DP plus deque
int solve_longest_subarray_with_constraints_d(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_longest_subarray_with_constraints_d(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Longest subarray with constraints DP plus deque
int solve_longest_subarray_with_constraints_d(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_longest_subarray_with_constraints_d(v); }
```
...
- **Core idea:** Longest subarray with constraints DP plus deque — Dynamic Programming.
