# Chapter: Bucket sort with distribution
...
## 1. Problem Statement
Master the topic: **Bucket sort with distribution**.
...
This problem exercises **Bucket sort with distribution** — a core technique in Heaps, Priority Queues & Advanced Sorting.
...
#include <iostream>
#include <vector>
// Brute-force template for: Bucket sort with distribution
int solve_bucket_sort_with_distribution(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_bucket_sort_with_distribution(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Bucket sort with distribution
int solve_bucket_sort_with_distribution(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_bucket_sort_with_distribution(v); }
```
...
- **Core idea:** Bucket sort with distribution — Heap / Priority Queue + Sorting.
