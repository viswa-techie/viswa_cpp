# Chapter: Huffman coding tree build
...
## 1. Problem Statement
Master the topic: **Huffman coding tree build**.
...
This problem exercises **Huffman coding tree build** — a core technique in Heaps, Priority Queues & Advanced Sorting.
...
#include <iostream>
#include <vector>
// Brute-force template for: Huffman coding tree build
int solve_huffman_coding_tree_build(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_huffman_coding_tree_build(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Huffman coding tree build
int solve_huffman_coding_tree_build(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_huffman_coding_tree_build(v); }
```
...
- **Core idea:** Huffman coding tree build — Heap / Priority Queue + Sorting.
