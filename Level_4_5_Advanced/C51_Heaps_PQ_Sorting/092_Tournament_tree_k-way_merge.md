# Chapter: Tournament tree k-way merge
...
## 1. Problem Statement
Master the topic: **Tournament tree k-way merge**.
...
This problem exercises **Tournament tree k-way merge** — a core technique in Heaps, Priority Queues & Advanced Sorting.
...
#include <iostream>
#include <vector>
// Brute-force template for: Tournament tree k-way merge
int solve_tournament_tree_k_way_merge(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_tournament_tree_k_way_merge(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Tournament tree k-way merge
int solve_tournament_tree_k_way_merge(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_tournament_tree_k_way_merge(v); }
```
...
- **Core idea:** Tournament tree k-way merge — Heap / Priority Queue + Sorting.
