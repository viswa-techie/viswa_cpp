...
## 1. Problem Statement
Master the topic: **Radix sort LSD vs MSD**.
...
This problem exercises **Radix sort LSD vs MSD** — a core technique in Heaps, Priority Queues & Advanced Sorting.
...
#include <iostream>
#include <vector>
// Brute-force template for: Radix sort LSD vs MSD
int solve_radix_sort_lsd_vs_msd(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_radix_sort_lsd_vs_msd(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Radix sort LSD vs MSD
int solve_radix_sort_lsd_vs_msd(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_radix_sort_lsd_vs_msd(v); }
```
...
- **Core idea:** Radix sort LSD vs MSD — Heap / Priority Queue + Sorting.
