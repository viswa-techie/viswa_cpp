# Chapter: Trapping rain water II 3D BFS plus heap
...
## 1. Problem Statement
Master the topic: **Trapping rain water II 3D BFS plus heap**.
...
This problem exercises **Trapping rain water II 3D BFS plus heap** — a core technique in Heaps, Priority Queues & Advanced Sorting.
...
#include <iostream>
#include <vector>
// Brute-force template for: Trapping rain water II 3D BFS plus heap
int solve_trapping_rain_water_ii_3d_bfs_plus_(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_trapping_rain_water_ii_3d_bfs_plus_(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Trapping rain water II 3D BFS plus heap
int solve_trapping_rain_water_ii_3d_bfs_plus_(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_trapping_rain_water_ii_3d_bfs_plus_(v); }
```
...
- **Core idea:** Trapping rain water II 3D BFS plus heap — Heap / Priority Queue + Sorting.
