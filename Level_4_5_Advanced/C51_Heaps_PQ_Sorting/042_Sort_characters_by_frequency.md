...
This problem exercises **Sort characters by frequency** — a core technique in Heaps, Priority Queues & Advanced Sorting.
...
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_sort_characters_by_frequency(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Sort characters by frequency
int solve_sort_characters_by_frequency(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_sort_characters_by_frequency(v); }
```
...
- **Core idea:** Sort characters by frequency — Heap / Priority Queue + Sorting.
