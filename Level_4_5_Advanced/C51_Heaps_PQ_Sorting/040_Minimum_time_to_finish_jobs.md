# Chapter: Minimum time to finish jobs
...
## 1. Problem Statement
Master the topic: **Minimum time to finish jobs**.
...
This problem exercises **Minimum time to finish jobs** — a core technique in Heaps, Priority Queues & Advanced Sorting.
...
#include <iostream>
#include <vector>
// Brute-force template for: Minimum time to finish jobs
int solve_minimum_time_to_finish_jobs(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_minimum_time_to_finish_jobs(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Minimum time to finish jobs
int solve_minimum_time_to_finish_jobs(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_minimum_time_to_finish_jobs(v); }
```
...
- **Core idea:** Minimum time to finish jobs — Heap / Priority Queue + Sorting.
