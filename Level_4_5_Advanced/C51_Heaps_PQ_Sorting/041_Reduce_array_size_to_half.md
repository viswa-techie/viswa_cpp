# Chapter: Reduce array size to half
...
## 1. Problem Statement
Master the topic: **Reduce array size to half**.
...
This problem exercises **Reduce array size to half** — a core technique in Heaps, Priority Queues & Advanced Sorting.
...
#include <iostream>
#include <vector>
// Brute-force template for: Reduce array size to half
int solve_reduce_array_size_to_half(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_reduce_array_size_to_half(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Reduce array size to half
int solve_reduce_array_size_to_half(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_reduce_array_size_to_half(v); }
```
...
- **Core idea:** Reduce array size to half — Heap / Priority Queue + Sorting.
