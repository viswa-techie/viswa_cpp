# Chapter: Global inversions local inversions
...
## 1. Problem Statement
Master the topic: **Global inversions local inversions**.
...
This problem exercises **Global inversions local inversions** — a core technique in Heaps, Priority Queues & Advanced Sorting.
...
#include <iostream>
#include <vector>
// Brute-force template for: Global inversions local inversions
int solve_global_inversions_local_inversions(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_global_inversions_local_inversions(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Global inversions local inversions
int solve_global_inversions_local_inversions(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_global_inversions_local_inversions(v); }
```
...
- **Core idea:** Global inversions local inversions — Heap / Priority Queue + Sorting.
