# Chapter: Minimum adjacent swaps to make valid
...
## 1. Problem Statement
Master the topic: **Minimum adjacent swaps to make valid**.
...
This problem exercises **Minimum adjacent swaps to make valid** — a core technique in Heaps, Priority Queues & Advanced Sorting.
...
#include <iostream>
#include <vector>
// Brute-force template for: Minimum adjacent swaps to make valid
int solve_minimum_adjacent_swaps_to_make_vali(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_minimum_adjacent_swaps_to_make_vali(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Minimum adjacent swaps to make valid
int solve_minimum_adjacent_swaps_to_make_vali(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_minimum_adjacent_swaps_to_make_vali(v); }
```
...
- **Core idea:** Minimum adjacent swaps to make valid — Heap / Priority Queue + Sorting.
