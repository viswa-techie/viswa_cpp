# Chapter: American flag sort in-place radix
...
## 1. Problem Statement
Master the topic: **American flag sort in-place radix**.
...
This problem exercises **American flag sort in-place radix** — a core technique in Heaps, Priority Queues & Advanced Sorting.
...
#include <iostream>
#include <vector>
// Brute-force template for: American flag sort in-place radix
int solve_american_flag_sort_in_place_radix(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_american_flag_sort_in_place_radix(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: American flag sort in-place radix
int solve_american_flag_sort_in_place_radix(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_american_flag_sort_in_place_radix(v); }
```
...
- **Core idea:** American flag sort in-place radix — Heap / Priority Queue + Sorting.
