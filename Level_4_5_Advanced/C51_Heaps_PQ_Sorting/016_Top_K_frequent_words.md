...
This problem exercises **Top K frequent words** — a core technique in Heaps, Priority Queues & Advanced Sorting.
...
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_top_k_frequent_words(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Top K frequent words
int solve_top_k_frequent_words(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_top_k_frequent_words(v); }
```
...
- **Core idea:** Top K frequent words — Heap / Priority Queue + Sorting.
