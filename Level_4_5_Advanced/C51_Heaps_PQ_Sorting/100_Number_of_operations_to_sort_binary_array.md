# Chapter: Number of operations to sort binary array
...
## 1. Problem Statement
Master the topic: **Number of operations to sort binary array**.
...
This problem exercises **Number of operations to sort binary array** — a core technique in Heaps, Priority Queues & Advanced Sorting.
...
#include <iostream>
#include <vector>
// Brute-force template for: Number of operations to sort binary array
int solve_number_of_operations_to_sort_binary(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_number_of_operations_to_sort_binary(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Number of operations to sort binary array
int solve_number_of_operations_to_sort_binary(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_number_of_operations_to_sort_binary(v); }
```
...
- **Core idea:** Number of operations to sort binary array — Heap / Priority Queue + Sorting.
