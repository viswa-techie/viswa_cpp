# Chapter: Parallel sort concepts
...
## 1. Problem Statement
Master the topic: **Parallel sort concepts**.
...
This problem exercises **Parallel sort concepts** — a core technique in Heaps, Priority Queues & Advanced Sorting.
...
#include <iostream>
#include <vector>
// Brute-force template for: Parallel sort concepts
int solve_parallel_sort_concepts(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_parallel_sort_concepts(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Parallel sort concepts
int solve_parallel_sort_concepts(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_parallel_sort_concepts(v); }
```
...
- **Core idea:** Parallel sort concepts — Heap / Priority Queue + Sorting.
