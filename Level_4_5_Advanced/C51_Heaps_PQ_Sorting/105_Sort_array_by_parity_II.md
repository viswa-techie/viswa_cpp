# Chapter: Sort array by parity II
...
## 1. Problem Statement
Master the topic: **Sort array by parity II**.
...
This problem exercises **Sort array by parity II** — a core technique in Heaps, Priority Queues & Advanced Sorting.
...
#include <iostream>
#include <vector>
// Brute-force template for: Sort array by parity II
int solve_sort_array_by_parity_ii(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_sort_array_by_parity_ii(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Sort array by parity II
int solve_sort_array_by_parity_ii(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_sort_array_by_parity_ii(v); }
```
...
- **Core idea:** Sort array by parity II — Heap / Priority Queue + Sorting.
