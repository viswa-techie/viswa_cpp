# Chapter: Sorting by frequency then alphabetically
...
## 1. Problem Statement
Master the topic: **Sorting by frequency then alphabetically**.
...
This problem exercises **Sorting by frequency then alphabetically** — a core technique in Heaps, Priority Queues & Advanced Sorting.
...
#include <iostream>
#include <vector>
// Brute-force template for: Sorting by frequency then alphabetically
int solve_sorting_by_frequency_then_alphabeti(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_sorting_by_frequency_then_alphabeti(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Sorting by frequency then alphabetically
int solve_sorting_by_frequency_then_alphabeti(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_sorting_by_frequency_then_alphabeti(v); }
```
...
- **Core idea:** Sorting by frequency then alphabetically — Heap / Priority Queue + Sorting.
