...
This problem exercises **Sliding window median** — a core technique in Heaps, Priority Queues & Advanced Sorting.
...
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_sliding_window_median(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Sliding window median
int solve_sliding_window_median(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_sliding_window_median(v); }
```
...
- **Core idea:** Sliding window median — Heap / Priority Queue + Sorting.
