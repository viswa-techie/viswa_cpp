# Chapter: Counting sort deep dive
...
## 1. Problem Statement
Master the topic: **Counting sort deep dive**.
...
This problem exercises **Counting sort deep dive** — a core technique in Heaps, Priority Queues & Advanced Sorting.
...
#include <iostream>
#include <vector>
// Brute-force template for: Counting sort deep dive
int solve_counting_sort_deep_dive(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_counting_sort_deep_dive(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Counting sort deep dive
int solve_counting_sort_deep_dive(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_counting_sort_deep_dive(v); }
```
...
- **Core idea:** Counting sort deep dive — Heap / Priority Queue + Sorting.
