# Chapter: Cut off trees for golf
...
## 1. Problem Statement
Master the topic: **Cut off trees for golf**.
...
This problem exercises **Cut off trees for golf** — a core technique in Heaps, Priority Queues & Advanced Sorting.
...
#include <iostream>
#include <vector>
// Brute-force template for: Cut off trees for golf
int solve_cut_off_trees_for_golf(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_cut_off_trees_for_golf(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Cut off trees for golf
int solve_cut_off_trees_for_golf(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_cut_off_trees_for_golf(v); }
```
...
- **Core idea:** Cut off trees for golf — Heap / Priority Queue + Sorting.
