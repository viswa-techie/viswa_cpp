# Chapter: Find the most competitive subsequence
...
## 1. Problem Statement
Master the topic: **Find the most competitive subsequence**.
...
This problem exercises **Find the most competitive subsequence** — a core technique in Heaps, Priority Queues & Advanced Sorting.
...
#include <iostream>
#include <vector>
// Brute-force template for: Find the most competitive subsequence
int solve_find_the_most_competitive_subsequen(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_find_the_most_competitive_subsequen(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Find the most competitive subsequence
int solve_find_the_most_competitive_subsequen(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_find_the_most_competitive_subsequen(v); }
```
...
- **Core idea:** Find the most competitive subsequence — Heap / Priority Queue + Sorting.
