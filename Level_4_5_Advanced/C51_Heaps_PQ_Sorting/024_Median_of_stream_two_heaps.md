# Chapter: Median of stream two heaps
...
## 1. Problem Statement
Master the topic: **Median of stream two heaps**.
...
This problem exercises **Median of stream two heaps** — a core technique in Heaps, Priority Queues & Advanced Sorting.
...
#include <iostream>
#include <vector>
// Brute-force template for: Median of stream two heaps
int solve_median_of_stream_two_heaps(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_median_of_stream_two_heaps(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Median of stream two heaps
int solve_median_of_stream_two_heaps(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_median_of_stream_two_heaps(v); }
```
...
- **Core idea:** Median of stream two heaps — Heap / Priority Queue + Sorting.
