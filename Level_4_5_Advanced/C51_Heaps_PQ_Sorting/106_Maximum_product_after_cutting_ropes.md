# Chapter: Maximum product after cutting ropes
...
## 1. Problem Statement
Master the topic: **Maximum product after cutting ropes**.
...
This problem exercises **Maximum product after cutting ropes** — a core technique in Heaps, Priority Queues & Advanced Sorting.
...
#include <iostream>
#include <vector>
// Brute-force template for: Maximum product after cutting ropes
int solve_maximum_product_after_cutting_ropes(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_maximum_product_after_cutting_ropes(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Maximum product after cutting ropes
int solve_maximum_product_after_cutting_ropes(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_maximum_product_after_cutting_ropes(v); }
```
...
- **Core idea:** Maximum product after cutting ropes — Heap / Priority Queue + Sorting.
