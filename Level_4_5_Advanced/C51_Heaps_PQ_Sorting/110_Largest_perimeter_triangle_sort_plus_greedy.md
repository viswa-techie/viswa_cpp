# Chapter: Largest perimeter triangle sort plus greedy
...
## 1. Problem Statement
Master the topic: **Largest perimeter triangle sort plus greedy**.
...
This problem exercises **Largest perimeter triangle sort plus greedy** — a core technique in Heaps, Priority Queues & Advanced Sorting.
...
#include <iostream>
#include <vector>
// Brute-force template for: Largest perimeter triangle sort plus greedy
int solve_largest_perimeter_triangle_sort_plu(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_largest_perimeter_triangle_sort_plu(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Largest perimeter triangle sort plus greedy
int solve_largest_perimeter_triangle_sort_plu(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_largest_perimeter_triangle_sort_plu(v); }
```
...
- **Core idea:** Largest perimeter triangle sort plus greedy — Heap / Priority Queue + Sorting.
