# Chapter: Smallest range covering K lists
...
## 1. Problem Statement
Master the topic: **Smallest range covering K lists**.
...
This problem exercises **Smallest range covering K lists** — a core technique in Heaps, Priority Queues & Advanced Sorting.
...
#include <iostream>
#include <vector>
// Brute-force template for: Smallest range covering K lists
int solve_smallest_range_covering_k_lists(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_smallest_range_covering_k_lists(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Smallest range covering K lists
int solve_smallest_range_covering_k_lists(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_smallest_range_covering_k_lists(v); }
```
...
- **Core idea:** Smallest range covering K lists — Heap / Priority Queue + Sorting.
