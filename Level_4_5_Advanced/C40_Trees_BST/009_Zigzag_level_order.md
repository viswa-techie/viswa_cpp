...
This problem exercises **Zigzag level order** — a core technique in Trees & Binary Search Trees.
...
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_zigzag_level_order(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Zigzag level order
int solve_zigzag_level_order(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_zigzag_level_order(v); }
```
...
- **Core idea:** Zigzag level order — Tree DFS / BST property.
