# Chapter: Two pointer on BST iterators
...
## 1. Problem Statement
Master the topic: **Two pointer on BST iterators**.
...
This problem exercises **Two pointer on BST iterators** — a core technique in Trees & Binary Search Trees.
...
#include <iostream>
#include <vector>
// Brute-force template for: Two pointer on BST iterators
int solve_two_pointer_on_bst_iterators(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_two_pointer_on_bst_iterators(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Two pointer on BST iterators
int solve_two_pointer_on_bst_iterators(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_two_pointer_on_bst_iterators(v); }
```
...
- **Core idea:** Two pointer on BST iterators — Tree DFS / BST property.
