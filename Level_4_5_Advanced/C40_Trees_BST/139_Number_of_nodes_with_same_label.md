# Chapter: Number of nodes with same label
...
## 1. Problem Statement
Master the topic: **Number of nodes with same label**.
...
This problem exercises **Number of nodes with same label** — a core technique in Trees & Binary Search Trees.
...
#include <iostream>
#include <vector>
// Brute-force template for: Number of nodes with same label
int solve_number_of_nodes_with_same_label(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_number_of_nodes_with_same_label(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Number of nodes with same label
int solve_number_of_nodes_with_same_label(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_number_of_nodes_with_same_label(v); }
```
...
- **Core idea:** Number of nodes with same label — Tree DFS / BST property.
