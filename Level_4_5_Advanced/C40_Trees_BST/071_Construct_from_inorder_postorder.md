# Chapter: Construct from inorder postorder
...
## 1. Problem Statement
Master the topic: **Construct from inorder postorder**.
...
This problem exercises **Construct from inorder postorder** — a core technique in Trees & Binary Search Trees.
...
#include <iostream>
#include <vector>
// Brute-force template for: Construct from inorder postorder
int solve_construct_from_inorder_postorder(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_construct_from_inorder_postorder(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Construct from inorder postorder
int solve_construct_from_inorder_postorder(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_construct_from_inorder_postorder(v); }
```
...
- **Core idea:** Construct from inorder postorder — Tree DFS / BST property.
