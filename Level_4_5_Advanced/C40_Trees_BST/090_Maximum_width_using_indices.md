# Chapter: Maximum width using indices
...
## 1. Problem Statement
Master the topic: **Maximum width using indices**.
...
This problem exercises **Maximum width using indices** — a core technique in Trees & Binary Search Trees.
...
#include <iostream>
#include <vector>
// Brute-force template for: Maximum width using indices
int solve_maximum_width_using_indices(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_maximum_width_using_indices(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Maximum width using indices
int solve_maximum_width_using_indices(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_maximum_width_using_indices(v); }
```
...
- **Core idea:** Maximum width using indices — Tree DFS / BST property.
