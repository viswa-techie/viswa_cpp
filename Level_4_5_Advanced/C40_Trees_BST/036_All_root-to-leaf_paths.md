# Chapter: All root-to-leaf paths
...
## 1. Problem Statement
Master the topic: **All root-to-leaf paths**.
...
This problem exercises **All root-to-leaf paths** — a core technique in Trees & Binary Search Trees.
...
#include <iostream>
#include <vector>
// Brute-force template for: All root-to-leaf paths
int solve_all_root_to_leaf_paths(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_all_root_to_leaf_paths(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: All root-to-leaf paths
int solve_all_root_to_leaf_paths(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_all_root_to_leaf_paths(v); }
```
...
- **Core idea:** All root-to-leaf paths — Tree DFS / BST property.
