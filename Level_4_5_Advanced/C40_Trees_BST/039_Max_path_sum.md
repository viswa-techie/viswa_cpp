...
At each node, best path through it = node + max(0,leftGain) + max(0,rightGain). Track global max.
...
- **Core idea:** Max path sum — Tree DFS / BST property.
