...
This problem exercises **Top view** — a core technique in Trees & Binary Search Trees.
...
#include <iostream>
#include <vector>
// Brute-force template for: Top view
int solve_top_view(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_top_view(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_top_view(v); }
```
...
- **Core idea:** Top view — Tree DFS / BST property.
