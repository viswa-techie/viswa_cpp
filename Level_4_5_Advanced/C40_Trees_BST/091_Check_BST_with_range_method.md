# Chapter: Check BST with range method
...
## 1. Problem Statement
Master the topic: **Check BST with range method**.
...
This problem exercises **Check BST with range method** — a core technique in Trees & Binary Search Trees.
...
#include <iostream>
#include <vector>
// Brute-force template for: Check BST with range method
int solve_check_bst_with_range_method(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_check_bst_with_range_method(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Check BST with range method
int solve_check_bst_with_range_method(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_check_bst_with_range_method(v); }
```
...
- **Core idea:** Check BST with range method — Tree DFS / BST property.
