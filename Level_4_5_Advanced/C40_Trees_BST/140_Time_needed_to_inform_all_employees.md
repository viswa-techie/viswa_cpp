# Chapter: Time needed to inform all employees
...
## 1. Problem Statement
Master the topic: **Time needed to inform all employees**.
...
This problem exercises **Time needed to inform all employees** — a core technique in Trees & Binary Search Trees.
...
#include <iostream>
#include <vector>
// Brute-force template for: Time needed to inform all employees
int solve_time_needed_to_inform_all_employees(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_time_needed_to_inform_all_employees(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Time needed to inform all employees
int solve_time_needed_to_inform_all_employees(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_time_needed_to_inform_all_employees(v); }
```
...
- **Core idea:** Time needed to inform all employees — Tree DFS / BST property.
