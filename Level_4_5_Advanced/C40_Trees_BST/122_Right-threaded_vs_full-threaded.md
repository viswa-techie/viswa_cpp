# Chapter: Right-threaded vs full-threaded
...
## 1. Problem Statement
Master the topic: **Right-threaded vs full-threaded**.
...
This problem exercises **Right-threaded vs full-threaded** — a core technique in Trees & Binary Search Trees.
...
#include <iostream>
#include <vector>
// Brute-force template for: Right-threaded vs full-threaded
int solve_right_threaded_vs_full_threaded(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_right_threaded_vs_full_threaded(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Right-threaded vs full-threaded
int solve_right_threaded_vs_full_threaded(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_right_threaded_vs_full_threaded(v); }
```
...
- **Core idea:** Right-threaded vs full-threaded — Tree DFS / BST property.
