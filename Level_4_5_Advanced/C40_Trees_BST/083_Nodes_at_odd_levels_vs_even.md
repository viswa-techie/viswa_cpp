# Chapter: Nodes at odd levels vs even
...
## 1. Problem Statement
Master the topic: **Nodes at odd levels vs even**.
...
This problem exercises **Nodes at odd levels vs even** — a core technique in Trees & Binary Search Trees.
...
#include <iostream>
#include <vector>
// Brute-force template for: Nodes at odd levels vs even
int solve_nodes_at_odd_levels_vs_even(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_nodes_at_odd_levels_vs_even(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Nodes at odd levels vs even
int solve_nodes_at_odd_levels_vs_even(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_nodes_at_odd_levels_vs_even(v); }
```
...
- **Core idea:** Nodes at odd levels vs even — Tree DFS / BST property.
