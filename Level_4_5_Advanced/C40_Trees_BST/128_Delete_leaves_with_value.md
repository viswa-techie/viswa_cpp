# Chapter: Delete leaves with value
...
## 1. Problem Statement
Master the topic: **Delete leaves with value**.
...
This problem exercises **Delete leaves with value** — a core technique in Trees & Binary Search Trees.
...
#include <iostream>
#include <vector>
// Brute-force template for: Delete leaves with value
int solve_delete_leaves_with_value(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_delete_leaves_with_value(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Delete leaves with value
int solve_delete_leaves_with_value(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_delete_leaves_with_value(v); }
```
...
- **Core idea:** Delete leaves with value — Tree DFS / BST property.
