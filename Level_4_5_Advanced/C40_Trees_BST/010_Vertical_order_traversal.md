# Chapter: Vertical order traversal
...
## 1. Problem Statement
Master the topic: **Vertical order traversal**.
...
This problem exercises **Vertical order traversal** — a core technique in Trees & Binary Search Trees.
...
#include <iostream>
#include <vector>
// Brute-force template for: Vertical order traversal
int solve_vertical_order_traversal(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_vertical_order_traversal(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Vertical order traversal
int solve_vertical_order_traversal(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_vertical_order_traversal(v); }
```
...
- **Core idea:** Vertical order traversal — Tree DFS / BST property.
