...
## 1. Problem Statement
Master the topic: **Check bipartite DFS**.
...
This problem exercises **Check bipartite DFS** — a core technique in Graphs — Fundamentals & Traversal.
...
#include <iostream>
#include <vector>
// Brute-force template for: Check bipartite DFS
int solve_check_bipartite_dfs(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_check_bipartite_dfs(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Check bipartite DFS
int solve_check_bipartite_dfs(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_check_bipartite_dfs(v); }
```
...
- **Core idea:** Check bipartite DFS — Graph traversal (BFS/DFS) + Union-Find.
