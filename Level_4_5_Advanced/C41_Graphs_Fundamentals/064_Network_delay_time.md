...
This problem exercises **Network delay time** — a core technique in Graphs — Fundamentals & Traversal.
...
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_network_delay_time(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Network delay time
int solve_network_delay_time(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_network_delay_time(v); }
```
...
- **Core idea:** Network delay time — Graph traversal (BFS/DFS) + Union-Find.
