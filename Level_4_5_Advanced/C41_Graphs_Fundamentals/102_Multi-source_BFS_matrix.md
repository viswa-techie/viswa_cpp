# Chapter: Multi-source BFS matrix
...
## 1. Problem Statement
Master the topic: **Multi-source BFS matrix**.
...
This problem exercises **Multi-source BFS matrix** — a core technique in Graphs — Fundamentals & Traversal.
...
#include <iostream>
#include <vector>
// Brute-force template for: Multi-source BFS matrix
int solve_multi_source_bfs_matrix(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_multi_source_bfs_matrix(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Multi-source BFS matrix
int solve_multi_source_bfs_matrix(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_multi_source_bfs_matrix(v); }
```
...
- **Core idea:** Multi-source BFS matrix — Graph traversal (BFS/DFS) + Union-Find.
