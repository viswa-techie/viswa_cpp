# Chapter: All paths from source to target
...
## 1. Problem Statement
Master the topic: **All paths from source to target**.
...
This problem exercises **All paths from source to target** — a core technique in Graphs — Fundamentals & Traversal.
...
#include <iostream>
#include <vector>
// Brute-force template for: All paths from source to target
int solve_all_paths_from_source_to_target(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_all_paths_from_source_to_target(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: All paths from source to target
int solve_all_paths_from_source_to_target(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_all_paths_from_source_to_target(v); }
```
...
- **Core idea:** All paths from source to target — Graph traversal (BFS/DFS) + Union-Find.
