...
This problem exercises **Minimum knight moves** — a core technique in Graphs — Fundamentals & Traversal.
...
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_minimum_knight_moves(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Minimum knight moves
int solve_minimum_knight_moves(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_minimum_knight_moves(v); }
```
...
- **Core idea:** Minimum knight moves — Graph traversal (BFS/DFS) + Union-Find.
