# Chapter: Parallel courses minimum semesters
...
## 1. Problem Statement
Master the topic: **Parallel courses minimum semesters**.
...
This problem exercises **Parallel courses minimum semesters** — a core technique in Graphs — Fundamentals & Traversal.
...
#include <iostream>
#include <vector>
// Brute-force template for: Parallel courses minimum semesters
int solve_parallel_courses_minimum_semesters(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_parallel_courses_minimum_semesters(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Parallel courses minimum semesters
int solve_parallel_courses_minimum_semesters(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_parallel_courses_minimum_semesters(v); }
```
...
- **Core idea:** Parallel courses minimum semesters — Graph traversal (BFS/DFS) + Union-Find.
