...
This problem exercises **Open the lock BFS** — a core technique in Graphs — Fundamentals & Traversal.
...
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_open_the_lock_bfs(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Open the lock BFS
int solve_open_the_lock_bfs(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_open_the_lock_bfs(v); }
```
...
- **Core idea:** Open the lock BFS — Graph traversal (BFS/DFS) + Union-Find.
