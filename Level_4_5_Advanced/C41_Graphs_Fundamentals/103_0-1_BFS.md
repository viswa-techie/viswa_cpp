...
This problem exercises **0-1 BFS** — a core technique in Graphs — Fundamentals & Traversal.
...
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_0_1_bfs(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_0_1_bfs(v); }
```
...
- **Core idea:** 0-1 BFS — Graph traversal (BFS/DFS) + Union-Find.
