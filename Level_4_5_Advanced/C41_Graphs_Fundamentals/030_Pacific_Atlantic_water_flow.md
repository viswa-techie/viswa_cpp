...
This problem exercises **Pacific Atlantic water flow** — a core technique in Graphs — Fundamentals & Traversal.
...
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_pacific_atlantic_water_flow(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Pacific Atlantic water flow
int solve_pacific_atlantic_water_flow(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_pacific_atlantic_water_flow(v); }
```
...
- **Core idea:** Pacific Atlantic water flow — Graph traversal (BFS/DFS) + Union-Find.
