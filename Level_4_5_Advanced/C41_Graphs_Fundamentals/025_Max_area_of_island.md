...
## 1. Problem Statement
Master the topic: **Max area of island**.
...
This problem exercises **Max area of island** — a core technique in Graphs — Fundamentals & Traversal.
...
#include <iostream>
#include <vector>
// Brute-force template for: Max area of island
int solve_max_area_of_island(std::vector<int> v){
    int best=0;
    for(size_t i=0;i<v.size();++i)
        for(size_t j=i;j<v.size();++j) best = std::max(best, v[i]+v[j]);
    return best;
}
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_max_area_of_island(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Max area of island
int solve_max_area_of_island(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_max_area_of_island(v); }
```
...
- **Core idea:** Max area of island — Graph traversal (BFS/DFS) + Union-Find.
