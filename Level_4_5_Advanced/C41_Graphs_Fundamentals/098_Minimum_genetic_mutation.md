...
This problem exercises **Minimum genetic mutation** — a core technique in Graphs — Fundamentals & Traversal.
...
int main(){ std::vector<int> v={1,2,3}; std::cout<<solve_minimum_genetic_mutation(v); }
```
**Time:** O(n²) or worse.  
**Space:** O(n).  
...
#include <vector>
#include <algorithm>
// Optimal template for: Minimum genetic mutation
int solve_minimum_genetic_mutation(std::vector<int>& v){
    if(v.empty()) return 0;
...
int main(){ std::vector<int> v={-2,1,-3,4,-1,2,1,-5,4}; std::cout<<solve_minimum_genetic_mutation(v); }
```
...
- **Core idea:** Minimum genetic mutation — Graph traversal (BFS/DFS) + Union-Find.
