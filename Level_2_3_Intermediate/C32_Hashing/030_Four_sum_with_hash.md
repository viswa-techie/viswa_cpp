...
## 1. Problem Statement
Master the topic: **Four sum with hash**.
...
This problem exercises **Four sum with hash** — a core concept in Hashing & Hash Tables.
...
Think of **Four sum with hash** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Four sum with hash
int solve_four_sum_with_hash(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_four_sum_with_hash(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Four sum with hash
int solve_four_sum_with_hash(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Four sum with hash
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Four sum with hash — O(1) average lookup via hash function.
