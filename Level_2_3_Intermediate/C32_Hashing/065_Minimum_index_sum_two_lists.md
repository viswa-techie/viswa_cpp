# Chapter: Minimum index sum two lists
...
## 1. Problem Statement
Master the topic: **Minimum index sum two lists**.
...
This problem exercises **Minimum index sum two lists** — a core concept in Hashing & Hash Tables.
...
Think of **Minimum index sum two lists** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Minimum index sum two lists
int solve_minimum_index_sum_two_lists(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_minimum_index_sum_two_lists(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Minimum index sum two lists
int solve_minimum_index_sum_two_lists(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Minimum index sum two lists
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Minimum index sum two lists — O(1) average lookup via hash function.
