# Chapter: Check if all chars appear equal times
...
## 1. Problem Statement
Master the topic: **Check if all chars appear equal times**.
...
This problem exercises **Check if all chars appear equal times** — a core concept in Hashing & Hash Tables.
...
Think of **Check if all chars appear equal times** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Check if all chars appear equal times
int solve_check_if_all_chars_appear_equal_times(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_check_if_all_chars_appear_equal_times(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Check if all chars appear equal times
int solve_check_if_all_chars_appear_equal_times(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Check if all chars appear equal times
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Check if all chars appear equal times — O(1) average lookup via hash function.
