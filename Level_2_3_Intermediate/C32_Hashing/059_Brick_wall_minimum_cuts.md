# Chapter: Brick wall minimum cuts
...
## 1. Problem Statement
Master the topic: **Brick wall minimum cuts**.
...
This problem exercises **Brick wall minimum cuts** — a core concept in Hashing & Hash Tables.
...
Think of **Brick wall minimum cuts** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Brick wall minimum cuts
int solve_brick_wall_minimum_cuts(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_brick_wall_minimum_cuts(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Brick wall minimum cuts
int solve_brick_wall_minimum_cuts(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Brick wall minimum cuts
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Brick wall minimum cuts — O(1) average lookup via hash function.
