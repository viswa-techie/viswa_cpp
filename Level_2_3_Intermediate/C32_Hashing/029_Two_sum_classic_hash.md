...
## 1. Problem Statement
Master the topic: **Two sum classic hash**.
...
Single pass: for each `a[i]`, check if `target - a[i]` exists in map.
...
Think of **Two sum classic hash** as a coat-check counter — give a key, get back the item in O(1).
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Two sum classic hash
int main() {
...
- **Core idea:** Two sum classic hash — O(1) average lookup via hash function.
