...
## 1. Problem Statement
Master the topic: **Count-min sketch**.
...
This problem exercises **Count-min sketch** — a core concept in Hashing & Hash Tables.
...
Think of **Count-min sketch** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Count-min sketch
int solve_count_min_sketch(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_count_min_sketch(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Count-min sketch
int solve_count_min_sketch(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Count-min sketch
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Count-min sketch — O(1) average lookup via hash function.
