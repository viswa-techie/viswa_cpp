# Chapter: Polynomial hash collision probability
...
## 1. Problem Statement
Master the topic: **Polynomial hash collision probability**.
...
This problem exercises **Polynomial hash collision probability** — a core concept in Hashing & Hash Tables.
...
Think of **Polynomial hash collision probability** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Polynomial hash collision probability
int solve_polynomial_hash_collision_probability(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_polynomial_hash_collision_probability(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Polynomial hash collision probability
int solve_polynomial_hash_collision_probability(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Polynomial hash collision probability
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Polynomial hash collision probability — O(1) average lookup via hash function.
