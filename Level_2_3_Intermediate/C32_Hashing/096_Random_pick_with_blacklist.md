# Chapter: Random pick with blacklist
...
## 1. Problem Statement
Master the topic: **Random pick with blacklist**.
...
This problem exercises **Random pick with blacklist** — a core concept in Hashing & Hash Tables.
...
Think of **Random pick with blacklist** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Random pick with blacklist
int solve_random_pick_with_blacklist(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_random_pick_with_blacklist(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Random pick with blacklist
int solve_random_pick_with_blacklist(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Random pick with blacklist
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Random pick with blacklist — O(1) average lookup via hash function.
