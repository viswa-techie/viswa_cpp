...
## 1. Problem Statement
Master the topic: **Contains duplicate**.
...
This problem exercises **Contains duplicate** — a core concept in Hashing & Hash Tables.
...
Think of **Contains duplicate** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Contains duplicate
int solve_contains_duplicate(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_contains_duplicate(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Contains duplicate
int solve_contains_duplicate(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Contains duplicate
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Contains duplicate — O(1) average lookup via hash function.
