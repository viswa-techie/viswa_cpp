...
This problem exercises **Bloom filter** — a core concept in Hashing & Hash Tables.
...
Think of **Bloom filter** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Bloom filter
int solve_bloom_filter(std::vector<int> v) {
    int result = 0;
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Bloom filter
int solve_bloom_filter(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Bloom filter
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Bloom filter — O(1) average lookup via hash function.
