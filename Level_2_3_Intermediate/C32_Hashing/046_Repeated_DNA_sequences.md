# Chapter: Repeated DNA sequences
...
## 1. Problem Statement
Master the topic: **Repeated DNA sequences**.
...
This problem exercises **Repeated DNA sequences** — a core concept in Hashing & Hash Tables.
...
Think of **Repeated DNA sequences** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Repeated DNA sequences
int solve_repeated_dna_sequences(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_repeated_dna_sequences(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Repeated DNA sequences
int solve_repeated_dna_sequences(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Repeated DNA sequences
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Repeated DNA sequences — O(1) average lookup via hash function.
