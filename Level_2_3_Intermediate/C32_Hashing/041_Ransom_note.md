...
This problem exercises **Ransom note** — a core concept in Hashing & Hash Tables.
...
Think of **Ransom note** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Ransom note
int solve_ransom_note(std::vector<int> v) {
    int result = 0;
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Ransom note
int solve_ransom_note(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Ransom note
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Ransom note — O(1) average lookup via hash function.
