...
## 1. Problem Statement
Master the topic: **Dynamic resizing**.
...
This problem exercises **Dynamic resizing** — a core concept in Hashing & Hash Tables.
...
Think of **Dynamic resizing** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Dynamic resizing
int solve_dynamic_resizing(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_dynamic_resizing(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Dynamic resizing
int solve_dynamic_resizing(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Dynamic resizing
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Dynamic resizing — O(1) average lookup via hash function.
