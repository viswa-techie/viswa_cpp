...
## 1. Problem Statement
Master the topic: **Subdomain visit count**.
...
This problem exercises **Subdomain visit count** — a core concept in Hashing & Hash Tables.
...
Think of **Subdomain visit count** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Subdomain visit count
int solve_subdomain_visit_count(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_subdomain_visit_count(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Subdomain visit count
int solve_subdomain_visit_count(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Subdomain visit count
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Subdomain visit count — O(1) average lookup via hash function.
