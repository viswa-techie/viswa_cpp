# Chapter: Count number of texts hash DP
...
## 1. Problem Statement
Master the topic: **Count number of texts hash DP**.
...
This problem exercises **Count number of texts hash DP** — a core concept in Hashing & Hash Tables.
...
Think of **Count number of texts hash DP** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Count number of texts hash DP
int solve_count_number_of_texts_hash_dp(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_count_number_of_texts_hash_dp(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Count number of texts hash DP
int solve_count_number_of_texts_hash_dp(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Count number of texts hash DP
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Count number of texts hash DP — O(1) average lookup via hash function.
