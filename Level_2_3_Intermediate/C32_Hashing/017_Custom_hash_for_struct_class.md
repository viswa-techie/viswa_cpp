# Chapter: Custom hash for struct class
...
## 1. Problem Statement
Master the topic: **Custom hash for struct class**.
...
This problem exercises **Custom hash for struct class** — a core concept in Hashing & Hash Tables.
...
Think of **Custom hash for struct class** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Custom hash for struct class
int solve_custom_hash_for_struct_class(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_custom_hash_for_struct_class(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Custom hash for struct class
int solve_custom_hash_for_struct_class(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Custom hash for struct class
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Custom hash for struct class — O(1) average lookup via hash function.
