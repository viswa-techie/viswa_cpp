# Chapter: Universal hashing concept
...
## 1. Problem Statement
Master the topic: **Universal hashing concept**.
...
This problem exercises **Universal hashing concept** — a core concept in Hashing & Hash Tables.
...
Think of **Universal hashing concept** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Universal hashing concept
int solve_universal_hashing_concept(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_universal_hashing_concept(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Universal hashing concept
int solve_universal_hashing_concept(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Universal hashing concept
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Universal hashing concept — O(1) average lookup via hash function.
