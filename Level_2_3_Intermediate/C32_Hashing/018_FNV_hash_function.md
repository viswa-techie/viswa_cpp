...
## 1. Problem Statement
Master the topic: **FNV hash function**.
...
This problem exercises **FNV hash function** — a core concept in Hashing & Hash Tables.
...
Think of **FNV hash function** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: FNV hash function
int solve_fnv_hash_function(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_fnv_hash_function(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: FNV hash function
int solve_fnv_hash_function(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: FNV hash function
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** FNV hash function — O(1) average lookup via hash function.
