# Chapter: Rolling hash Rabin-Karp
...
## 1. Problem Statement
Master the topic: **Rolling hash Rabin-Karp**.
...
This problem exercises **Rolling hash Rabin-Karp** — a core concept in Hashing & Hash Tables.
...
Think of **Rolling hash Rabin-Karp** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Rolling hash Rabin-Karp
int solve_rolling_hash_rabin_karp(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_rolling_hash_rabin_karp(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Rolling hash Rabin-Karp
int solve_rolling_hash_rabin_karp(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Rolling hash Rabin-Karp
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Rolling hash Rabin-Karp — O(1) average lookup via hash function.
