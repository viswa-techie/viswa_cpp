# Chapter: Max frequency stack FreqStack
...
## 1. Problem Statement
Master the topic: **Max frequency stack FreqStack**.
...
This problem exercises **Max frequency stack FreqStack** — a core concept in Hashing & Hash Tables.
...
Think of **Max frequency stack FreqStack** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Max frequency stack FreqStack
int solve_max_frequency_stack_freqstack(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_max_frequency_stack_freqstack(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Max frequency stack FreqStack
int solve_max_frequency_stack_freqstack(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Max frequency stack FreqStack
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Max frequency stack FreqStack — O(1) average lookup via hash function.
