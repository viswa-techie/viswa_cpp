...
## 1. Problem Statement
Master the topic: **Jewels and stones**.
...
This problem exercises **Jewels and stones** — a core concept in Hashing & Hash Tables.
...
Think of **Jewels and stones** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Jewels and stones
int solve_jewels_and_stones(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_jewels_and_stones(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Jewels and stones
int solve_jewels_and_stones(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Jewels and stones
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Jewels and stones — O(1) average lookup via hash function.
