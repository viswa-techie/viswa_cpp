# Chapter: Anti-hash test worst case
...
## 1. Problem Statement
Master the topic: **Anti-hash test worst case**.
...
This problem exercises **Anti-hash test worst case** — a core concept in Hashing & Hash Tables.
...
Think of **Anti-hash test worst case** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Anti-hash test worst case
int solve_anti_hash_test_worst_case(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_anti_hash_test_worst_case(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Anti-hash test worst case
int solve_anti_hash_test_worst_case(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Anti-hash test worst case
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Anti-hash test worst case — O(1) average lookup via hash function.
