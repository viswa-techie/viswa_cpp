# Chapter: Happy number cycle detect
...
## 1. Problem Statement
Master the topic: **Happy number cycle detect**.
...
This problem exercises **Happy number cycle detect** — a core concept in Hashing & Hash Tables.
...
Think of **Happy number cycle detect** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Happy number cycle detect
int solve_happy_number_cycle_detect(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_happy_number_cycle_detect(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Happy number cycle detect
int solve_happy_number_cycle_detect(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Happy number cycle detect
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Happy number cycle detect — O(1) average lookup via hash function.
