...
## 1. Problem Statement
Master the topic: **Chaining separate**.
...
This problem exercises **Chaining separate** — a core concept in Hashing & Hash Tables.
...
Think of **Chaining separate** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Chaining separate
int solve_chaining_separate(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_chaining_separate(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Chaining separate
int solve_chaining_separate(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Chaining separate
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Chaining separate — O(1) average lookup via hash function.
