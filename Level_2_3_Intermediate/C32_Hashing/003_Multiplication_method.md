...
## 1. Problem Statement
Master the topic: **Multiplication method**.
...
This problem exercises **Multiplication method** — a core concept in Hashing & Hash Tables.
...
Think of **Multiplication method** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Multiplication method
int solve_multiplication_method(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_multiplication_method(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Multiplication method
int solve_multiplication_method(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Multiplication method
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Multiplication method — O(1) average lookup via hash function.
