# Chapter: CityHash SipHash concept
...
## 1. Problem Statement
Master the topic: **CityHash SipHash concept**.
...
This problem exercises **CityHash SipHash concept** — a core concept in Hashing & Hash Tables.
...
Think of **CityHash SipHash concept** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: CityHash SipHash concept
int solve_cityhash_siphash_concept(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_cityhash_siphash_concept(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: CityHash SipHash concept
int solve_cityhash_siphash_concept(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: CityHash SipHash concept
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** CityHash SipHash concept — O(1) average lookup via hash function.
