# Chapter: Longest subarray with sum k
...
## 1. Problem Statement
Master the topic: **Longest subarray with sum k**.
...
This problem exercises **Longest subarray with sum k** — a core concept in Hashing & Hash Tables.
...
Think of **Longest subarray with sum k** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Longest subarray with sum k
int solve_longest_subarray_with_sum_k(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_longest_subarray_with_sum_k(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Longest subarray with sum k
int solve_longest_subarray_with_sum_k(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Longest subarray with sum k
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Longest subarray with sum k — O(1) average lookup via hash function.
