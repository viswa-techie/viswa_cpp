# Chapter: Minimum window containing all chars
...
## 1. Problem Statement
Master the topic: **Minimum window containing all chars**.
...
This problem exercises **Minimum window containing all chars** — a core concept in Hashing & Hash Tables.
...
Think of **Minimum window containing all chars** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Minimum window containing all chars
int solve_minimum_window_containing_all_chars(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_minimum_window_containing_all_chars(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Minimum window containing all chars
int solve_minimum_window_containing_all_chars(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Minimum window containing all chars
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Minimum window containing all chars — O(1) average lookup via hash function.
