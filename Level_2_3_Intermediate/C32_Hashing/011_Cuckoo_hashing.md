...
## 1. Problem Statement
Master the topic: **Cuckoo hashing**.
...
This problem exercises **Cuckoo hashing** — a core concept in Hashing & Hash Tables.
...
Think of **Cuckoo hashing** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Cuckoo hashing
int solve_cuckoo_hashing(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_cuckoo_hashing(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Cuckoo hashing
int solve_cuckoo_hashing(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Cuckoo hashing
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Cuckoo hashing — O(1) average lookup via hash function.
