# Chapter: Equal row and column pairs
...
## 1. Problem Statement
Master the topic: **Equal row and column pairs**.
...
This problem exercises **Equal row and column pairs** — a core concept in Hashing & Hash Tables.
...
Think of **Equal row and column pairs** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Equal row and column pairs
int solve_equal_row_and_column_pairs(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_equal_row_and_column_pairs(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Equal row and column pairs
int solve_equal_row_and_column_pairs(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Equal row and column pairs
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Equal row and column pairs — O(1) average lookup via hash function.
