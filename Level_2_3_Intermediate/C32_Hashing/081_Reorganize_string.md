...
## 1. Problem Statement
Master the topic: **Reorganize string**.
...
This problem exercises **Reorganize string** — a core concept in Hashing & Hash Tables.
...
Think of **Reorganize string** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Reorganize string
int solve_reorganize_string(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_reorganize_string(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Reorganize string
int solve_reorganize_string(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Reorganize string
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Reorganize string — O(1) average lookup via hash function.
