# Chapter: Continuous subarray sum k n
...
## 1. Problem Statement
Master the topic: **Continuous subarray sum k n**.
...
This problem exercises **Continuous subarray sum k n** — a core concept in Hashing & Hash Tables.
...
Think of **Continuous subarray sum k n** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Continuous subarray sum k n
int solve_continuous_subarray_sum_k_n(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_continuous_subarray_sum_k_n(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Continuous subarray sum k n
int solve_continuous_subarray_sum_k_n(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Continuous subarray sum k n
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Continuous subarray sum k n — O(1) average lookup via hash function.
