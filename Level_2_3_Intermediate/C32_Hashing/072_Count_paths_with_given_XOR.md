# Chapter: Count paths with given XOR
...
## 1. Problem Statement
Master the topic: **Count paths with given XOR**.
...
This problem exercises **Count paths with given XOR** — a core concept in Hashing & Hash Tables.
...
Think of **Count paths with given XOR** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Count paths with given XOR
int solve_count_paths_with_given_xor(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_count_paths_with_given_xor(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Count paths with given XOR
int solve_count_paths_with_given_xor(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Count paths with given XOR
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Count paths with given XOR — O(1) average lookup via hash function.
