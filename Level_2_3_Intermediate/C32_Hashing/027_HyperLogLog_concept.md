...
## 1. Problem Statement
Master the topic: **HyperLogLog concept**.
...
This problem exercises **HyperLogLog concept** — a core concept in Hashing & Hash Tables.
...
Think of **HyperLogLog concept** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: HyperLogLog concept
int solve_hyperloglog_concept(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_hyperloglog_concept(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: HyperLogLog concept
int solve_hyperloglog_concept(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: HyperLogLog concept
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** HyperLogLog concept — O(1) average lookup via hash function.
