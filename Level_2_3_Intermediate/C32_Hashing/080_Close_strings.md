...
## 1. Problem Statement
Master the topic: **Close strings**.
...
This problem exercises **Close strings** — a core concept in Hashing & Hash Tables.
...
Think of **Close strings** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Close strings
int solve_close_strings(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_close_strings(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Close strings
int solve_close_strings(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Close strings
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Close strings — O(1) average lookup via hash function.
