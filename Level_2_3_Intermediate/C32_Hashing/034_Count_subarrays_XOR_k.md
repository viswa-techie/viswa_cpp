...
## 1. Problem Statement
Master the topic: **Count subarrays XOR k**.
...
This problem exercises **Count subarrays XOR k** — a core concept in Hashing & Hash Tables.
...
Think of **Count subarrays XOR k** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Count subarrays XOR k
int solve_count_subarrays_xor_k(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_count_subarrays_xor_k(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Count subarrays XOR k
int solve_count_subarrays_xor_k(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Count subarrays XOR k
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Count subarrays XOR k — O(1) average lookup via hash function.
