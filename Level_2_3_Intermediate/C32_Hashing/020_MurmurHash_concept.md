...
## 1. Problem Statement
Master the topic: **MurmurHash concept**.
...
This problem exercises **MurmurHash concept** — a core concept in Hashing & Hash Tables.
...
Think of **MurmurHash concept** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: MurmurHash concept
int solve_murmurhash_concept(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_murmurhash_concept(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: MurmurHash concept
int solve_murmurhash_concept(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: MurmurHash concept
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** MurmurHash concept — O(1) average lookup via hash function.
