# Chapter: Fingerprinting with hash file dedup
...
## 1. Problem Statement
Master the topic: **Fingerprinting with hash file dedup**.
...
This problem exercises **Fingerprinting with hash file dedup** — a core concept in Hashing & Hash Tables.
...
Think of **Fingerprinting with hash file dedup** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Fingerprinting with hash file dedup
int solve_fingerprinting_with_hash_file_dedup(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_fingerprinting_with_hash_file_dedup(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Fingerprinting with hash file dedup
int solve_fingerprinting_with_hash_file_dedup(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Fingerprinting with hash file dedup
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Fingerprinting with hash file dedup — O(1) average lookup via hash function.
