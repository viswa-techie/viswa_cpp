# Chapter: Longest equal 0s and 1s
...
## 1. Problem Statement
Master the topic: **Longest equal 0s and 1s**.
...
This problem exercises **Longest equal 0s and 1s** — a core concept in Hashing & Hash Tables.
...
Think of **Longest equal 0s and 1s** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Longest equal 0s and 1s
int solve_longest_equal_0s_and_1s(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_longest_equal_0s_and_1s(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Longest equal 0s and 1s
int solve_longest_equal_0s_and_1s(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Longest equal 0s and 1s
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Longest equal 0s and 1s — O(1) average lookup via hash function.
