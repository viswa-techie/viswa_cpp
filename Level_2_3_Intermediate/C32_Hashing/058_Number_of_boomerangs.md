...
## 1. Problem Statement
Master the topic: **Number of boomerangs**.
...
This problem exercises **Number of boomerangs** — a core concept in Hashing & Hash Tables.
...
Think of **Number of boomerangs** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Number of boomerangs
int solve_number_of_boomerangs(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_number_of_boomerangs(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Number of boomerangs
int solve_number_of_boomerangs(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Number of boomerangs
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Number of boomerangs — O(1) average lookup via hash function.
