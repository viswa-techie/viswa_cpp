# Chapter: Unique number of occurrences
...
## 1. Problem Statement
Master the topic: **Unique number of occurrences**.
...
This problem exercises **Unique number of occurrences** — a core concept in Hashing & Hash Tables.
...
Think of **Unique number of occurrences** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Unique number of occurrences
int solve_unique_number_of_occurrences(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_unique_number_of_occurrences(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Unique number of occurrences
int solve_unique_number_of_occurrences(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Unique number of occurrences
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Unique number of occurrences — O(1) average lookup via hash function.
