# Chapter: Design in-memory file system
...
## 1. Problem Statement
Master the topic: **Design in-memory file system**.
...
This problem exercises **Design in-memory file system** — a core concept in Hashing & Hash Tables.
...
Think of **Design in-memory file system** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Design in-memory file system
int solve_design_in_memory_file_system(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_design_in_memory_file_system(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Design in-memory file system
int solve_design_in_memory_file_system(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Design in-memory file system
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Design in-memory file system — O(1) average lookup via hash function.
