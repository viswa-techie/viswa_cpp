# Chapter: Find all duplicates in array
...
## 1. Problem Statement
Master the topic: **Find all duplicates in array**.
...
This problem exercises **Find all duplicates in array** — a core concept in Hashing & Hash Tables.
...
Think of **Find all duplicates in array** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Find all duplicates in array
int solve_find_all_duplicates_in_array(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_find_all_duplicates_in_array(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Find all duplicates in array
int solve_find_all_duplicates_in_array(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Find all duplicates in array
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Find all duplicates in array — O(1) average lookup via hash function.
