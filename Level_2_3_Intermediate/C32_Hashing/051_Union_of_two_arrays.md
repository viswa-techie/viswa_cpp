...
## 1. Problem Statement
Master the topic: **Union of two arrays**.
...
This problem exercises **Union of two arrays** — a core concept in Hashing & Hash Tables.
...
Think of **Union of two arrays** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Union of two arrays
int solve_union_of_two_arrays(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_union_of_two_arrays(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Union of two arrays
int solve_union_of_two_arrays(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Union of two arrays
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Union of two arrays — O(1) average lookup via hash function.
