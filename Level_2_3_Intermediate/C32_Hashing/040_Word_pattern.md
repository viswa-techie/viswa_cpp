...
This problem exercises **Word pattern** — a core concept in Hashing & Hash Tables.
...
Think of **Word pattern** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Word pattern
int solve_word_pattern(std::vector<int> v) {
    int result = 0;
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Word pattern
int solve_word_pattern(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Word pattern
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Word pattern — O(1) average lookup via hash function.
