# Chapter: Amortized cost analysis
...
## 1. Problem Statement
Master the topic: **Amortized cost analysis**.
...
This problem exercises **Amortized cost analysis** — a core concept in Hashing & Hash Tables.
...
Think of **Amortized cost analysis** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Amortized cost analysis
int solve_amortized_cost_analysis(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_amortized_cost_analysis(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Amortized cost analysis
int solve_amortized_cost_analysis(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Amortized cost analysis
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Amortized cost analysis — O(1) average lookup via hash function.
