...
## 1. Problem Statement
Master the topic: **Find anagram mappings**.
...
This problem exercises **Find anagram mappings** — a core concept in Hashing & Hash Tables.
...
Think of **Find anagram mappings** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Find anagram mappings
int solve_find_anagram_mappings(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_find_anagram_mappings(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Find anagram mappings
int solve_find_anagram_mappings(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Find anagram mappings
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Find anagram mappings — O(1) average lookup via hash function.
