...
## 1. Problem Statement
Master the topic: **Common characters**.
...
This problem exercises **Common characters** — a core concept in Hashing & Hash Tables.
...
Think of **Common characters** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Common characters
int solve_common_characters(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_common_characters(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Common characters
int solve_common_characters(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Common characters
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Common characters — O(1) average lookup via hash function.
