# Chapter: Encode and decode TinyURL
...
## 1. Problem Statement
Master the topic: **Encode and decode TinyURL**.
...
This problem exercises **Encode and decode TinyURL** — a core concept in Hashing & Hash Tables.
...
Think of **Encode and decode TinyURL** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Encode and decode TinyURL
int solve_encode_and_decode_tinyurl(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_encode_and_decode_tinyurl(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Encode and decode TinyURL
int solve_encode_and_decode_tinyurl(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Encode and decode TinyURL
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Encode and decode TinyURL — O(1) average lookup via hash function.
