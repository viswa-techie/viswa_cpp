...
## 1. Problem Statement
Master the topic: **Destination city hash**.
...
This problem exercises **Destination city hash** — a core concept in Hashing & Hash Tables.
...
Think of **Destination city hash** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Destination city hash
int solve_destination_city_hash(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_destination_city_hash(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Destination city hash
int solve_destination_city_hash(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Destination city hash
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Destination city hash — O(1) average lookup via hash function.
