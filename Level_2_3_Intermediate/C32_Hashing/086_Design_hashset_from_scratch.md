# Chapter: Design hashset from scratch
...
## 1. Problem Statement
Master the topic: **Design hashset from scratch**.
...
This problem exercises **Design hashset from scratch** — a core concept in Hashing & Hash Tables.
...
Think of **Design hashset from scratch** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Design hashset from scratch
int solve_design_hashset_from_scratch(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_design_hashset_from_scratch(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Design hashset from scratch
int solve_design_hashset_from_scratch(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Design hashset from scratch
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Design hashset from scratch — O(1) average lookup via hash function.
