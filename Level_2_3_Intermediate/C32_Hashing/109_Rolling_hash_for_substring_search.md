# Chapter: Rolling hash for substring search
...
## 1. Problem Statement
Master the topic: **Rolling hash for substring search**.
...
This problem exercises **Rolling hash for substring search** — a core concept in Hashing & Hash Tables.
...
Think of **Rolling hash for substring search** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Rolling hash for substring search
int solve_rolling_hash_for_substring_search(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_rolling_hash_for_substring_search(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Rolling hash for substring search
int solve_rolling_hash_for_substring_search(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Rolling hash for substring search
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Rolling hash for substring search — O(1) average lookup via hash function.
