# Chapter: Word frequency counter
...
## 1. Problem Statement
Master the topic: **Word frequency counter**.
...
This problem exercises **Word frequency counter** — a core concept in Hashing & Hash Tables.
...
Think of **Word frequency counter** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Word frequency counter
int solve_word_frequency_counter(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_word_frequency_counter(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Word frequency counter
int solve_word_frequency_counter(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Word frequency counter
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Word frequency counter — O(1) average lookup via hash function.
