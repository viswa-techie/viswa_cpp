# Chapter: Find all disappeared numbers
...
## 1. Problem Statement
Master the topic: **Find all disappeared numbers**.
...
This problem exercises **Find all disappeared numbers** — a core concept in Hashing & Hash Tables.
...
Think of **Find all disappeared numbers** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Find all disappeared numbers
int solve_find_all_disappeared_numbers(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_find_all_disappeared_numbers(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Find all disappeared numbers
int solve_find_all_disappeared_numbers(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Find all disappeared numbers
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Find all disappeared numbers — O(1) average lookup via hash function.
