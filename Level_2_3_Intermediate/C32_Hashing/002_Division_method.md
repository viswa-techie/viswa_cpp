...
## 1. Problem Statement
Master the topic: **Division method**.
...
This problem exercises **Division method** — a core concept in Hashing & Hash Tables.
...
Think of **Division method** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Division method
int solve_division_method(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_division_method(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Division method
int solve_division_method(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Division method
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Division method — O(1) average lookup via hash function.
