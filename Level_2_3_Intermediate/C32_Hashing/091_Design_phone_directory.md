# Chapter: Design phone directory
...
## 1. Problem Statement
Master the topic: **Design phone directory**.
...
This problem exercises **Design phone directory** — a core concept in Hashing & Hash Tables.
...
Think of **Design phone directory** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Design phone directory
int solve_design_phone_directory(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_design_phone_directory(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Design phone directory
int solve_design_phone_directory(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Design phone directory
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Design phone directory — O(1) average lookup via hash function.
