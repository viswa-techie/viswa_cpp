# Chapter: LRU Cache hash plus DLL
...
## 1. Problem Statement
Master the topic: **LRU Cache hash plus DLL**.
...
This problem exercises **LRU Cache hash plus DLL** — a core concept in Hashing & Hash Tables.
...
Think of **LRU Cache hash plus DLL** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: LRU Cache hash plus DLL
int solve_lru_cache_hash_plus_dll(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_lru_cache_hash_plus_dll(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: LRU Cache hash plus DLL
int solve_lru_cache_hash_plus_dll(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: LRU Cache hash plus DLL
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** LRU Cache hash plus DLL — O(1) average lookup via hash function.
