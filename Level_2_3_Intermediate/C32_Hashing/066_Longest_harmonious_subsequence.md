# Chapter: Longest harmonious subsequence
...
## 1. Problem Statement
Master the topic: **Longest harmonious subsequence**.
...
This problem exercises **Longest harmonious subsequence** — a core concept in Hashing & Hash Tables.
...
Think of **Longest harmonious subsequence** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Longest harmonious subsequence
int solve_longest_harmonious_subsequence(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_longest_harmonious_subsequence(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Longest harmonious subsequence
int solve_longest_harmonious_subsequence(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Longest harmonious subsequence
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Longest harmonious subsequence — O(1) average lookup via hash function.
