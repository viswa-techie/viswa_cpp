# Chapter: Count good meals power of 2 sum
...
## 1. Problem Statement
Master the topic: **Count good meals power of 2 sum**.
...
This problem exercises **Count good meals power of 2 sum** — a core concept in Hashing & Hash Tables.
...
Think of **Count good meals power of 2 sum** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Count good meals power of 2 sum
int solve_count_good_meals_power_of_2_sum(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_count_good_meals_power_of_2_sum(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Count good meals power of 2 sum
int solve_count_good_meals_power_of_2_sum(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Count good meals power of 2 sum
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Count good meals power of 2 sum — O(1) average lookup via hash function.
