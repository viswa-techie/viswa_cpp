...
## 1. Problem Statement
Master the topic: **Subarray zero sum**.
...
This problem exercises **Subarray zero sum** — a core concept in Hashing & Hash Tables.
...
Think of **Subarray zero sum** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Subarray zero sum
int solve_subarray_zero_sum(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_subarray_zero_sum(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Subarray zero sum
int solve_subarray_zero_sum(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Subarray zero sum
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Subarray zero sum — O(1) average lookup via hash function.
