...
## 1. Problem Statement
Master the topic: **Robin Hood hashing**.
...
This problem exercises **Robin Hood hashing** — a core concept in Hashing & Hash Tables.
...
Think of **Robin Hood hashing** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Robin Hood hashing
int solve_robin_hood_hashing(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_robin_hood_hashing(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Robin Hood hashing
int solve_robin_hood_hashing(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Robin Hood hashing
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Robin Hood hashing — O(1) average lookup via hash function.
