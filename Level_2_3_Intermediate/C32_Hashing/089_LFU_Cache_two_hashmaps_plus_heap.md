# Chapter: LFU Cache two hashmaps plus heap
...
## 1. Problem Statement
Master the topic: **LFU Cache two hashmaps plus heap**.
...
This problem exercises **LFU Cache two hashmaps plus heap** — a core concept in Hashing & Hash Tables.
...
Think of **LFU Cache two hashmaps plus heap** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: LFU Cache two hashmaps plus heap
int solve_lfu_cache_two_hashmaps_plus_heap(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_lfu_cache_two_hashmaps_plus_heap(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: LFU Cache two hashmaps plus heap
int solve_lfu_cache_two_hashmaps_plus_heap(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: LFU Cache two hashmaps plus heap
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** LFU Cache two hashmaps plus heap — O(1) average lookup via hash function.
