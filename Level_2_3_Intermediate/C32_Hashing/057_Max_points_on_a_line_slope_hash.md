# Chapter: Max points on a line slope hash
...
## 1. Problem Statement
Master the topic: **Max points on a line slope hash**.
...
This problem exercises **Max points on a line slope hash** — a core concept in Hashing & Hash Tables.
...
Think of **Max points on a line slope hash** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Max points on a line slope hash
int solve_max_points_on_a_line_slope_hash(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_max_points_on_a_line_slope_hash(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Max points on a line slope hash
int solve_max_points_on_a_line_slope_hash(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Max points on a line slope hash
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Max points on a line slope hash — O(1) average lookup via hash function.
