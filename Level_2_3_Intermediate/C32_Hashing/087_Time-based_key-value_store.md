# Chapter: Time-based key-value store
...
## 1. Problem Statement
Master the topic: **Time-based key-value store**.
...
This problem exercises **Time-based key-value store** — a core concept in Hashing & Hash Tables.
...
Think of **Time-based key-value store** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Time-based key-value store
int solve_time_based_key_value_store(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_time_based_key_value_store(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Time-based key-value store
int solve_time_based_key_value_store(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Time-based key-value store
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Time-based key-value store — O(1) average lookup via hash function.
