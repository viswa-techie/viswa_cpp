# Chapter: Minimum consecutive cards to pick up
...
## 1. Problem Statement
Master the topic: **Minimum consecutive cards to pick up**.
...
This problem exercises **Minimum consecutive cards to pick up** — a core concept in Hashing & Hash Tables.
...
Think of **Minimum consecutive cards to pick up** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Minimum consecutive cards to pick up
int solve_minimum_consecutive_cards_to_pick_up(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_minimum_consecutive_cards_to_pick_up(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Minimum consecutive cards to pick up
int solve_minimum_consecutive_cards_to_pick_up(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Minimum consecutive cards to pick up
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Minimum consecutive cards to pick up — O(1) average lookup via hash function.
