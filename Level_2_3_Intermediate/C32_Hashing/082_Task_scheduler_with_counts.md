# Chapter: Task scheduler with counts
...
## 1. Problem Statement
Master the topic: **Task scheduler with counts**.
...
This problem exercises **Task scheduler with counts** — a core concept in Hashing & Hash Tables.
...
Think of **Task scheduler with counts** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Task scheduler with counts
int solve_task_scheduler_with_counts(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_task_scheduler_with_counts(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Task scheduler with counts
int solve_task_scheduler_with_counts(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Task scheduler with counts
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Task scheduler with counts — O(1) average lookup via hash function.
