# Chapter: Polynomial rolling hash
...
## 1. Problem Statement
Master the topic: **Polynomial rolling hash**.
...
This problem exercises **Polynomial rolling hash** — a core concept in Hashing & Hash Tables.
...
Think of **Polynomial rolling hash** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Polynomial rolling hash
int solve_polynomial_rolling_hash(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_polynomial_rolling_hash(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Polynomial rolling hash
int solve_polynomial_rolling_hash(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Polynomial rolling hash
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Polynomial rolling hash — O(1) average lookup via hash function.
