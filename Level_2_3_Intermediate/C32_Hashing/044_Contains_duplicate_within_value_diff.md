# Chapter: Contains duplicate within value diff
...
## 1. Problem Statement
Master the topic: **Contains duplicate within value diff**.
...
This problem exercises **Contains duplicate within value diff** — a core concept in Hashing & Hash Tables.
...
Think of **Contains duplicate within value diff** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Contains duplicate within value diff
int solve_contains_duplicate_within_value_diff(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_contains_duplicate_within_value_diff(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Contains duplicate within value diff
int solve_contains_duplicate_within_value_diff(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Contains duplicate within value diff
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Contains duplicate within value diff — O(1) average lookup via hash function.
