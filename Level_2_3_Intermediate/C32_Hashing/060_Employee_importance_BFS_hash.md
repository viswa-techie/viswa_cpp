# Chapter: Employee importance BFS hash
...
## 1. Problem Statement
Master the topic: **Employee importance BFS hash**.
...
This problem exercises **Employee importance BFS hash** — a core concept in Hashing & Hash Tables.
...
Think of **Employee importance BFS hash** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Employee importance BFS hash
int solve_employee_importance_bfs_hash(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_employee_importance_bfs_hash(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Employee importance BFS hash
int solve_employee_importance_bfs_hash(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Employee importance BFS hash
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Employee importance BFS hash — O(1) average lookup via hash function.
