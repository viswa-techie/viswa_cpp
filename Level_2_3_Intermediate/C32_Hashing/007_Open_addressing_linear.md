# Chapter: Open addressing linear
...
## 1. Problem Statement
Master the topic: **Open addressing linear**.
...
This problem exercises **Open addressing linear** — a core concept in Hashing & Hash Tables.
...
Think of **Open addressing linear** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Open addressing linear
int solve_open_addressing_linear(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_open_addressing_linear(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Open addressing linear
int solve_open_addressing_linear(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Open addressing linear
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Open addressing linear — O(1) average lookup via hash function.
