# Chapter: Hash table from scratch
...
## 1. Problem Statement
Master the topic: **Hash table from scratch**.
...
This problem exercises **Hash table from scratch** — a core concept in Hashing & Hash Tables.
...
Think of **Hash table from scratch** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Hash table from scratch
int solve_hash_table_from_scratch(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_hash_table_from_scratch(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Hash table from scratch
int solve_hash_table_from_scratch(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Hash table from scratch
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Hash table from scratch — O(1) average lookup via hash function.
