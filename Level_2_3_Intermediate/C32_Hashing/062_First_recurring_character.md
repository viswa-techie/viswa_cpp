# Chapter: First recurring character
...
## 1. Problem Statement
Master the topic: **First recurring character**.
...
This problem exercises **First recurring character** — a core concept in Hashing & Hash Tables.
...
Think of **First recurring character** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: First recurring character
int solve_first_recurring_character(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_first_recurring_character(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: First recurring character
int solve_first_recurring_character(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: First recurring character
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** First recurring character — O(1) average lookup via hash function.
