...
## 1. Problem Statement
Master the topic: **Quadratic probing**.
...
This problem exercises **Quadratic probing** — a core concept in Hashing & Hash Tables.
...
Think of **Quadratic probing** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Quadratic probing
int solve_quadratic_probing(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_quadratic_probing(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Quadratic probing
int solve_quadratic_probing(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Quadratic probing
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Quadratic probing — O(1) average lookup via hash function.
