# Chapter: Isomorphic strings hash
...
## 1. Problem Statement
Master the topic: **Isomorphic strings hash**.
...
This problem exercises **Isomorphic strings hash** — a core concept in Hashing & Hash Tables.
...
Think of **Isomorphic strings hash** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Isomorphic strings hash
int solve_isomorphic_strings_hash(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_isomorphic_strings_hash(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Isomorphic strings hash
int solve_isomorphic_strings_hash(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Isomorphic strings hash
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Isomorphic strings hash — O(1) average lookup via hash function.
