# Chapter: Distinct values in window
...
## 1. Problem Statement
Master the topic: **Distinct values in window**.
...
This problem exercises **Distinct values in window** — a core concept in Hashing & Hash Tables.
...
Think of **Distinct values in window** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Distinct values in window
int solve_distinct_values_in_window(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_distinct_values_in_window(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Distinct values in window
int solve_distinct_values_in_window(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Distinct values in window
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Distinct values in window — O(1) average lookup via hash function.
