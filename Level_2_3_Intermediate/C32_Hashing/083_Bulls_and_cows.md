...
## 1. Problem Statement
Master the topic: **Bulls and cows**.
...
This problem exercises **Bulls and cows** — a core concept in Hashing & Hash Tables.
...
Think of **Bulls and cows** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Bulls and cows
int solve_bulls_and_cows(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_bulls_and_cows(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Bulls and cows
int solve_bulls_and_cows(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Bulls and cows
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Bulls and cows — O(1) average lookup via hash function.
