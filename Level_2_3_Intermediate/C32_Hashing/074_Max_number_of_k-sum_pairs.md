# Chapter: Max number of k-sum pairs
...
## 1. Problem Statement
Master the topic: **Max number of k-sum pairs**.
...
This problem exercises **Max number of k-sum pairs** — a core concept in Hashing & Hash Tables.
...
Think of **Max number of k-sum pairs** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Max number of k-sum pairs
int solve_max_number_of_k_sum_pairs(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_max_number_of_k_sum_pairs(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Max number of k-sum pairs
int solve_max_number_of_k_sum_pairs(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Max number of k-sum pairs
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Max number of k-sum pairs — O(1) average lookup via hash function.
