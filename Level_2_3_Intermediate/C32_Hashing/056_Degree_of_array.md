...
## 1. Problem Statement
Master the topic: **Degree of array**.
...
This problem exercises **Degree of array** — a core concept in Hashing & Hash Tables.
...
Think of **Degree of array** as a coat-check counter — give a key, get back the item in O(1).
...
#include <vector>
#include <string>
// Brute-force template for: Degree of array
int solve_degree_of_array(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_degree_of_array(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Degree of array
int solve_degree_of_array(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Degree of array
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Degree of array — O(1) average lookup via hash function.
