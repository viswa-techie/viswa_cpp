# Chapter: Inheritance basics public
...
## 1. Problem Statement
Master the topic: **Inheritance basics public**.
...
This problem exercises **Inheritance basics public** — a core concept in OOP — Classes & Objects.
...
Think of **Inheritance basics public** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Inheritance basics public
int solve_inheritance_basics_public(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_inheritance_basics_public(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Inheritance basics public
int solve_inheritance_basics_public(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Inheritance basics public
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Inheritance basics public — model real-world entities with state + behavior.
