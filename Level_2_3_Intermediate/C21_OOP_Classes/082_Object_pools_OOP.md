...
## 1. Problem Statement
Master the topic: **Object pools OOP**.
...
This problem exercises **Object pools OOP** — a core concept in OOP — Classes & Objects.
...
Think of **Object pools OOP** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Object pools OOP
int solve_object_pools_oop(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_object_pools_oop(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Object pools OOP
int solve_object_pools_oop(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Object pools OOP
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Object pools OOP — model real-world entities with state + behavior.
