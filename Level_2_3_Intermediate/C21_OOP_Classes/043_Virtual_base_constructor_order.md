# Chapter: Virtual base constructor order
...
## 1. Problem Statement
Master the topic: **Virtual base constructor order**.
...
This problem exercises **Virtual base constructor order** — a core concept in OOP — Classes & Objects.
...
Think of **Virtual base constructor order** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Virtual base constructor order
int solve_virtual_base_constructor_order(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_virtual_base_constructor_order(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Virtual base constructor order
int solve_virtual_base_constructor_order(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Virtual base constructor order
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Virtual base constructor order — model real-world entities with state + behavior.
