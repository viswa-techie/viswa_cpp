...
## 1. Problem Statement
Master the topic: **Lazy initialization**.
...
This problem exercises **Lazy initialization** — a core concept in OOP — Classes & Objects.
...
Think of **Lazy initialization** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Lazy initialization
int solve_lazy_initialization(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_lazy_initialization(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Lazy initialization
int solve_lazy_initialization(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Lazy initialization
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Lazy initialization — model real-world entities with state + behavior.
