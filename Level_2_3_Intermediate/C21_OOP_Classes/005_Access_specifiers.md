...
## 1. Problem Statement
Master the topic: **Access specifiers**.
...
This problem exercises **Access specifiers** — a core concept in OOP — Classes & Objects.
...
Think of **Access specifiers** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Access specifiers
int solve_access_specifiers(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_access_specifiers(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Access specifiers
int solve_access_specifiers(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Access specifiers
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Access specifiers — model real-world entities with state + behavior.
