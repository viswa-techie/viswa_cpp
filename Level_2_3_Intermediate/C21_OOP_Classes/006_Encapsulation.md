...
## 1. Problem Statement
Master the topic: **Encapsulation**.
...
This problem exercises **Encapsulation** — a core concept in OOP — Classes & Objects.
...
Think of **Encapsulation** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Encapsulation
int solve_encapsulation(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_encapsulation(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Encapsulation
int solve_encapsulation(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Encapsulation
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Encapsulation — model real-world entities with state + behavior.
