# Chapter: Exception in destructor danger
...
## 1. Problem Statement
Master the topic: **Exception in destructor danger**.
...
This problem exercises **Exception in destructor danger** — a core concept in OOP — Classes & Objects.
...
Think of **Exception in destructor danger** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Exception in destructor danger
int solve_exception_in_destructor_danger(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_exception_in_destructor_danger(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Exception in destructor danger
int solve_exception_in_destructor_danger(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Exception in destructor danger
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Exception in destructor danger — model real-world entities with state + behavior.
