# Chapter: NVI Non-Virtual Interface
...
## 1. Problem Statement
Master the topic: **NVI Non-Virtual Interface**.
...
This problem exercises **NVI Non-Virtual Interface** — a core concept in OOP — Classes & Objects.
...
Think of **NVI Non-Virtual Interface** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: NVI Non-Virtual Interface
int solve_nvi_non_virtual_interface(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_nvi_non_virtual_interface(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: NVI Non-Virtual Interface
int solve_nvi_non_virtual_interface(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: NVI Non-Virtual Interface
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** NVI Non-Virtual Interface — model real-world entities with state + behavior.
