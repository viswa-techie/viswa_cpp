...
## 1. Problem Statement
Master the topic: **Comparison operators**.
...
This problem exercises **Comparison operators** — a core concept in OOP — Classes & Objects.
...
Think of **Comparison operators** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Comparison operators
int solve_comparison_operators(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_comparison_operators(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Comparison operators
int solve_comparison_operators(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Comparison operators
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Comparison operators — model real-world entities with state + behavior.
