# Chapter: Converting constructors
...
## 1. Problem Statement
Master the topic: **Converting constructors**.
...
This problem exercises **Converting constructors** — a core concept in OOP — Classes & Objects.
...
Think of **Converting constructors** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Converting constructors
int solve_converting_constructors(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_converting_constructors(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Converting constructors
int solve_converting_constructors(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Converting constructors
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Converting constructors — model real-world entities with state + behavior.
