...
## 1. Problem Statement
Master the topic: **Spaceship operator**.
...
This problem exercises **Spaceship operator** — a core concept in OOP — Classes & Objects.
...
Think of **Spaceship operator** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Spaceship operator
int solve_spaceship_operator(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_spaceship_operator(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Spaceship operator
int solve_spaceship_operator(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Spaceship operator
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Spaceship operator — model real-world entities with state + behavior.
