# Chapter: Delegating constructors
...
## 1. Problem Statement
Master the topic: **Delegating constructors**.
...
This problem exercises **Delegating constructors** — a core concept in OOP — Classes & Objects.
...
Think of **Delegating constructors** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Delegating constructors
int solve_delegating_constructors(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_delegating_constructors(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Delegating constructors
int solve_delegating_constructors(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Delegating constructors
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Delegating constructors — model real-world entities with state + behavior.
