...
## 1. Problem Statement
Master the topic: **Multiple inheritance**.
...
This problem exercises **Multiple inheritance** — a core concept in OOP — Classes & Objects.
...
Think of **Multiple inheritance** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Multiple inheritance
int solve_multiple_inheritance(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_multiple_inheritance(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Multiple inheritance
int solve_multiple_inheritance(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Multiple inheritance
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Multiple inheritance — model real-world entities with state + behavior.
