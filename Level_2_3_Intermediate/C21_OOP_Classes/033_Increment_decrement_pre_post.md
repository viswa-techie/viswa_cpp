# Chapter: Increment decrement pre post
...
## 1. Problem Statement
Master the topic: **Increment decrement pre post**.
...
This problem exercises **Increment decrement pre post** — a core concept in OOP — Classes & Objects.
...
Think of **Increment decrement pre post** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Increment decrement pre post
int solve_increment_decrement_pre_post(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_increment_decrement_pre_post(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Increment decrement pre post
int solve_increment_decrement_pre_post(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Increment decrement pre post
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Increment decrement pre post — model real-world entities with state + behavior.
