...
## 1. Problem Statement
Master the topic: **Virtual inheritance**.
...
This problem exercises **Virtual inheritance** — a core concept in OOP — Classes & Objects.
...
Think of **Virtual inheritance** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Virtual inheritance
int solve_virtual_inheritance(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_virtual_inheritance(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Virtual inheritance
int solve_virtual_inheritance(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Virtual inheritance
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Virtual inheritance — model real-world entities with state + behavior.
