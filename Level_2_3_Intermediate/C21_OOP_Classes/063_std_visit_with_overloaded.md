# Chapter: std visit with overloaded
...
## 1. Problem Statement
Master the topic: **std visit with overloaded**.
...
This problem exercises **std visit with overloaded** — a core concept in OOP — Classes & Objects.
...
Think of **std visit with overloaded** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: std visit with overloaded
int solve_std_visit_with_overloaded(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_visit_with_overloaded(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std visit with overloaded
int solve_std_visit_with_overloaded(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std visit with overloaded
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std visit with overloaded — model real-world entities with state + behavior.
