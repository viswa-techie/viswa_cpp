...
## 1. Problem Statement
Master the topic: **final keyword**.
...
This problem exercises **final keyword** — a core concept in OOP — Classes & Objects.
...
Think of **final keyword** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: final keyword
int solve_final_keyword(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_final_keyword(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: final keyword
int solve_final_keyword(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: final keyword
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** final keyword — model real-world entities with state + behavior.
