...
## 1. Problem Statement
Master the topic: **Anonymous classes**.
...
This problem exercises **Anonymous classes** — a core concept in OOP — Classes & Objects.
...
Think of **Anonymous classes** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Anonymous classes
int solve_anonymous_classes(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_anonymous_classes(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Anonymous classes
int solve_anonymous_classes(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Anonymous classes
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Anonymous classes — model real-world entities with state + behavior.
