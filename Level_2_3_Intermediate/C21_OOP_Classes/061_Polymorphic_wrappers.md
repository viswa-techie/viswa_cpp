...
## 1. Problem Statement
Master the topic: **Polymorphic wrappers**.
...
This problem exercises **Polymorphic wrappers** — a core concept in OOP — Classes & Objects.
...
Think of **Polymorphic wrappers** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Polymorphic wrappers
int solve_polymorphic_wrappers(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_polymorphic_wrappers(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Polymorphic wrappers
int solve_polymorphic_wrappers(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Polymorphic wrappers
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Polymorphic wrappers — model real-world entities with state + behavior.
