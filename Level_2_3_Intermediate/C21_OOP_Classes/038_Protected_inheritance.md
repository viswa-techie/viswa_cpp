...
## 1. Problem Statement
Master the topic: **Protected inheritance**.
...
This problem exercises **Protected inheritance** — a core concept in OOP — Classes & Objects.
...
Think of **Protected inheritance** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Protected inheritance
int solve_protected_inheritance(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_protected_inheritance(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Protected inheritance
int solve_protected_inheritance(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Protected inheritance
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Protected inheritance — model real-world entities with state + behavior.
