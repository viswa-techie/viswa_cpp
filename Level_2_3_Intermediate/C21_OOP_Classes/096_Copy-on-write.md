...
## 1. Problem Statement
Master the topic: **Copy-on-write**.
...
This problem exercises **Copy-on-write** — a core concept in OOP — Classes & Objects.
...
Think of **Copy-on-write** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Copy-on-write
int solve_copy_on_write(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_copy_on_write(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Copy-on-write
int solve_copy_on_write(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Copy-on-write
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Copy-on-write — model real-world entities with state + behavior.
