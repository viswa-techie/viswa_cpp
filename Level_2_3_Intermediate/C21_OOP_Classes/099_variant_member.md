...
## 1. Problem Statement
Master the topic: **variant member**.
...
This problem exercises **variant member** — a core concept in OOP — Classes & Objects.
...
Think of **variant member** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: variant member
int solve_variant_member(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_variant_member(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: variant member
int solve_variant_member(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: variant member
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** variant member — model real-world entities with state + behavior.
