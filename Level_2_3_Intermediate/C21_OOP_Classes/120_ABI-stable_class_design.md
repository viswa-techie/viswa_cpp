# Chapter: ABI-stable class design
...
## 1. Problem Statement
Master the topic: **ABI-stable class design**.
...
This problem exercises **ABI-stable class design** — a core concept in OOP — Classes & Objects.
...
Think of **ABI-stable class design** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: ABI-stable class design
int solve_abi_stable_class_design(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_abi_stable_class_design(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: ABI-stable class design
int solve_abi_stable_class_design(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: ABI-stable class design
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** ABI-stable class design — model real-world entities with state + behavior.
