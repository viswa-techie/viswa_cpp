# Chapter: Empty base optimization
...
## 1. Problem Statement
Master the topic: **Empty base optimization**.
...
This problem exercises **Empty base optimization** — a core concept in OOP — Classes & Objects.
...
Think of **Empty base optimization** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Empty base optimization
int solve_empty_base_optimization(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_empty_base_optimization(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Empty base optimization
int solve_empty_base_optimization(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Empty base optimization
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Empty base optimization — model real-world entities with state + behavior.
