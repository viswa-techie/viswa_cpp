...
This problem exercises **this pointer** — a core concept in OOP — Classes & Objects.
...
Think of **this pointer** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: this pointer
int solve_this_pointer(std::vector<int> v) {
    int result = 0;
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: this pointer
int solve_this_pointer(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: this pointer
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** this pointer — model real-world entities with state + behavior.
