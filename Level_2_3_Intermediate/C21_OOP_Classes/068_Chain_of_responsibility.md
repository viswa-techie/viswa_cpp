...
## 1. Problem Statement
Master the topic: **Chain of responsibility**.
...
This problem exercises **Chain of responsibility** — a core concept in OOP — Classes & Objects.
...
Think of **Chain of responsibility** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Chain of responsibility
int solve_chain_of_responsibility(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_chain_of_responsibility(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Chain of responsibility
int solve_chain_of_responsibility(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Chain of responsibility
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Chain of responsibility — model real-world entities with state + behavior.
