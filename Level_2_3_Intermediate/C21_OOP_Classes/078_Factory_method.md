...
## 1. Problem Statement
Master the topic: **Factory method**.
...
This problem exercises **Factory method** — a core concept in OOP — Classes & Objects.
...
Think of **Factory method** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Factory method
int solve_factory_method(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_factory_method(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Factory method
int solve_factory_method(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Factory method
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Factory method — model real-world entities with state + behavior.
