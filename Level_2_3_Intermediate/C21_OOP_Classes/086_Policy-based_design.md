...
## 1. Problem Statement
Master the topic: **Policy-based design**.
...
This problem exercises **Policy-based design** — a core concept in OOP — Classes & Objects.
...
Think of **Policy-based design** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Policy-based design
int solve_policy_based_design(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_policy_based_design(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Policy-based design
int solve_policy_based_design(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Policy-based design
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Policy-based design — model real-world entities with state + behavior.
