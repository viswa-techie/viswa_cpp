# Chapter: Pimpl idiom in class design
...
## 1. Problem Statement
Master the topic: **Pimpl idiom in class design**.
...
This problem exercises **Pimpl idiom in class design** — a core concept in OOP — Classes & Objects.
...
Think of **Pimpl idiom in class design** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Pimpl idiom in class design
int solve_pimpl_idiom_in_class_design(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_pimpl_idiom_in_class_design(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Pimpl idiom in class design
int solve_pimpl_idiom_in_class_design(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Pimpl idiom in class design
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Pimpl idiom in class design — model real-world entities with state + behavior.
