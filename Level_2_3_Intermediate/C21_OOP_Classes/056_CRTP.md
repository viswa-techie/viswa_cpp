...
This problem exercises **CRTP** — a core concept in OOP — Classes & Objects.
...
Think of **CRTP** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: CRTP
int solve_crtp(std::vector<int> v) {
    int result = 0;
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: CRTP
int solve_crtp(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: CRTP
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** CRTP — model real-world entities with state + behavior.
