...
## 1. Problem Statement
Master the topic: **Private inheritance**.
...
This problem exercises **Private inheritance** — a core concept in OOP — Classes & Objects.
...
Think of **Private inheritance** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Private inheritance
int solve_private_inheritance(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_private_inheritance(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Private inheritance
int solve_private_inheritance(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Private inheritance
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Private inheritance — model real-world entities with state + behavior.
