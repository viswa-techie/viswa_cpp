...
## 1. Problem Statement
Master the topic: **Diamond problem**.
...
This problem exercises **Diamond problem** — a core concept in OOP — Classes & Objects.
...
Think of **Diamond problem** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Diamond problem
int solve_diamond_problem(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_diamond_problem(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Diamond problem
int solve_diamond_problem(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Diamond problem
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Diamond problem — model real-world entities with state + behavior.
