...
## 1. Problem Statement
Master the topic: **Binary serialization**.
...
This problem exercises **Binary serialization** — a core concept in OOP — Classes & Objects.
...
Think of **Binary serialization** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Binary serialization
int solve_binary_serialization(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_binary_serialization(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Binary serialization
int solve_binary_serialization(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Binary serialization
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Binary serialization — model real-world entities with state + behavior.
