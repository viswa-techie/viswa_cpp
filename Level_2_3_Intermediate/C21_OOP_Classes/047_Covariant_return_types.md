# Chapter: Covariant return types
...
## 1. Problem Statement
Master the topic: **Covariant return types**.
...
This problem exercises **Covariant return types** — a core concept in OOP — Classes & Objects.
...
Think of **Covariant return types** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Covariant return types
int solve_covariant_return_types(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_covariant_return_types(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Covariant return types
int solve_covariant_return_types(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Covariant return types
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Covariant return types — model real-world entities with state + behavior.
