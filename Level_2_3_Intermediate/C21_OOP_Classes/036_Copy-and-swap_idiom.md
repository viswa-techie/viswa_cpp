...
## 1. Problem Statement
Master the topic: **Copy-and-swap idiom**.
...
This problem exercises **Copy-and-swap idiom** — a core concept in OOP — Classes & Objects.
...
Think of **Copy-and-swap idiom** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Copy-and-swap idiom
int solve_copy_and_swap_idiom(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_copy_and_swap_idiom(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Copy-and-swap idiom
int solve_copy_and_swap_idiom(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Copy-and-swap idiom
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Copy-and-swap idiom — model real-world entities with state + behavior.
