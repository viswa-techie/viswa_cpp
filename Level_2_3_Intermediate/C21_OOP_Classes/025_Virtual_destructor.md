...
## 1. Problem Statement
Master the topic: **Virtual destructor**.
...
Required when deleting derived via base pointer to avoid leaks/UB.
...
Think of **Virtual destructor** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Virtual destructor
int main() {
...
- **Core idea:** Virtual destructor — model real-world entities with state + behavior.
