...
## 1. Problem Statement
Master the topic: **Getters and setters**.
...
This problem exercises **Getters and setters** — a core concept in OOP — Classes & Objects.
...
Think of **Getters and setters** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Getters and setters
int solve_getters_and_setters(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_getters_and_setters(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Getters and setters
int solve_getters_and_setters(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Getters and setters
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Getters and setters — model real-world entities with state + behavior.
