# Chapter: JSON serialization pattern
...
## 1. Problem Statement
Master the topic: **JSON serialization pattern**.
...
This problem exercises **JSON serialization pattern** — a core concept in OOP — Classes & Objects.
...
Think of **JSON serialization pattern** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: JSON serialization pattern
int solve_json_serialization_pattern(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_json_serialization_pattern(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: JSON serialization pattern
int solve_json_serialization_pattern(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: JSON serialization pattern
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** JSON serialization pattern — model real-world entities with state + behavior.
