...
## 1. Problem Statement
Master the topic: **Handle-Body idiom**.
...
This problem exercises **Handle-Body idiom** — a core concept in OOP — Classes & Objects.
...
Think of **Handle-Body idiom** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Handle-Body idiom
int solve_handle_body_idiom(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_handle_body_idiom(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Handle-Body idiom
int solve_handle_body_idiom(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Handle-Body idiom
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Handle-Body idiom — model real-world entities with state + behavior.
