...
## 1. Problem Statement
Master the topic: **no_unique_address**.
...
This problem exercises **no_unique_address** — a core concept in OOP — Classes & Objects.
...
Think of **no_unique_address** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: no_unique_address
int solve_no_unique_address(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_no_unique_address(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: no_unique_address
int solve_no_unique_address(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: no_unique_address
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** no_unique_address — model real-world entities with state + behavior.
