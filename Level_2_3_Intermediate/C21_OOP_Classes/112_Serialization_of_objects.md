# Chapter: Serialization of objects
...
## 1. Problem Statement
Master the topic: **Serialization of objects**.
...
This problem exercises **Serialization of objects** — a core concept in OOP — Classes & Objects.
...
Think of **Serialization of objects** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Serialization of objects
int solve_serialization_of_objects(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_serialization_of_objects(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Serialization of objects
int solve_serialization_of_objects(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Serialization of objects
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Serialization of objects — model real-world entities with state + behavior.
