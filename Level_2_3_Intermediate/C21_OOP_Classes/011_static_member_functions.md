# Chapter: static member functions
...
## 1. Problem Statement
Master the topic: **static member functions**.
...
This problem exercises **static member functions** — a core concept in OOP — Classes & Objects.
...
Think of **static member functions** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: static member functions
int solve_static_member_functions(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_static_member_functions(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: static member functions
int solve_static_member_functions(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: static member functions
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** static member functions — model real-world entities with state + behavior.
