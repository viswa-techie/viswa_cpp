...
## 1. Problem Statement
Master the topic: **explicit keyword**.
...
This problem exercises **explicit keyword** — a core concept in OOP — Classes & Objects.
...
Think of **explicit keyword** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: explicit keyword
int solve_explicit_keyword(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_explicit_keyword(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: explicit keyword
int solve_explicit_keyword(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: explicit keyword
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** explicit keyword — model real-world entities with state + behavior.
