# Chapter: if constexpr in methods
...
## 1. Problem Statement
Master the topic: **if constexpr in methods**.
...
This problem exercises **if constexpr in methods** — a core concept in OOP — Classes & Objects.
...
Think of **if constexpr in methods** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: if constexpr in methods
int solve_if_constexpr_in_methods(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_if_constexpr_in_methods(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: if constexpr in methods
int solve_if_constexpr_in_methods(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: if constexpr in methods
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** if constexpr in methods — model real-world entities with state + behavior.
