# Chapter: Operator overloading arithmetic
...
## 1. Problem Statement
Master the topic: **Operator overloading arithmetic**.
...
This problem exercises **Operator overloading arithmetic** — a core concept in OOP — Classes & Objects.
...
Think of **Operator overloading arithmetic** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Operator overloading arithmetic
int solve_operator_overloading_arithmetic(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_operator_overloading_arithmetic(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Operator overloading arithmetic
int solve_operator_overloading_arithmetic(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Operator overloading arithmetic
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Operator overloading arithmetic — model real-world entities with state + behavior.
