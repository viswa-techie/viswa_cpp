...
## 1. Problem Statement
Master the topic: **RAII wrapper class**.
...
This problem exercises **RAII wrapper class** — a core concept in OOP — Classes & Objects.
...
Think of **RAII wrapper class** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: RAII wrapper class
int solve_raii_wrapper_class(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_raii_wrapper_class(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: RAII wrapper class
int solve_raii_wrapper_class(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: RAII wrapper class
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** RAII wrapper class — model real-world entities with state + behavior.
