# Chapter: Dynamic dispatch vtable
...
## 1. Problem Statement
Master the topic: **Dynamic dispatch vtable**.
...
This problem exercises **Dynamic dispatch vtable** — a core concept in OOP — Classes & Objects.
...
Think of **Dynamic dispatch vtable** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Dynamic dispatch vtable
int solve_dynamic_dispatch_vtable(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_dynamic_dispatch_vtable(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Dynamic dispatch vtable
int solve_dynamic_dispatch_vtable(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Dynamic dispatch vtable
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Dynamic dispatch vtable — model real-world entities with state + behavior.
