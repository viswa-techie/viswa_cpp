...
## 1. Problem Statement
Master the topic: **std variant visitor**.
...
This problem exercises **std variant visitor** — a core concept in OOP — Classes & Objects.
...
Think of **std variant visitor** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: std variant visitor
int solve_std_variant_visitor(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_variant_visitor(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std variant visitor
int solve_std_variant_visitor(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std variant visitor
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std variant visitor — model real-world entities with state + behavior.
