# Chapter: static_assert in class
...
## 1. Problem Statement
Master the topic: **static_assert in class**.
...
This problem exercises **static_assert in class** — a core concept in OOP — Classes & Objects.
...
Think of **static_assert in class** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: static_assert in class
int solve_static_assert_in_class(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_static_assert_in_class(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: static_assert in class
int solve_static_assert_in_class(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: static_assert in class
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** static_assert in class — model real-world entities with state + behavior.
