...
## 1. Problem Statement
Master the topic: **Method overriding**.
...
This problem exercises **Method overriding** — a core concept in OOP — Classes & Objects.
...
Think of **Method overriding** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Method overriding
int solve_method_overriding(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_method_overriding(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Method overriding
int solve_method_overriding(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Method overriding
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Method overriding — model real-world entities with state + behavior.
