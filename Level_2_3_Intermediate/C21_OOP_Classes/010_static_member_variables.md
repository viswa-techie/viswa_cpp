# Chapter: static member variables
...
## 1. Problem Statement
Master the topic: **static member variables**.
...
This problem exercises **static member variables** — a core concept in OOP — Classes & Objects.
...
Think of **static member variables** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: static member variables
int solve_static_member_variables(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_static_member_variables(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: static member variables
int solve_static_member_variables(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: static member variables
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** static member variables — model real-world entities with state + behavior.
