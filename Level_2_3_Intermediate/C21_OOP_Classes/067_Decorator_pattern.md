...
## 1. Problem Statement
Master the topic: **Decorator pattern**.
...
This problem exercises **Decorator pattern** — a core concept in OOP — Classes & Objects.
...
Think of **Decorator pattern** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Decorator pattern
int solve_decorator_pattern(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_decorator_pattern(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Decorator pattern
int solve_decorator_pattern(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Decorator pattern
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Decorator pattern — model real-world entities with state + behavior.
