...
## 1. Problem Statement
Master the topic: **Class with concepts**.
...
This problem exercises **Class with concepts** — a core concept in OOP — Classes & Objects.
...
Think of **Class with concepts** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Class with concepts
int solve_class_with_concepts(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_class_with_concepts(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Class with concepts
int solve_class_with_concepts(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Class with concepts
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Class with concepts — model real-world entities with state + behavior.
