...
## 1. Problem Statement
Master the topic: **mutable keyword**.
...
This problem exercises **mutable keyword** — a core concept in OOP — Classes & Objects.
...
Think of **mutable keyword** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: mutable keyword
int solve_mutable_keyword(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_mutable_keyword(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: mutable keyword
int solve_mutable_keyword(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: mutable keyword
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** mutable keyword — model real-world entities with state + behavior.
