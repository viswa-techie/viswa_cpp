# Chapter: State machine with classes
...
## 1. Problem Statement
Master the topic: **State machine with classes**.
...
This problem exercises **State machine with classes** — a core concept in OOP — Classes & Objects.
...
Think of **State machine with classes** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: State machine with classes
int solve_state_machine_with_classes(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_state_machine_with_classes(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: State machine with classes
int solve_state_machine_with_classes(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: State machine with classes
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** State machine with classes — model real-world entities with state + behavior.
