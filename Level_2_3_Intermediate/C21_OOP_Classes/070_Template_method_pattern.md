...
## 1. Problem Statement
Master the topic: **Template method pattern**.
...
This problem exercises **Template method pattern** — a core concept in OOP — Classes & Objects.
...
Think of **Template method pattern** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Template method pattern
int solve_template_method_pattern(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_template_method_pattern(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Template method pattern
int solve_template_method_pattern(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Template method pattern
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Template method pattern — model real-world entities with state + behavior.
