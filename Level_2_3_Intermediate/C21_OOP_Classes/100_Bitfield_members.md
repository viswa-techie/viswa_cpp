...
## 1. Problem Statement
Master the topic: **Bitfield members**.
...
This problem exercises **Bitfield members** — a core concept in OOP — Classes & Objects.
...
Think of **Bitfield members** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Bitfield members
int solve_bitfield_members(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_bitfield_members(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Bitfield members
int solve_bitfield_members(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Bitfield members
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Bitfield members — model real-world entities with state + behavior.
