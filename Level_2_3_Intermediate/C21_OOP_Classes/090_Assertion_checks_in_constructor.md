# Chapter: Assertion checks in constructor
...
## 1. Problem Statement
Master the topic: **Assertion checks in constructor**.
...
This problem exercises **Assertion checks in constructor** — a core concept in OOP — Classes & Objects.
...
Think of **Assertion checks in constructor** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Assertion checks in constructor
int solve_assertion_checks_in_constructor(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_assertion_checks_in_constructor(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Assertion checks in constructor
int solve_assertion_checks_in_constructor(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Assertion checks in constructor
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Assertion checks in constructor — model real-world entities with state + behavior.
