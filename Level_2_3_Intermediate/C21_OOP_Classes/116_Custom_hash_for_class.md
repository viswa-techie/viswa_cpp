...
## 1. Problem Statement
Master the topic: **Custom hash for class**.
...
This problem exercises **Custom hash for class** — a core concept in OOP — Classes & Objects.
...
Think of **Custom hash for class** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Custom hash for class
int solve_custom_hash_for_class(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_custom_hash_for_class(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Custom hash for class
int solve_custom_hash_for_class(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Custom hash for class
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Custom hash for class — model real-world entities with state + behavior.
