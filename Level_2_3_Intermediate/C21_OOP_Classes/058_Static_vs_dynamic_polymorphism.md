# Chapter: Static vs dynamic polymorphism
...
## 1. Problem Statement
Master the topic: **Static vs dynamic polymorphism**.
...
This problem exercises **Static vs dynamic polymorphism** — a core concept in OOP — Classes & Objects.
...
Think of **Static vs dynamic polymorphism** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Static vs dynamic polymorphism
int solve_static_vs_dynamic_polymorphism(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_static_vs_dynamic_polymorphism(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Static vs dynamic polymorphism
int solve_static_vs_dynamic_polymorphism(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Static vs dynamic polymorphism
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Static vs dynamic polymorphism — model real-world entities with state + behavior.
