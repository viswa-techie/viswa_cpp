...
## 1. Problem Statement
Master the topic: **Singleton thread-safe**.
...
This problem exercises **Singleton thread-safe** — a core concept in OOP — Classes & Objects.
...
Think of **Singleton thread-safe** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Singleton thread-safe
int solve_singleton_thread_safe(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_singleton_thread_safe(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Singleton thread-safe
int solve_singleton_thread_safe(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Singleton thread-safe
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Singleton thread-safe — model real-world entities with state + behavior.
