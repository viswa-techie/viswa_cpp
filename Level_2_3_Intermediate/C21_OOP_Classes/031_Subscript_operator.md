...
## 1. Problem Statement
Master the topic: **Subscript operator**.
...
This problem exercises **Subscript operator** — a core concept in OOP — Classes & Objects.
...
Think of **Subscript operator** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Subscript operator
int solve_subscript_operator(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_subscript_operator(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Subscript operator
int solve_subscript_operator(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Subscript operator
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Subscript operator — model real-world entities with state + behavior.
