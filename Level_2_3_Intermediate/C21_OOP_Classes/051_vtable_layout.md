...
## 1. Problem Statement
Master the topic: **vtable layout**.
...
This problem exercises **vtable layout** — a core concept in OOP — Classes & Objects.
...
Think of **vtable layout** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: vtable layout
int solve_vtable_layout(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_vtable_layout(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: vtable layout
int solve_vtable_layout(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: vtable layout
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** vtable layout — model real-world entities with state + behavior.
