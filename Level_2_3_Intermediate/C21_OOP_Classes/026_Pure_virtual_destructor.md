# Chapter: Pure virtual destructor
...
## 1. Problem Statement
Master the topic: **Pure virtual destructor**.
...
This problem exercises **Pure virtual destructor** — a core concept in OOP — Classes & Objects.
...
Think of **Pure virtual destructor** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Pure virtual destructor
int solve_pure_virtual_destructor(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_pure_virtual_destructor(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Pure virtual destructor
int solve_pure_virtual_destructor(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Pure virtual destructor
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Pure virtual destructor — model real-world entities with state + behavior.
