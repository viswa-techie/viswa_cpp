# Chapter: Exception safety levels
...
## 1. Problem Statement
Master the topic: **Exception safety levels**.
...
This problem exercises **Exception safety levels** — a core concept in OOP — Classes & Objects.
...
Think of **Exception safety levels** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Exception safety levels
int solve_exception_safety_levels(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_exception_safety_levels(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Exception safety levels
int solve_exception_safety_levels(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Exception safety levels
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Exception safety levels — model real-world entities with state + behavior.
