...
## 1. Problem Statement
Master the topic: **vptr overhead**.
...
This problem exercises **vptr overhead** — a core concept in OOP — Classes & Objects.
...
Think of **vptr overhead** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: vptr overhead
int solve_vptr_overhead(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_vptr_overhead(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: vptr overhead
int solve_vptr_overhead(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: vptr overhead
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** vptr overhead — model real-world entities with state + behavior.
