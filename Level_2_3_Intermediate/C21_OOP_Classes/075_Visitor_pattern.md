...
## 1. Problem Statement
Master the topic: **Visitor pattern**.
...
This problem exercises **Visitor pattern** — a core concept in OOP — Classes & Objects.
...
Think of **Visitor pattern** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Visitor pattern
int solve_visitor_pattern(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_visitor_pattern(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Visitor pattern
int solve_visitor_pattern(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Visitor pattern
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Visitor pattern — model real-world entities with state + behavior.
