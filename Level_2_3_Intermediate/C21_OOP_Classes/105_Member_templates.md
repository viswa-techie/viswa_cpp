...
## 1. Problem Statement
Master the topic: **Member templates**.
...
This problem exercises **Member templates** — a core concept in OOP — Classes & Objects.
...
Think of **Member templates** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Member templates
int solve_member_templates(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_member_templates(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Member templates
int solve_member_templates(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Member templates
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Member templates — model real-world entities with state + behavior.
