# Chapter: Iterator pattern custom
...
## 1. Problem Statement
Master the topic: **Iterator pattern custom**.
...
This problem exercises **Iterator pattern custom** — a core concept in OOP — Classes & Objects.
...
Think of **Iterator pattern custom** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Iterator pattern custom
int solve_iterator_pattern_custom(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_iterator_pattern_custom(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Iterator pattern custom
int solve_iterator_pattern_custom(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Iterator pattern custom
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Iterator pattern custom — model real-world entities with state + behavior.
