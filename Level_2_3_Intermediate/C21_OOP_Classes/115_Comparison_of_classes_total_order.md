# Chapter: Comparison of classes total order
...
## 1. Problem Statement
Master the topic: **Comparison of classes total order**.
...
This problem exercises **Comparison of classes total order** — a core concept in OOP — Classes & Objects.
...
Think of **Comparison of classes total order** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Comparison of classes total order
int solve_comparison_of_classes_total_order(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_comparison_of_classes_total_order(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Comparison of classes total order
int solve_comparison_of_classes_total_order(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Comparison of classes total order
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Comparison of classes total order — model real-world entities with state + behavior.
