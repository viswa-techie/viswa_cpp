# Chapter: Base class pointer to derived
...
## 1. Problem Statement
Master the topic: **Base class pointer to derived**.
...
This problem exercises **Base class pointer to derived** — a core concept in OOP — Classes & Objects.
...
Think of **Base class pointer to derived** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Base class pointer to derived
int solve_base_class_pointer_to_derived(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_base_class_pointer_to_derived(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Base class pointer to derived
int solve_base_class_pointer_to_derived(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Base class pointer to derived
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Base class pointer to derived — model real-world entities with state + behavior.
