...
## 1. Problem Statement
Master the topic: **Constructor init list**.
...
This problem exercises **Constructor init list** — a core concept in OOP — Classes & Objects.
...
Think of **Constructor init list** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Constructor init list
int solve_constructor_init_list(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_constructor_init_list(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Constructor init list
int solve_constructor_init_list(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Constructor init list
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Constructor init list — model real-world entities with state + behavior.
