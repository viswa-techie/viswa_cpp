...
## 1. Problem Statement
Master the topic: **Mixin via CRTP**.
...
This problem exercises **Mixin via CRTP** — a core concept in OOP — Classes & Objects.
...
Think of **Mixin via CRTP** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Mixin via CRTP
int solve_mixin_via_crtp(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_mixin_via_crtp(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Mixin via CRTP
int solve_mixin_via_crtp(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Mixin via CRTP
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Mixin via CRTP — model real-world entities with state + behavior.
