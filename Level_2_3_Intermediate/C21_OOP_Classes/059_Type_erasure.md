...
This problem exercises **Type erasure** — a core concept in OOP — Classes & Objects.
...
Think of **Type erasure** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Type erasure
int solve_type_erasure(std::vector<int> v) {
    int result = 0;
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Type erasure
int solve_type_erasure(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Type erasure
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Type erasure — model real-world entities with state + behavior.
