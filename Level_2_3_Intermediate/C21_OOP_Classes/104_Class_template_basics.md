...
## 1. Problem Statement
Master the topic: **Class template basics**.
...
This problem exercises **Class template basics** — a core concept in OOP — Classes & Objects.
...
Think of **Class template basics** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Class template basics
int solve_class_template_basics(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_class_template_basics(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Class template basics
int solve_class_template_basics(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Class template basics
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Class template basics — model real-world entities with state + behavior.
