...
## 1. Problem Statement
Master the topic: **Class in std set**.
...
This problem exercises **Class in std set** — a core concept in OOP — Classes & Objects.
...
Think of **Class in std set** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Class in std set
int solve_class_in_std_set(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_class_in_std_set(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Class in std set
int solve_class_in_std_set(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Class in std set
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Class in std set — model real-world entities with state + behavior.
