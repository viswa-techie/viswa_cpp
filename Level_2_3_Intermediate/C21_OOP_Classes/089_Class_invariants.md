...
## 1. Problem Statement
Master the topic: **Class invariants**.
...
This problem exercises **Class invariants** — a core concept in OOP — Classes & Objects.
...
Think of **Class invariants** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Class invariants
int solve_class_invariants(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_class_invariants(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Class invariants
int solve_class_invariants(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Class invariants
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Class invariants — model real-world entities with state + behavior.
