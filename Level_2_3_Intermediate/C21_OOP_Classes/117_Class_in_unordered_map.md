# Chapter: Class in unordered_map
...
## 1. Problem Statement
Master the topic: **Class in unordered_map**.
...
This problem exercises **Class in unordered_map** — a core concept in OOP — Classes & Objects.
...
Think of **Class in unordered_map** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Class in unordered_map
int solve_class_in_unordered_map(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_class_in_unordered_map(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Class in unordered_map
int solve_class_in_unordered_map(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Class in unordered_map
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Class in unordered_map — model real-world entities with state + behavior.
