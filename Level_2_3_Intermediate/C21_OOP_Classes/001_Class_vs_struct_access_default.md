# Chapter: Class vs struct access default
...
## 1. Problem Statement
Master the topic: **Class vs struct access default**.
...
This problem exercises **Class vs struct access default** — a core concept in OOP — Classes & Objects.
...
Think of **Class vs struct access default** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Class vs struct access default
int solve_class_vs_struct_access_default(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_class_vs_struct_access_default(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Class vs struct access default
int solve_class_vs_struct_access_default(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Class vs struct access default
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Class vs struct access default — model real-world entities with state + behavior.
