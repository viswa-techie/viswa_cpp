# Chapter: Assignment operator chaining
...
## 1. Problem Statement
Master the topic: **Assignment operator chaining**.
...
This problem exercises **Assignment operator chaining** — a core concept in OOP — Classes & Objects.
...
Think of **Assignment operator chaining** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Assignment operator chaining
int solve_assignment_operator_chaining(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_assignment_operator_chaining(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Assignment operator chaining
int solve_assignment_operator_chaining(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Assignment operator chaining
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Assignment operator chaining — model real-world entities with state + behavior.
