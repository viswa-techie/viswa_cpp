...
## 1. Problem Statement
Master the topic: **noexcept specifier**.
...
This problem exercises **noexcept specifier** — a core concept in OOP — Classes & Objects.
...
Think of **noexcept specifier** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: noexcept specifier
int solve_noexcept_specifier(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_noexcept_specifier(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: noexcept specifier
int solve_noexcept_specifier(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: noexcept specifier
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** noexcept specifier — model real-world entities with state + behavior.
