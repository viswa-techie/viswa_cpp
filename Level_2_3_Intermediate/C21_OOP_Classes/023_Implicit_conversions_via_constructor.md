# Chapter: Implicit conversions via constructor
...
## 1. Problem Statement
Master the topic: **Implicit conversions via constructor**.
...
This problem exercises **Implicit conversions via constructor** — a core concept in OOP — Classes & Objects.
...
Think of **Implicit conversions via constructor** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Implicit conversions via constructor
int solve_implicit_conversions_via_constructor(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_implicit_conversions_via_constructor(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Implicit conversions via constructor
int solve_implicit_conversions_via_constructor(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Implicit conversions via constructor
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Implicit conversions via constructor — model real-world entities with state + behavior.
