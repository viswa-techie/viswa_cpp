...
This problem exercises **Tag dispatch** — a core concept in OOP — Classes & Objects.
...
Think of **Tag dispatch** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Tag dispatch
int solve_tag_dispatch(std::vector<int> v) {
    int result = 0;
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Tag dispatch
int solve_tag_dispatch(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Tag dispatch
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Tag dispatch — model real-world entities with state + behavior.
