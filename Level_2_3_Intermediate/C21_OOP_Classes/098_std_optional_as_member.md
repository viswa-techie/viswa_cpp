# Chapter: std optional as member
...
## 1. Problem Statement
Master the topic: **std optional as member**.
...
This problem exercises **std optional as member** — a core concept in OOP — Classes & Objects.
...
Think of **std optional as member** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: std optional as member
int solve_std_optional_as_member(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_optional_as_member(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std optional as member
int solve_std_optional_as_member(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std optional as member
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std optional as member — model real-world entities with state + behavior.
