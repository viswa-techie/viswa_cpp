...
## 1. Problem Statement
Master the topic: **Friend functions**.
...
This problem exercises **Friend functions** — a core concept in OOP — Classes & Objects.
...
Think of **Friend functions** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Friend functions
int solve_friend_functions(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_friend_functions(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Friend functions
int solve_friend_functions(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Friend functions
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Friend functions — model real-world entities with state + behavior.
