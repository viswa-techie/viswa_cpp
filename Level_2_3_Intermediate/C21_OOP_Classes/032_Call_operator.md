...
## 1. Problem Statement
Master the topic: **Call operator**.
...
This problem exercises **Call operator** — a core concept in OOP — Classes & Objects.
...
Think of **Call operator** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Call operator
int solve_call_operator(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_call_operator(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Call operator
int solve_call_operator(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Call operator
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Call operator — model real-world entities with state + behavior.
