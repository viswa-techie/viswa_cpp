# Chapter: std any for type erasure
...
## 1. Problem Statement
Master the topic: **std any for type erasure**.
...
This problem exercises **std any for type erasure** — a core concept in OOP — Classes & Objects.
...
Think of **std any for type erasure** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: std any for type erasure
int solve_std_any_for_type_erasure(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_any_for_type_erasure(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std any for type erasure
int solve_std_any_for_type_erasure(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std any for type erasure
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std any for type erasure — model real-world entities with state + behavior.
