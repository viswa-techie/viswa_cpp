...
## 1. Problem Statement
Master the topic: **Stream operators**.
...
This problem exercises **Stream operators** — a core concept in OOP — Classes & Objects.
...
Think of **Stream operators** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Stream operators
int solve_stream_operators(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_stream_operators(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Stream operators
int solve_stream_operators(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Stream operators
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Stream operators — model real-world entities with state + behavior.
