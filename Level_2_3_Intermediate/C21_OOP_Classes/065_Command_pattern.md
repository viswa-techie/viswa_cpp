...
## 1. Problem Statement
Master the topic: **Command pattern**.
...
This problem exercises **Command pattern** — a core concept in OOP — Classes & Objects.
...
Think of **Command pattern** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Command pattern
int solve_command_pattern(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_command_pattern(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Command pattern
int solve_command_pattern(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Command pattern
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Command pattern — model real-world entities with state + behavior.
