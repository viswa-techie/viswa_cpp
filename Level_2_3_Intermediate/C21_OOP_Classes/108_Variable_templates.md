...
## 1. Problem Statement
Master the topic: **Variable templates**.
...
This problem exercises **Variable templates** — a core concept in OOP — Classes & Objects.
...
Think of **Variable templates** as a blueprint (class) producing instances (objects) sharing behavior via a vtable.
...
#include <vector>
#include <string>
// Brute-force template for: Variable templates
int solve_variable_templates(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_variable_templates(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Variable templates
int solve_variable_templates(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Variable templates
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Variable templates — model real-world entities with state + behavior.
