# Chapter: Undefined behavior catalog
...
## 1. Problem Statement
Master the topic: **Undefined behavior catalog**.
...
This problem exercises **Undefined behavior catalog** — a core concept in Pointers & Memory Management.
...
Think of **Undefined behavior catalog** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Undefined behavior catalog
int solve_undefined_behavior_catalog(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_undefined_behavior_catalog(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Undefined behavior catalog
int solve_undefined_behavior_catalog(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Undefined behavior catalog
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Undefined behavior catalog — manage who owns the memory.
