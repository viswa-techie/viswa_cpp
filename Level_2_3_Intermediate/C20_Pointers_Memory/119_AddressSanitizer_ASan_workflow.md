# Chapter: AddressSanitizer ASan workflow
...
## 1. Problem Statement
Master the topic: **AddressSanitizer ASan workflow**.
...
This problem exercises **AddressSanitizer ASan workflow** — a core concept in Pointers & Memory Management.
...
Think of **AddressSanitizer ASan workflow** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: AddressSanitizer ASan workflow
int solve_addresssanitizer_asan_workflow(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_addresssanitizer_asan_workflow(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: AddressSanitizer ASan workflow
int solve_addresssanitizer_asan_workflow(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: AddressSanitizer ASan workflow
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** AddressSanitizer ASan workflow — manage who owns the memory.
