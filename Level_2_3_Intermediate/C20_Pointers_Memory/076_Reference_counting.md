...
## 1. Problem Statement
Master the topic: **Reference counting**.
...
This problem exercises **Reference counting** — a core concept in Pointers & Memory Management.
...
Think of **Reference counting** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Reference counting
int solve_reference_counting(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_reference_counting(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Reference counting
int solve_reference_counting(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Reference counting
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Reference counting — manage who owns the memory.
