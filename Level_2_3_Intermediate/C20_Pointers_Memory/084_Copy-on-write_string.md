...
## 1. Problem Statement
Master the topic: **Copy-on-write string**.
...
This problem exercises **Copy-on-write string** — a core concept in Pointers & Memory Management.
...
Think of **Copy-on-write string** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Copy-on-write string
int solve_copy_on_write_string(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_copy_on_write_string(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Copy-on-write string
int solve_copy_on_write_string(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Copy-on-write string
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Copy-on-write string — manage who owns the memory.
