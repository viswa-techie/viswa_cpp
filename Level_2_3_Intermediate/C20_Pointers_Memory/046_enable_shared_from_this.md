# Chapter: enable_shared_from_this
...
## 1. Problem Statement
Master the topic: **enable_shared_from_this**.
...
This problem exercises **enable_shared_from_this** — a core concept in Pointers & Memory Management.
...
Think of **enable_shared_from_this** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: enable_shared_from_this
int solve_enable_shared_from_this(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_enable_shared_from_this(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: enable_shared_from_this
int solve_enable_shared_from_this(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: enable_shared_from_this
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** enable_shared_from_this — manage who owns the memory.
