# Chapter: Pointer to struct member
...
## 1. Problem Statement
Master the topic: **Pointer to struct member**.
...
This problem exercises **Pointer to struct member** — a core concept in Pointers & Memory Management.
...
Think of **Pointer to struct member** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Pointer to struct member
int solve_pointer_to_struct_member(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_pointer_to_struct_member(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Pointer to struct member
int solve_pointer_to_struct_member(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Pointer to struct member
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Pointer to struct member — manage who owns the memory.
