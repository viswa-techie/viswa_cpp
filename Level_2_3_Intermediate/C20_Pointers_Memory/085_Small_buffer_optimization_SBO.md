# Chapter: Small buffer optimization SBO
...
## 1. Problem Statement
Master the topic: **Small buffer optimization SBO**.
...
This problem exercises **Small buffer optimization SBO** — a core concept in Pointers & Memory Management.
...
Think of **Small buffer optimization SBO** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Small buffer optimization SBO
int solve_small_buffer_optimization_sbo(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_small_buffer_optimization_sbo(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Small buffer optimization SBO
int solve_small_buffer_optimization_sbo(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Small buffer optimization SBO
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Small buffer optimization SBO — manage who owns the memory.
