# Chapter: Dynamic allocation new delete
...
## 1. Problem Statement
Master the topic: **Dynamic allocation new delete**.
...
This problem exercises **Dynamic allocation new delete** — a core concept in Pointers & Memory Management.
...
Think of **Dynamic allocation new delete** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Dynamic allocation new delete
int solve_dynamic_allocation_new_delete(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_dynamic_allocation_new_delete(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Dynamic allocation new delete
int solve_dynamic_allocation_new_delete(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Dynamic allocation new delete
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Dynamic allocation new delete — manage who owns the memory.
