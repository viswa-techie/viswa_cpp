# Chapter: Bit manipulation via pointer
...
## 1. Problem Statement
Master the topic: **Bit manipulation via pointer**.
...
This problem exercises **Bit manipulation via pointer** — a core concept in Pointers & Memory Management.
...
Think of **Bit manipulation via pointer** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Bit manipulation via pointer
int solve_bit_manipulation_via_pointer(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_bit_manipulation_via_pointer(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Bit manipulation via pointer
int solve_bit_manipulation_via_pointer(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Bit manipulation via pointer
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Bit manipulation via pointer — manage who owns the memory.
