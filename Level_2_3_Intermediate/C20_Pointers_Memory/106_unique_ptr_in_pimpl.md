...
## 1. Problem Statement
Master the topic: **unique_ptr in pimpl**.
...
This problem exercises **unique_ptr in pimpl** — a core concept in Pointers & Memory Management.
...
Think of **unique_ptr in pimpl** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: unique_ptr in pimpl
int solve_unique_ptr_in_pimpl(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_unique_ptr_in_pimpl(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: unique_ptr in pimpl
int solve_unique_ptr_in_pimpl(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: unique_ptr in pimpl
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** unique_ptr in pimpl — manage who owns the memory.
