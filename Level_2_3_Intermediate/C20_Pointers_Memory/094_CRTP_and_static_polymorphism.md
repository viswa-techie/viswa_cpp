# Chapter: CRTP and static polymorphism
...
## 1. Problem Statement
Master the topic: **CRTP and static polymorphism**.
...
This problem exercises **CRTP and static polymorphism** — a core concept in Pointers & Memory Management.
...
Think of **CRTP and static polymorphism** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: CRTP and static polymorphism
int solve_crtp_and_static_polymorphism(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_crtp_and_static_polymorphism(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: CRTP and static polymorphism
int solve_crtp_and_static_polymorphism(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: CRTP and static polymorphism
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** CRTP and static polymorphism — manage who owns the memory.
