# Chapter: Implementing vector from scratch
...
## 1. Problem Statement
Master the topic: **Implementing vector from scratch**.
...
This problem exercises **Implementing vector from scratch** — a core concept in Pointers & Memory Management.
...
Think of **Implementing vector from scratch** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Implementing vector from scratch
int solve_implementing_vector_from_scratch(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_implementing_vector_from_scratch(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Implementing vector from scratch
int solve_implementing_vector_from_scratch(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Implementing vector from scratch
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Implementing vector from scratch — manage who owns the memory.
