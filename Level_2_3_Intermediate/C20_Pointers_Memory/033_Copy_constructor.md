...
## 1. Problem Statement
Master the topic: **Copy constructor**.
...
This problem exercises **Copy constructor** — a core concept in Pointers & Memory Management.
...
Think of **Copy constructor** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Copy constructor
int solve_copy_constructor(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_copy_constructor(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Copy constructor
int solve_copy_constructor(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Copy constructor
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Copy constructor — manage who owns the memory.
