# Chapter: Sequenced-before relation
...
## 1. Problem Statement
Master the topic: **Sequenced-before relation**.
...
This problem exercises **Sequenced-before relation** — a core concept in Pointers & Memory Management.
...
Think of **Sequenced-before relation** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Sequenced-before relation
int solve_sequenced_before_relation(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_sequenced_before_relation(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Sequenced-before relation
int solve_sequenced_before_relation(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Sequenced-before relation
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Sequenced-before relation — manage who owns the memory.
