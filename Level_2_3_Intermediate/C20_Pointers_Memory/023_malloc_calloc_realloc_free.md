# Chapter: malloc calloc realloc free
...
## 1. Problem Statement
Master the topic: **malloc calloc realloc free**.
...
This problem exercises **malloc calloc realloc free** — a core concept in Pointers & Memory Management.
...
Think of **malloc calloc realloc free** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: malloc calloc realloc free
int solve_malloc_calloc_realloc_free(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_malloc_calloc_realloc_free(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: malloc calloc realloc free
int solve_malloc_calloc_realloc_free(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: malloc calloc realloc free
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** malloc calloc realloc free — manage who owns the memory.
