...
This problem exercises **std mem_fn** — a core concept in Pointers & Memory Management.
...
Think of **std mem_fn** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: std mem_fn
int solve_std_mem_fn(std::vector<int> v) {
    int result = 0;
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std mem_fn
int solve_std_mem_fn(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std mem_fn
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std mem_fn — manage who owns the memory.
