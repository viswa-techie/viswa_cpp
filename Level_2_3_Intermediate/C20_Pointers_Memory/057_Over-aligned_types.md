...
## 1. Problem Statement
Master the topic: **Over-aligned types**.
...
This problem exercises **Over-aligned types** — a core concept in Pointers & Memory Management.
...
Think of **Over-aligned types** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Over-aligned types
int solve_over_aligned_types(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_over_aligned_types(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Over-aligned types
int solve_over_aligned_types(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Over-aligned types
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Over-aligned types — manage who owns the memory.
