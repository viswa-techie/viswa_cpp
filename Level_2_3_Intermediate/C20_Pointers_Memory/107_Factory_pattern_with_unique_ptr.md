# Chapter: Factory pattern with unique_ptr
...
## 1. Problem Statement
Master the topic: **Factory pattern with unique_ptr**.
...
This problem exercises **Factory pattern with unique_ptr** — a core concept in Pointers & Memory Management.
...
Think of **Factory pattern with unique_ptr** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Factory pattern with unique_ptr
int solve_factory_pattern_with_unique_ptr(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_factory_pattern_with_unique_ptr(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Factory pattern with unique_ptr
int solve_factory_pattern_with_unique_ptr(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Factory pattern with unique_ptr
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Factory pattern with unique_ptr — manage who owns the memory.
