# Chapter: Type punning via memcpy
...
## 1. Problem Statement
Master the topic: **Type punning via memcpy**.
...
This problem exercises **Type punning via memcpy** — a core concept in Pointers & Memory Management.
...
Think of **Type punning via memcpy** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Type punning via memcpy
int solve_type_punning_via_memcpy(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_type_punning_via_memcpy(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Type punning via memcpy
int solve_type_punning_via_memcpy(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Type punning via memcpy
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Type punning via memcpy — manage who owns the memory.
