# Chapter: Smart pointer internals
...
## 1. Problem Statement
Master the topic: **Smart pointer internals**.
...
This problem exercises **Smart pointer internals** — a core concept in Pointers & Memory Management.
...
Think of **Smart pointer internals** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Smart pointer internals
int solve_smart_pointer_internals(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_smart_pointer_internals(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Smart pointer internals
int solve_smart_pointer_internals(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Smart pointer internals
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Smart pointer internals — manage who owns the memory.
