# Chapter: Valgrind memcheck workflow
...
## 1. Problem Statement
Master the topic: **Valgrind memcheck workflow**.
...
This problem exercises **Valgrind memcheck workflow** — a core concept in Pointers & Memory Management.
...
Think of **Valgrind memcheck workflow** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Valgrind memcheck workflow
int solve_valgrind_memcheck_workflow(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_valgrind_memcheck_workflow(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Valgrind memcheck workflow
int solve_valgrind_memcheck_workflow(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Valgrind memcheck workflow
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Valgrind memcheck workflow — manage who owns the memory.
