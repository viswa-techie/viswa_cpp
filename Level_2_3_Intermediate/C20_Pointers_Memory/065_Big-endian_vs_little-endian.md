# Chapter: Big-endian vs little-endian
...
## 1. Problem Statement
Master the topic: **Big-endian vs little-endian**.
...
This problem exercises **Big-endian vs little-endian** — a core concept in Pointers & Memory Management.
...
Think of **Big-endian vs little-endian** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Big-endian vs little-endian
int solve_big_endian_vs_little_endian(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_big_endian_vs_little_endian(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Big-endian vs little-endian
int solve_big_endian_vs_little_endian(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Big-endian vs little-endian
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Big-endian vs little-endian — manage who owns the memory.
