...
## 1. Problem Statement
Master the topic: **Memory leak detection**.
...
This problem exercises **Memory leak detection** — a core concept in Pointers & Memory Management.
...
Think of **Memory leak detection** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Memory leak detection
int solve_memory_leak_detection(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_memory_leak_detection(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Memory leak detection
int solve_memory_leak_detection(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Memory leak detection
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Memory leak detection — manage who owns the memory.
