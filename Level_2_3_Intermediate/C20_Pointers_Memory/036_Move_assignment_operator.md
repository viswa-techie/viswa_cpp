# Chapter: Move assignment operator
...
## 1. Problem Statement
Master the topic: **Move assignment operator**.
...
This problem exercises **Move assignment operator** — a core concept in Pointers & Memory Management.
...
Think of **Move assignment operator** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Move assignment operator
int solve_move_assignment_operator(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_move_assignment_operator(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Move assignment operator
int solve_move_assignment_operator(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Move assignment operator
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Move assignment operator — manage who owns the memory.
