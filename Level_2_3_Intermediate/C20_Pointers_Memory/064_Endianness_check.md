...
## 1. Problem Statement
Master the topic: **Endianness check**.
...
This problem exercises **Endianness check** — a core concept in Pointers & Memory Management.
...
Think of **Endianness check** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Endianness check
int solve_endianness_check(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_endianness_check(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Endianness check
int solve_endianness_check(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Endianness check
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Endianness check — manage who owns the memory.
