...
## 1. Problem Statement
Master the topic: **Dangling pointer**.
...
This problem exercises **Dangling pointer** — a core concept in Pointers & Memory Management.
...
Think of **Dangling pointer** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Dangling pointer
int solve_dangling_pointer(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_dangling_pointer(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Dangling pointer
int solve_dangling_pointer(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Dangling pointer
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Dangling pointer — manage who owns the memory.
