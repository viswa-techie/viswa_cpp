...
This problem exercises **RAII pattern** — a core concept in Pointers & Memory Management.
...
Think of **RAII pattern** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: RAII pattern
int solve_raii_pattern(std::vector<int> v) {
    int result = 0;
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: RAII pattern
int solve_raii_pattern(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: RAII pattern
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** RAII pattern — manage who owns the memory.
