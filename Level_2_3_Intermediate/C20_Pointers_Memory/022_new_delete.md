...
This problem exercises **new[] delete[]** — a core concept in Pointers & Memory Management.
...
Think of **new[] delete[]** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: new[] delete[]
int solve_new___delete__(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_new___delete__(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: new[] delete[]
int solve_new___delete__(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: new[] delete[]
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** new[] delete[] — manage who owns the memory.
