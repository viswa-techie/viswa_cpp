# Chapter: Copy assignment operator
...
## 1. Problem Statement
Master the topic: **Copy assignment operator**.
...
This problem exercises **Copy assignment operator** — a core concept in Pointers & Memory Management.
...
Think of **Copy assignment operator** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Copy assignment operator
int solve_copy_assignment_operator(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_copy_assignment_operator(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Copy assignment operator
int solve_copy_assignment_operator(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Copy assignment operator
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Copy assignment operator — manage who owns the memory.
