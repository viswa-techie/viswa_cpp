...
## 1. Problem Statement
Master the topic: **ABI stability**.
...
This problem exercises **ABI stability** — a core concept in Pointers & Memory Management.
...
Think of **ABI stability** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: ABI stability
int solve_abi_stability(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_abi_stability(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: ABI stability
int solve_abi_stability(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: ABI stability
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** ABI stability — manage who owns the memory.
