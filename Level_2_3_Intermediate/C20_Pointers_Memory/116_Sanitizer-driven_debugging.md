# Chapter: Sanitizer-driven debugging
...
## 1. Problem Statement
Master the topic: **Sanitizer-driven debugging**.
...
This problem exercises **Sanitizer-driven debugging** — a core concept in Pointers & Memory Management.
...
Think of **Sanitizer-driven debugging** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Sanitizer-driven debugging
int solve_sanitizer_driven_debugging(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_sanitizer_driven_debugging(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Sanitizer-driven debugging
int solve_sanitizer_driven_debugging(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Sanitizer-driven debugging
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Sanitizer-driven debugging — manage who owns the memory.
