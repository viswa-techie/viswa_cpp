...
## 1. Problem Statement
Master the topic: **Array name as pointer**.
...
This problem exercises **Array name as pointer** — a core concept in Pointers & Memory Management.
...
Think of **Array name as pointer** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Array name as pointer
int solve_array_name_as_pointer(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_array_name_as_pointer(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Array name as pointer
int solve_array_name_as_pointer(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Array name as pointer
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Array name as pointer — manage who owns the memory.
