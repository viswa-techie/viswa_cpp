...
## 1. Problem Statement
Master the topic: **Arrow operator**.
...
This problem exercises **Arrow operator** — a core concept in Pointers & Memory Management.
...
Think of **Arrow operator** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Arrow operator
int solve_arrow_operator(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_arrow_operator(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Arrow operator
int solve_arrow_operator(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Arrow operator
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Arrow operator — manage who owns the memory.
