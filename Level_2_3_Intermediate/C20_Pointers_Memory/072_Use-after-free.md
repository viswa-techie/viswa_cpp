...
## 1. Problem Statement
Master the topic: **Use-after-free**.
...
This problem exercises **Use-after-free** — a core concept in Pointers & Memory Management.
...
Think of **Use-after-free** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Use-after-free
int solve_use_after_free(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_use_after_free(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Use-after-free
int solve_use_after_free(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Use-after-free
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Use-after-free — manage who owns the memory.
