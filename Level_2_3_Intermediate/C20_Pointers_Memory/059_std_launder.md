...
This problem exercises **std launder** — a core concept in Pointers & Memory Management.
...
Think of **std launder** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: std launder
int solve_std_launder(std::vector<int> v) {
    int result = 0;
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std launder
int solve_std_launder(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std launder
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std launder — manage who owns the memory.
