...
## 1. Problem Statement
Master the topic: **intrusive_ptr concept**.
...
This problem exercises **intrusive_ptr concept** — a core concept in Pointers & Memory Management.
...
Think of **intrusive_ptr concept** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: intrusive_ptr concept
int solve_intrusive_ptr_concept(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_intrusive_ptr_concept(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: intrusive_ptr concept
int solve_intrusive_ptr_concept(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: intrusive_ptr concept
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** intrusive_ptr concept — manage who owns the memory.
