...
## 1. Problem Statement
Master the topic: **Deleter with lambda**.
...
This problem exercises **Deleter with lambda** — a core concept in Pointers & Memory Management.
...
Think of **Deleter with lambda** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Deleter with lambda
int solve_deleter_with_lambda(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_deleter_with_lambda(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Deleter with lambda
int solve_deleter_with_lambda(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Deleter with lambda
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Deleter with lambda — manage who owns the memory.
