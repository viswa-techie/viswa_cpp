# Chapter: Singleton with static local
...
## 1. Problem Statement
Master the topic: **Singleton with static local**.
...
This problem exercises **Singleton with static local** — a core concept in Pointers & Memory Management.
...
Think of **Singleton with static local** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Singleton with static local
int solve_singleton_with_static_local(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_singleton_with_static_local(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Singleton with static local
int solve_singleton_with_static_local(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Singleton with static local
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Singleton with static local — manage who owns the memory.
