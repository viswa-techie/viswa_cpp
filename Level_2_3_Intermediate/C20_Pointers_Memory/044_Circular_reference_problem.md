# Chapter: Circular reference problem
...
## 1. Problem Statement
Master the topic: **Circular reference problem**.
...
This problem exercises **Circular reference problem** — a core concept in Pointers & Memory Management.
...
Think of **Circular reference problem** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Circular reference problem
int solve_circular_reference_problem(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_circular_reference_problem(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Circular reference problem
int solve_circular_reference_problem(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Circular reference problem
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Circular reference problem — manage who owns the memory.
