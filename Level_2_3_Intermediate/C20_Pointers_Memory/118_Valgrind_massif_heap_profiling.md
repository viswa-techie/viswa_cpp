# Chapter: Valgrind massif heap profiling
...
## 1. Problem Statement
Master the topic: **Valgrind massif heap profiling**.
...
This problem exercises **Valgrind massif heap profiling** — a core concept in Pointers & Memory Management.
...
Think of **Valgrind massif heap profiling** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Valgrind massif heap profiling
int solve_valgrind_massif_heap_profiling(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_valgrind_massif_heap_profiling(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Valgrind massif heap profiling
int solve_valgrind_massif_heap_profiling(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Valgrind massif heap profiling
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Valgrind massif heap profiling — manage who owns the memory.
