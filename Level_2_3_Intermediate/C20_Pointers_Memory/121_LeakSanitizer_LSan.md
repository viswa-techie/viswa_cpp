...
## 1. Problem Statement
Master the topic: **LeakSanitizer LSan**.
...
This problem exercises **LeakSanitizer LSan** — a core concept in Pointers & Memory Management.
...
Think of **LeakSanitizer LSan** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: LeakSanitizer LSan
int solve_leaksanitizer_lsan(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_leaksanitizer_lsan(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: LeakSanitizer LSan
int solve_leaksanitizer_lsan(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: LeakSanitizer LSan
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** LeakSanitizer LSan — manage who owns the memory.
