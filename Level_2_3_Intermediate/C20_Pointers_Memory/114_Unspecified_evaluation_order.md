# Chapter: Unspecified evaluation order
...
## 1. Problem Statement
Master the topic: **Unspecified evaluation order**.
...
This problem exercises **Unspecified evaluation order** — a core concept in Pointers & Memory Management.
...
Think of **Unspecified evaluation order** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Unspecified evaluation order
int solve_unspecified_evaluation_order(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_unspecified_evaluation_order(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Unspecified evaluation order
int solve_unspecified_evaluation_order(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Unspecified evaluation order
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Unspecified evaluation order — manage who owns the memory.
