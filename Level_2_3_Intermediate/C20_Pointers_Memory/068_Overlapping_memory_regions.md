# Chapter: Overlapping memory regions
...
## 1. Problem Statement
Master the topic: **Overlapping memory regions**.
...
This problem exercises **Overlapping memory regions** — a core concept in Pointers & Memory Management.
...
Think of **Overlapping memory regions** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Overlapping memory regions
int solve_overlapping_memory_regions(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_overlapping_memory_regions(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Overlapping memory regions
int solve_overlapping_memory_regions(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Overlapping memory regions
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Overlapping memory regions — manage who owns the memory.
