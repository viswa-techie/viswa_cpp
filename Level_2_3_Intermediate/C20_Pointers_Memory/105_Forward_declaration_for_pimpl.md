# Chapter: Forward declaration for pimpl
...
## 1. Problem Statement
Master the topic: **Forward declaration for pimpl**.
...
This problem exercises **Forward declaration for pimpl** — a core concept in Pointers & Memory Management.
...
Think of **Forward declaration for pimpl** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Forward declaration for pimpl
int solve_forward_declaration_for_pimpl(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_forward_declaration_for_pimpl(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Forward declaration for pimpl
int solve_forward_declaration_for_pimpl(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Forward declaration for pimpl
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Forward declaration for pimpl — manage who owns the memory.
