...
## 1. Problem Statement
Master the topic: **Pointer casting**.
...
This problem exercises **Pointer casting** — a core concept in Pointers & Memory Management.
...
Think of **Pointer casting** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Pointer casting
int solve_pointer_casting(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_pointer_casting(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Pointer casting
int solve_pointer_casting(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Pointer casting
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Pointer casting — manage who owns the memory.
