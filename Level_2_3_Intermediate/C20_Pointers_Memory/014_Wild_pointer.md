...
This problem exercises **Wild pointer** — a core concept in Pointers & Memory Management.
...
Think of **Wild pointer** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Wild pointer
int solve_wild_pointer(std::vector<int> v) {
    int result = 0;
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Wild pointer
int solve_wild_pointer(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Wild pointer
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Wild pointer — manage who owns the memory.
