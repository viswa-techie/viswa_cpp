...
## 1. Problem Statement
Master the topic: **Allocator traits**.
...
This problem exercises **Allocator traits** — a core concept in Pointers & Memory Management.
...
Think of **Allocator traits** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Allocator traits
int solve_allocator_traits(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_allocator_traits(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Allocator traits
int solve_allocator_traits(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Allocator traits
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Allocator traits — manage who owns the memory.
