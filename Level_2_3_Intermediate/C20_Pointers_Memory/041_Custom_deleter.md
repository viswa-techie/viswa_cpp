...
## 1. Problem Statement
Master the topic: **Custom deleter**.
...
This problem exercises **Custom deleter** — a core concept in Pointers & Memory Management.
...
Think of **Custom deleter** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Custom deleter
int solve_custom_deleter(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_custom_deleter(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Custom deleter
int solve_custom_deleter(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Custom deleter
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Custom deleter — manage who owns the memory.
