...
## 1. Problem Statement
Master the topic: **std unique_ptr**.
...
Single-owner smart pointer; non-copyable, movable. Auto-deletes on scope exit. Use `std::make_unique<T>(args)`.
...
Think of **std unique_ptr** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std unique_ptr
int main() {
...
- **Core idea:** std unique_ptr — manage who owns the memory.
