# Chapter: Pointer-to-member function
...
## 1. Problem Statement
Master the topic: **Pointer-to-member function**.
...
This problem exercises **Pointer-to-member function** — a core concept in Pointers & Memory Management.
...
Think of **Pointer-to-member function** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Pointer-to-member function
int solve_pointer_to_member_function(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_pointer_to_member_function(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Pointer-to-member function
int solve_pointer_to_member_function(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Pointer-to-member function
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Pointer-to-member function — manage who owns the memory.
