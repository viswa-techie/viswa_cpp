...
## 1. Problem Statement
Master the topic: **Placement new**.
...
This problem exercises **Placement new** — a core concept in Pointers & Memory Management.
...
Think of **Placement new** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Placement new
int solve_placement_new(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_placement_new(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Placement new
int solve_placement_new(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Placement new
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Placement new — manage who owns the memory.
