...
This problem exercises **Double free** — a core concept in Pointers & Memory Management.
...
Think of **Double free** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Double free
int solve_double_free(std::vector<int> v) {
    int result = 0;
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Double free
int solve_double_free(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Double free
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Double free — manage who owns the memory.
