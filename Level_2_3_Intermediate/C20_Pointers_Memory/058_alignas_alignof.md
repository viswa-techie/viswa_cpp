...
## 1. Problem Statement
Master the topic: **alignas alignof**.
...
This problem exercises **alignas alignof** — a core concept in Pointers & Memory Management.
...
Think of **alignas alignof** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: alignas alignof
int solve_alignas_alignof(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_alignas_alignof(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: alignas alignof
int solve_alignas_alignof(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: alignas alignof
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** alignas alignof — manage who owns the memory.
