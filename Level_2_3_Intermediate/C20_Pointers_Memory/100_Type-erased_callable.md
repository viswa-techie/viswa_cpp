...
## 1. Problem Statement
Master the topic: **Type-erased callable**.
...
This problem exercises **Type-erased callable** — a core concept in Pointers & Memory Management.
...
Think of **Type-erased callable** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Type-erased callable
int solve_type_erased_callable(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_type_erased_callable(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Type-erased callable
int solve_type_erased_callable(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Type-erased callable
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Type-erased callable — manage who owns the memory.
