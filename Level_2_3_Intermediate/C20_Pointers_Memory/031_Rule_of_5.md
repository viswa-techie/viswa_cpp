...
This problem exercises **Rule of 5** — a core concept in Pointers & Memory Management.
...
Think of **Rule of 5** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Rule of 5
int solve_rule_of_5(std::vector<int> v) {
    int result = 0;
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Rule of 5
int solve_rule_of_5(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Rule of 5
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Rule of 5 — manage who owns the memory.
