...
## 1. Problem Statement
Master the topic: **MemorySanitizer MSan**.
...
This problem exercises **MemorySanitizer MSan** — a core concept in Pointers & Memory Management.
...
Think of **MemorySanitizer MSan** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: MemorySanitizer MSan
int solve_memorysanitizer_msan(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_memorysanitizer_msan(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: MemorySanitizer MSan
int solve_memorysanitizer_msan(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: MemorySanitizer MSan
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** MemorySanitizer MSan — manage who owns the memory.
