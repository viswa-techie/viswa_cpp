# Chapter: Custom allocator for STL
...
## 1. Problem Statement
Master the topic: **Custom allocator for STL**.
...
This problem exercises **Custom allocator for STL** — a core concept in Pointers & Memory Management.
...
Think of **Custom allocator for STL** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Custom allocator for STL
int solve_custom_allocator_for_stl(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_custom_allocator_for_stl(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Custom allocator for STL
int solve_custom_allocator_for_stl(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Custom allocator for STL
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Custom allocator for STL — manage who owns the memory.
