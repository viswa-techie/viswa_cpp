# Chapter: shared_ptr reference count
...
## 1. Problem Statement
Master the topic: **shared_ptr reference count**.
...
This problem exercises **shared_ptr reference count** — a core concept in Pointers & Memory Management.
...
Think of **shared_ptr reference count** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: shared_ptr reference count
int solve_shared_ptr_reference_count(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_shared_ptr_reference_count(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: shared_ptr reference count
int solve_shared_ptr_reference_count(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: shared_ptr reference count
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** shared_ptr reference count — manage who owns the memory.
