...
## 1. Problem Statement
Master the topic: **Operator new overload**.
...
This problem exercises **Operator new overload** — a core concept in Pointers & Memory Management.
...
Think of **Operator new overload** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Operator new overload
int solve_operator_new_overload(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_operator_new_overload(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Operator new overload
int solve_operator_new_overload(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Operator new overload
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Operator new overload — manage who owns the memory.
