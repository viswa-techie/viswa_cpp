...
This problem exercises **Pimpl idiom** — a core concept in Pointers & Memory Management.
...
Think of **Pimpl idiom** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Pimpl idiom
int solve_pimpl_idiom(std::vector<int> v) {
    int result = 0;
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Pimpl idiom
int solve_pimpl_idiom(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Pimpl idiom
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Pimpl idiom — manage who owns the memory.
