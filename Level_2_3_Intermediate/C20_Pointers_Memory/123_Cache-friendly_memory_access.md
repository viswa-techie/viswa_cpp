# Chapter: Cache-friendly memory access
...
## 1. Problem Statement
Master the topic: **Cache-friendly memory access**.
...
This problem exercises **Cache-friendly memory access** — a core concept in Pointers & Memory Management.
...
Think of **Cache-friendly memory access** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Cache-friendly memory access
int solve_cache_friendly_memory_access(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_cache_friendly_memory_access(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Cache-friendly memory access
int solve_cache_friendly_memory_access(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Cache-friendly memory access
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Cache-friendly memory access — manage who owns the memory.
