...
## 1. Problem Statement
Master the topic: **std shared_ptr**.
...
Reference-counted shared ownership. Two atomic counters: strong (controls lifetime) + weak.
...
Think of **std shared_ptr** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std shared_ptr
int main() {
...
- **Core idea:** std shared_ptr — manage who owns the memory.
