# Chapter: Mark-and-sweep concept
...
## 1. Problem Statement
Master the topic: **Mark-and-sweep concept**.
...
This problem exercises **Mark-and-sweep concept** — a core concept in Pointers & Memory Management.
...
Think of **Mark-and-sweep concept** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Mark-and-sweep concept
int solve_mark_and_sweep_concept(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_mark_and_sweep_concept(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Mark-and-sweep concept
int solve_mark_and_sweep_concept(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Mark-and-sweep concept
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Mark-and-sweep concept — manage who owns the memory.
