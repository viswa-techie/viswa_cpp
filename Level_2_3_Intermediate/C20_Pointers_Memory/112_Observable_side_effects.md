# Chapter: Observable side effects
...
## 1. Problem Statement
Master the topic: **Observable side effects**.
...
This problem exercises **Observable side effects** — a core concept in Pointers & Memory Management.
...
Think of **Observable side effects** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Observable side effects
int solve_observable_side_effects(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_observable_side_effects(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Observable side effects
int solve_observable_side_effects(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Observable side effects
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Observable side effects — manage who owns the memory.
