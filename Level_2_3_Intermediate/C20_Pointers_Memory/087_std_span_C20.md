...
## 1. Problem Statement
Master the topic: **std span C++20**.
...
This problem exercises **std span C++20** — a core concept in Pointers & Memory Management.
...
Think of **std span C++20** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: std span C++20
int solve_std_span_c__20(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_span_c__20(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std span C++20
int solve_std_span_c__20(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std span C++20
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std span C++20 — manage who owns the memory.
