# Chapter: Aligned allocation aligned_new
...
## 1. Problem Statement
Master the topic: **Aligned allocation aligned_new**.
...
This problem exercises **Aligned allocation aligned_new** — a core concept in Pointers & Memory Management.
...
Think of **Aligned allocation aligned_new** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Aligned allocation aligned_new
int solve_aligned_allocation_aligned_new(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_aligned_allocation_aligned_new(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Aligned allocation aligned_new
int solve_aligned_allocation_aligned_new(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Aligned allocation aligned_new
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Aligned allocation aligned_new — manage who owns the memory.
