...
## 1. Problem Statement
Master the topic: **Opaque pointer**.
...
This problem exercises **Opaque pointer** — a core concept in Pointers & Memory Management.
...
Think of **Opaque pointer** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Opaque pointer
int solve_opaque_pointer(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_opaque_pointer(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Opaque pointer
int solve_opaque_pointer(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Opaque pointer
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Opaque pointer — manage who owns the memory.
