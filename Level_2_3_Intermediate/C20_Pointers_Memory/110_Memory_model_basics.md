...
## 1. Problem Statement
Master the topic: **Memory model basics**.
...
This problem exercises **Memory model basics** — a core concept in Pointers & Memory Management.
...
Think of **Memory model basics** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Memory model basics
int solve_memory_model_basics(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_memory_model_basics(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Memory model basics
int solve_memory_model_basics(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Memory model basics
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Memory model basics — manage who owns the memory.
