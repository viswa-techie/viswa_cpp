# Chapter: Dynamic cast with polymorphism
...
## 1. Problem Statement
Master the topic: **Dynamic cast with polymorphism**.
...
This problem exercises **Dynamic cast with polymorphism** — a core concept in Pointers & Memory Management.
...
Think of **Dynamic cast with polymorphism** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Dynamic cast with polymorphism
int solve_dynamic_cast_with_polymorphism(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_dynamic_cast_with_polymorphism(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Dynamic cast with polymorphism
int solve_dynamic_cast_with_polymorphism(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Dynamic cast with polymorphism
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Dynamic cast with polymorphism — manage who owns the memory.
