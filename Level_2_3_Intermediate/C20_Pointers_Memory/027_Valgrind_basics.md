...
## 1. Problem Statement
Master the topic: **Valgrind basics**.
...
This problem exercises **Valgrind basics** — a core concept in Pointers & Memory Management.
...
Think of **Valgrind basics** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Valgrind basics
int solve_valgrind_basics(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_valgrind_basics(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Valgrind basics
int solve_valgrind_basics(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Valgrind basics
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Valgrind basics — manage who owns the memory.
