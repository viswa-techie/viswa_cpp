...
## 1. Problem Statement
Master the topic: **Pointer aliasing**.
...
This problem exercises **Pointer aliasing** — a core concept in Pointers & Memory Management.
...
Think of **Pointer aliasing** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Pointer aliasing
int solve_pointer_aliasing(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_pointer_aliasing(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Pointer aliasing
int solve_pointer_aliasing(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Pointer aliasing
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Pointer aliasing — manage who owns the memory.
