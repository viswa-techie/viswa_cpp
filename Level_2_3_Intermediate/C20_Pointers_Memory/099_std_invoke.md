...
This problem exercises **std invoke** — a core concept in Pointers & Memory Management.
...
Think of **std invoke** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: std invoke
int solve_std_invoke(std::vector<int> v) {
    int result = 0;
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std invoke
int solve_std_invoke(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std invoke
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std invoke — manage who owns the memory.
