# Chapter: UndefinedBehaviorSanitizer
...
## 1. Problem Statement
Master the topic: **UndefinedBehaviorSanitizer**.
...
This problem exercises **UndefinedBehaviorSanitizer** — a core concept in Pointers & Memory Management.
...
Think of **UndefinedBehaviorSanitizer** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: UndefinedBehaviorSanitizer
int solve_undefinedbehaviorsanitizer(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_undefinedbehaviorsanitizer(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: UndefinedBehaviorSanitizer
int solve_undefinedbehaviorsanitizer(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: UndefinedBehaviorSanitizer
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** UndefinedBehaviorSanitizer — manage who owns the memory.
