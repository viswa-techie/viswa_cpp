...
## 1. Problem Statement
Master the topic: **Pointer subtraction**.
...
This problem exercises **Pointer subtraction** — a core concept in Pointers & Memory Management.
...
Think of **Pointer subtraction** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Pointer subtraction
int solve_pointer_subtraction(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_pointer_subtraction(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Pointer subtraction
int solve_pointer_subtraction(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Pointer subtraction
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Pointer subtraction — manage who owns the memory.
