# Chapter: Casting pointers safely
...
## 1. Problem Statement
Master the topic: **Casting pointers safely**.
...
This problem exercises **Casting pointers safely** — a core concept in Pointers & Memory Management.
...
Think of **Casting pointers safely** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Casting pointers safely
int solve_casting_pointers_safely(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_casting_pointers_safely(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Casting pointers safely
int solve_casting_pointers_safely(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Casting pointers safely
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Casting pointers safely — manage who owns the memory.
