...
## 1. Problem Statement
Master the topic: **Heap overflow**.
...
This problem exercises **Heap overflow** — a core concept in Pointers & Memory Management.
...
Think of **Heap overflow** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Heap overflow
int solve_heap_overflow(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_heap_overflow(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Heap overflow
int solve_heap_overflow(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Heap overflow
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Heap overflow — manage who owns the memory.
