...
## 1. Problem Statement
Master the topic: **Hidden implementation**.
...
This problem exercises **Hidden implementation** — a core concept in Pointers & Memory Management.
...
Think of **Hidden implementation** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Hidden implementation
int solve_hidden_implementation(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_hidden_implementation(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Hidden implementation
int solve_hidden_implementation(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Hidden implementation
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Hidden implementation — manage who owns the memory.
