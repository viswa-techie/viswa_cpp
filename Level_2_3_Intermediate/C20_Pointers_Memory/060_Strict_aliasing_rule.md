...
## 1. Problem Statement
Master the topic: **Strict aliasing rule**.
...
This problem exercises **Strict aliasing rule** — a core concept in Pointers & Memory Management.
...
Think of **Strict aliasing rule** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Strict aliasing rule
int solve_strict_aliasing_rule(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_strict_aliasing_rule(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Strict aliasing rule
int solve_strict_aliasing_rule(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Strict aliasing rule
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Strict aliasing rule — manage who owns the memory.
