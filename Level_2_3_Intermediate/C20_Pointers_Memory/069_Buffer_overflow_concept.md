# Chapter: Buffer overflow concept
...
## 1. Problem Statement
Master the topic: **Buffer overflow concept**.
...
This problem exercises **Buffer overflow concept** — a core concept in Pointers & Memory Management.
...
Think of **Buffer overflow concept** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Buffer overflow concept
int solve_buffer_overflow_concept(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_buffer_overflow_concept(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Buffer overflow concept
int solve_buffer_overflow_concept(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Buffer overflow concept
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Buffer overflow concept — manage who owns the memory.
