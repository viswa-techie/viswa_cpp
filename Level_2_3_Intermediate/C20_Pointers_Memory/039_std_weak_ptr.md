...
This problem exercises **std weak_ptr** — a core concept in Pointers & Memory Management.
...
Think of **std weak_ptr** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: std weak_ptr
int solve_std_weak_ptr(std::vector<int> v) {
    int result = 0;
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std weak_ptr
int solve_std_weak_ptr(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std weak_ptr
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std weak_ptr — manage who owns the memory.
