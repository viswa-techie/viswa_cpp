...
This problem exercises **void pointer** — a core concept in Pointers & Memory Management.
...
Think of **void pointer** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: void pointer
int solve_void_pointer(std::vector<int> v) {
    int result = 0;
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: void pointer
int solve_void_pointer(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: void pointer
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** void pointer — manage who owns the memory.
