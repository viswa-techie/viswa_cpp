# Chapter: Breaking cycles with weak_ptr
...
## 1. Problem Statement
Master the topic: **Breaking cycles with weak_ptr**.
...
This problem exercises **Breaking cycles with weak_ptr** — a core concept in Pointers & Memory Management.
...
Think of **Breaking cycles with weak_ptr** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Breaking cycles with weak_ptr
int solve_breaking_cycles_with_weak_ptr(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_breaking_cycles_with_weak_ptr(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Breaking cycles with weak_ptr
int solve_breaking_cycles_with_weak_ptr(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Breaking cycles with weak_ptr
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Breaking cycles with weak_ptr — manage who owns the memory.
