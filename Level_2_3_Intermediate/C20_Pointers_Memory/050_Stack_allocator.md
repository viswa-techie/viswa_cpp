...
## 1. Problem Statement
Master the topic: **Stack allocator**.
...
This problem exercises **Stack allocator** — a core concept in Pointers & Memory Management.
...
Think of **Stack allocator** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Stack allocator
int solve_stack_allocator(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_stack_allocator(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Stack allocator
int solve_stack_allocator(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Stack allocator
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Stack allocator — manage who owns the memory.
