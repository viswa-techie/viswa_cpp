# Chapter: Destructor responsibility
...
## 1. Problem Statement
Master the topic: **Destructor responsibility**.
...
This problem exercises **Destructor responsibility** — a core concept in Pointers & Memory Management.
...
Think of **Destructor responsibility** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Destructor responsibility
int solve_destructor_responsibility(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_destructor_responsibility(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Destructor responsibility
int solve_destructor_responsibility(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Destructor responsibility
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Destructor responsibility — manage who owns the memory.
