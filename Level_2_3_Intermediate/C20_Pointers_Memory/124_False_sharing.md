...
## 1. Problem Statement
Master the topic: **False sharing**.
...
This problem exercises **False sharing** — a core concept in Pointers & Memory Management.
...
Think of **False sharing** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: False sharing
int solve_false_sharing(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_false_sharing(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: False sharing
int solve_false_sharing(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: False sharing
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** False sharing — manage who owns the memory.
