# Chapter: make_unique make_shared
...
## 1. Problem Statement
Master the topic: **make_unique make_shared**.
...
This problem exercises **make_unique make_shared** — a core concept in Pointers & Memory Management.
...
Think of **make_unique make_shared** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: make_unique make_shared
int solve_make_unique_make_shared(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_make_unique_make_shared(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: make_unique make_shared
int solve_make_unique_make_shared(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: make_unique make_shared
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** make_unique make_shared — manage who owns the memory.
