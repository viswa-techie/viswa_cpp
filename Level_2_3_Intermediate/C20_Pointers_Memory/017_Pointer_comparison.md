...
## 1. Problem Statement
Master the topic: **Pointer comparison**.
...
This problem exercises **Pointer comparison** — a core concept in Pointers & Memory Management.
...
Think of **Pointer comparison** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Pointer comparison
int solve_pointer_comparison(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_pointer_comparison(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Pointer comparison
int solve_pointer_comparison(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Pointer comparison
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Pointer comparison — manage who owns the memory.
