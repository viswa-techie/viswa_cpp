# Chapter: const T pointer variants
...
## 1. Problem Statement
Master the topic: **const T pointer variants**.
...
This problem exercises **const T pointer variants** — a core concept in Pointers & Memory Management.
...
Think of **const T pointer variants** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: const T pointer variants
int solve_const_t_pointer_variants(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_const_t_pointer_variants(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: const T pointer variants
int solve_const_t_pointer_variants(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: const T pointer variants
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** const T pointer variants — manage who owns the memory.
