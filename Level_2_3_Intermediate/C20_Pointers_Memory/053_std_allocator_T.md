...
## 1. Problem Statement
Master the topic: **std allocator T**.
...
This problem exercises **std allocator T** — a core concept in Pointers & Memory Management.
...
Think of **std allocator T** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: std allocator T
int solve_std_allocator_t(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_allocator_t(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std allocator T
int solve_std_allocator_t(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std allocator T
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std allocator T — manage who owns the memory.
