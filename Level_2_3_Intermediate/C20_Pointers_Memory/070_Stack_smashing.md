...
## 1. Problem Statement
Master the topic: **Stack smashing**.
...
This problem exercises **Stack smashing** — a core concept in Pointers & Memory Management.
...
Think of **Stack smashing** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Stack smashing
int solve_stack_smashing(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_stack_smashing(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Stack smashing
int solve_stack_smashing(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Stack smashing
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Stack smashing — manage who owns the memory.
