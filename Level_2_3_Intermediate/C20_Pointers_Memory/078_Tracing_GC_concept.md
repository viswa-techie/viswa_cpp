...
## 1. Problem Statement
Master the topic: **Tracing GC concept**.
...
This problem exercises **Tracing GC concept** — a core concept in Pointers & Memory Management.
...
Think of **Tracing GC concept** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Tracing GC concept
int solve_tracing_gc_concept(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_tracing_gc_concept(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Tracing GC concept
int solve_tracing_gc_concept(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Tracing GC concept
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Tracing GC concept — manage who owns the memory.
