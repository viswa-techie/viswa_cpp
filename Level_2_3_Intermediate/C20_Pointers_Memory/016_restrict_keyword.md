...
## 1. Problem Statement
Master the topic: **restrict keyword**.
...
This problem exercises **restrict keyword** — a core concept in Pointers & Memory Management.
...
Think of **restrict keyword** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: restrict keyword
int solve_restrict_keyword(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_restrict_keyword(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: restrict keyword
int solve_restrict_keyword(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: restrict keyword
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** restrict keyword — manage who owns the memory.
