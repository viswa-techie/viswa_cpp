...
## 1. Problem Statement
Master the topic: **Union type punning**.
...
This problem exercises **Union type punning** — a core concept in Pointers & Memory Management.
...
Think of **Union type punning** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Union type punning
int solve_union_type_punning(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_union_type_punning(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Union type punning
int solve_union_type_punning(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Union type punning
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Union type punning — manage who owns the memory.
