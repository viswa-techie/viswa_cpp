...
## 1. Problem Statement
Master the topic: **Sequence point rules**.
...
This problem exercises **Sequence point rules** — a core concept in Pointers & Memory Management.
...
Think of **Sequence point rules** as an arrow pointing to a memory cell — the arrow itself lives in a variable.
...
#include <vector>
#include <string>
// Brute-force template for: Sequence point rules
int solve_sequence_point_rules(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_sequence_point_rules(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Sequence point rules
int solve_sequence_point_rules(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Sequence point rules
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Sequence point rules — manage who owns the memory.
