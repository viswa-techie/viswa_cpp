...
## 1. Problem Statement
Master the topic: **Remove Nth from end**.
...
This problem exercises **Remove Nth from end** — a core concept in Linked Lists.
...
Think of **Remove Nth from end** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Remove Nth from end
int solve_remove_nth_from_end(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_remove_nth_from_end(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Remove Nth from end
int solve_remove_nth_from_end(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Remove Nth from end
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Remove Nth from end — chain of nodes via pointers.
