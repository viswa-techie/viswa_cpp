...
## 1. Problem Statement
Master the topic: **Middle of list**.
...
This problem exercises **Middle of list** — a core concept in Linked Lists.
...
Think of **Middle of list** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Middle of list
int solve_middle_of_list(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_middle_of_list(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Middle of list
int solve_middle_of_list(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Middle of list
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Middle of list — chain of nodes via pointers.
