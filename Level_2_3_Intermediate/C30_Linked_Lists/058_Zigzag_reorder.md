...
## 1. Problem Statement
Master the topic: **Zigzag reorder**.
...
This problem exercises **Zigzag reorder** — a core concept in Linked Lists.
...
Think of **Zigzag reorder** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Zigzag reorder
int solve_zigzag_reorder(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_zigzag_reorder(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Zigzag reorder
int solve_zigzag_reorder(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Zigzag reorder
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Zigzag reorder — chain of nodes via pointers.
