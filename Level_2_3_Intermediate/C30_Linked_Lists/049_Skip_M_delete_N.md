...
## 1. Problem Statement
Master the topic: **Skip M delete N**.
...
This problem exercises **Skip M delete N** — a core concept in Linked Lists.
...
Think of **Skip M delete N** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Skip M delete N
int solve_skip_m_delete_n(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_skip_m_delete_n(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Skip M delete N
int solve_skip_m_delete_n(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Skip M delete N
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Skip M delete N — chain of nodes via pointers.
