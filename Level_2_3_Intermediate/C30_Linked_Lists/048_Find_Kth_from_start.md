...
## 1. Problem Statement
Master the topic: **Find Kth from start**.
...
This problem exercises **Find Kth from start** — a core concept in Linked Lists.
...
Think of **Find Kth from start** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Find Kth from start
int solve_find_kth_from_start(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_find_kth_from_start(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Find Kth from start
int solve_find_kth_from_start(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Find Kth from start
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Find Kth from start — chain of nodes via pointers.
