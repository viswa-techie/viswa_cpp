...
## 1. Problem Statement
Master the topic: **Swap nodes in pairs**.
...
This problem exercises **Swap nodes in pairs** — a core concept in Linked Lists.
...
Think of **Swap nodes in pairs** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Swap nodes in pairs
int solve_swap_nodes_in_pairs(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_swap_nodes_in_pairs(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Swap nodes in pairs
int solve_swap_nodes_in_pairs(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Swap nodes in pairs
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Swap nodes in pairs — chain of nodes via pointers.
