# Chapter: Remove nodes not in range
...
## 1. Problem Statement
Master the topic: **Remove nodes not in range**.
...
This problem exercises **Remove nodes not in range** — a core concept in Linked Lists.
...
Think of **Remove nodes not in range** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Remove nodes not in range
int solve_remove_nodes_not_in_range(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_remove_nodes_not_in_range(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Remove nodes not in range
int solve_remove_nodes_not_in_range(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Remove nodes not in range
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Remove nodes not in range — chain of nodes via pointers.
