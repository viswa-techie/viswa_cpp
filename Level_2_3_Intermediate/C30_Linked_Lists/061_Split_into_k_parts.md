...
## 1. Problem Statement
Master the topic: **Split into k parts**.
...
This problem exercises **Split into k parts** — a core concept in Linked Lists.
...
Think of **Split into k parts** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Split into k parts
int solve_split_into_k_parts(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_split_into_k_parts(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Split into k parts
int solve_split_into_k_parts(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Split into k parts
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Split into k parts — chain of nodes via pointers.
