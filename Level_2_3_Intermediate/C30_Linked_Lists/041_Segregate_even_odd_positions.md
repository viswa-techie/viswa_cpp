# Chapter: Segregate even odd positions
...
## 1. Problem Statement
Master the topic: **Segregate even odd positions**.
...
This problem exercises **Segregate even odd positions** — a core concept in Linked Lists.
...
Think of **Segregate even odd positions** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Segregate even odd positions
int solve_segregate_even_odd_positions(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_segregate_even_odd_positions(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Segregate even odd positions
int solve_segregate_even_odd_positions(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Segregate even odd positions
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Segregate even odd positions — chain of nodes via pointers.
