# Chapter: Queue using two stacks
...
## 1. Problem Statement
Master the topic: **Queue using two stacks**.
...
This problem exercises **Queue using two stacks** — a core concept in Linked Lists.
...
Think of **Queue using two stacks** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Queue using two stacks
int solve_queue_using_two_stacks(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_queue_using_two_stacks(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Queue using two stacks
int solve_queue_using_two_stacks(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Queue using two stacks
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Queue using two stacks — chain of nodes via pointers.
