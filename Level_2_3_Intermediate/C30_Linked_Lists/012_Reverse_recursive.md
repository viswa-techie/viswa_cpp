...
## 1. Problem Statement
Master the topic: **Reverse recursive**.
...
This problem exercises **Reverse recursive** — a core concept in Linked Lists.
...
Think of **Reverse recursive** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Reverse recursive
int solve_reverse_recursive(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_reverse_recursive(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Reverse recursive
int solve_reverse_recursive(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Reverse recursive
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Reverse recursive — chain of nodes via pointers.
