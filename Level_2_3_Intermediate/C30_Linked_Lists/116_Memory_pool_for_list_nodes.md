# Chapter: Memory pool for list nodes
...
## 1. Problem Statement
Master the topic: **Memory pool for list nodes**.
...
This problem exercises **Memory pool for list nodes** — a core concept in Linked Lists.
...
Think of **Memory pool for list nodes** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Memory pool for list nodes
int solve_memory_pool_for_list_nodes(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_memory_pool_for_list_nodes(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Memory pool for list nodes
int solve_memory_pool_for_list_nodes(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Memory pool for list nodes
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Memory pool for list nodes — chain of nodes via pointers.
