# Chapter: Odd-even node rearrangement
...
## 1. Problem Statement
Master the topic: **Odd-even node rearrangement**.
...
This problem exercises **Odd-even node rearrangement** — a core concept in Linked Lists.
...
Think of **Odd-even node rearrangement** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Odd-even node rearrangement
int solve_odd_even_node_rearrangement(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_odd_even_node_rearrangement(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Odd-even node rearrangement
int solve_odd_even_node_rearrangement(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Odd-even node rearrangement
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Odd-even node rearrangement — chain of nodes via pointers.
