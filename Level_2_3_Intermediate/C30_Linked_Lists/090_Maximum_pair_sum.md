...
## 1. Problem Statement
Master the topic: **Maximum pair sum**.
...
This problem exercises **Maximum pair sum** — a core concept in Linked Lists.
...
Think of **Maximum pair sum** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Maximum pair sum
int solve_maximum_pair_sum(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_maximum_pair_sum(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Maximum pair sum
int solve_maximum_pair_sum(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Maximum pair sum
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Maximum pair sum — chain of nodes via pointers.
