# Chapter: Partition list around x
...
## 1. Problem Statement
Master the topic: **Partition list around x**.
...
This problem exercises **Partition list around x** — a core concept in Linked Lists.
...
Think of **Partition list around x** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Partition list around x
int solve_partition_list_around_x(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_partition_list_around_x(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Partition list around x
int solve_partition_list_around_x(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Partition list around x
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Partition list around x — chain of nodes via pointers.
