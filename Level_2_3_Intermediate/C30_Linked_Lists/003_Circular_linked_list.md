...
## 1. Problem Statement
Master the topic: **Circular linked list**.
...
This problem exercises **Circular linked list** — a core concept in Linked Lists.
...
Think of **Circular linked list** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Circular linked list
int solve_circular_linked_list(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_circular_linked_list(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Circular linked list
int solve_circular_linked_list(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Circular linked list
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Circular linked list — chain of nodes via pointers.
