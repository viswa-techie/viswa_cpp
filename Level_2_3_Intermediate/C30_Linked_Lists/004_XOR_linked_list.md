...
## 1. Problem Statement
Master the topic: **XOR linked list**.
...
This problem exercises **XOR linked list** — a core concept in Linked Lists.
...
Think of **XOR linked list** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: XOR linked list
int solve_xor_linked_list(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_xor_linked_list(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: XOR linked list
int solve_xor_linked_list(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: XOR linked list
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** XOR linked list — chain of nodes via pointers.
