...
## 1. Problem Statement
Master the topic: **Self-organizing list**.
...
This problem exercises **Self-organizing list** — a core concept in Linked Lists.
...
Think of **Self-organizing list** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Self-organizing list
int solve_self_organizing_list(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_self_organizing_list(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Self-organizing list
int solve_self_organizing_list(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Self-organizing list
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Self-organizing list — chain of nodes via pointers.
