...
This problem exercises **Reorder list** — a core concept in Linked Lists.
...
Think of **Reorder list** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Reorder list
int solve_reorder_list(std::vector<int> v) {
    int result = 0;
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Reorder list
int solve_reorder_list(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Reorder list
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Reorder list — chain of nodes via pointers.
