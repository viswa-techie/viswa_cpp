# Chapter: Split list into two halves
...
## 1. Problem Statement
Master the topic: **Split list into two halves**.
...
This problem exercises **Split list into two halves** — a core concept in Linked Lists.
...
Think of **Split list into two halves** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Split list into two halves
int solve_split_list_into_two_halves(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_split_list_into_two_halves(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Split list into two halves
int solve_split_list_into_two_halves(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Split list into two halves
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Split list into two halves — chain of nodes via pointers.
