...
## 1. Problem Statement
Master the topic: **List merge sorted**.
...
This problem exercises **List merge sorted** — a core concept in Linked Lists.
...
Think of **List merge sorted** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: List merge sorted
int solve_list_merge_sorted(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_list_merge_sorted(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: List merge sorted
int solve_list_merge_sorted(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: List merge sorted
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** List merge sorted — chain of nodes via pointers.
