# Chapter: Delete without head pointer
...
## 1. Problem Statement
Master the topic: **Delete without head pointer**.
...
This problem exercises **Delete without head pointer** — a core concept in Linked Lists.
...
Think of **Delete without head pointer** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Delete without head pointer
int solve_delete_without_head_pointer(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_delete_without_head_pointer(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Delete without head pointer
int solve_delete_without_head_pointer(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Delete without head pointer
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Delete without head pointer — chain of nodes via pointers.
