# Chapter: Multiply two numbers list
...
## 1. Problem Statement
Master the topic: **Multiply two numbers list**.
...
This problem exercises **Multiply two numbers list** — a core concept in Linked Lists.
...
Think of **Multiply two numbers list** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Multiply two numbers list
int solve_multiply_two_numbers_list(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_multiply_two_numbers_list(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Multiply two numbers list
int solve_multiply_two_numbers_list(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Multiply two numbers list
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Multiply two numbers list — chain of nodes via pointers.
