...
## 1. Problem Statement
Master the topic: **Print list forward**.
...
This problem exercises **Print list forward** — a core concept in Linked Lists.
...
Think of **Print list forward** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Print list forward
int solve_print_list_forward(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_print_list_forward(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Print list forward
int solve_print_list_forward(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Print list forward
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Print list forward — chain of nodes via pointers.
