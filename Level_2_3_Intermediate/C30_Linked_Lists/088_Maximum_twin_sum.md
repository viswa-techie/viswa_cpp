...
## 1. Problem Statement
Master the topic: **Maximum twin sum**.
...
This problem exercises **Maximum twin sum** — a core concept in Linked Lists.
...
Think of **Maximum twin sum** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Maximum twin sum
int solve_maximum_twin_sum(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_maximum_twin_sum(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Maximum twin sum
int solve_maximum_twin_sum(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Maximum twin sum
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Maximum twin sum — chain of nodes via pointers.
