...
## 1. Problem Statement
Master the topic: **Check list is sorted**.
...
This problem exercises **Check list is sorted** — a core concept in Linked Lists.
...
Think of **Check list is sorted** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Check list is sorted
int solve_check_list_is_sorted(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_check_list_is_sorted(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Check list is sorted
int solve_check_list_is_sorted(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Check list is sorted
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Check list is sorted — chain of nodes via pointers.
