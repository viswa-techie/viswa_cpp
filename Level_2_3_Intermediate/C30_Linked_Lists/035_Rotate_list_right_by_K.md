# Chapter: Rotate list right by K
...
## 1. Problem Statement
Master the topic: **Rotate list right by K**.
...
This problem exercises **Rotate list right by K** — a core concept in Linked Lists.
...
Think of **Rotate list right by K** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Rotate list right by K
int solve_rotate_list_right_by_k(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_rotate_list_right_by_k(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Rotate list right by K
int solve_rotate_list_right_by_k(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Rotate list right by K
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Rotate list right by K — chain of nodes via pointers.
