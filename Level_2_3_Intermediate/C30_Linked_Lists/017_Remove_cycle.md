...
This problem exercises **Remove cycle** — a core concept in Linked Lists.
...
Think of **Remove cycle** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Remove cycle
int solve_remove_cycle(std::vector<int> v) {
    int result = 0;
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Remove cycle
int solve_remove_cycle(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Remove cycle
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Remove cycle — chain of nodes via pointers.
