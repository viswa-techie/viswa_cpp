...
## 1. Problem Statement
Master the topic: **Add 1 to list number**.
...
This problem exercises **Add 1 to list number** — a core concept in Linked Lists.
...
Think of **Add 1 to list number** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Add 1 to list number
int solve_add_1_to_list_number(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_add_1_to_list_number(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Add 1 to list number
int solve_add_1_to_list_number(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Add 1 to list number
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Add 1 to list number — chain of nodes via pointers.
