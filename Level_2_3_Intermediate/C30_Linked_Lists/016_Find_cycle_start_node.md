...
## 1. Problem Statement
Master the topic: **Find cycle start node**.
...
This problem exercises **Find cycle start node** — a core concept in Linked Lists.
...
Think of **Find cycle start node** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Find cycle start node
int solve_find_cycle_start_node(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_find_cycle_start_node(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Find cycle start node
int solve_find_cycle_start_node(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Find cycle start node
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Find cycle start node — chain of nodes via pointers.
