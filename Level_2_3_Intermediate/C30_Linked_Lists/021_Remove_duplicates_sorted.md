# Chapter: Remove duplicates sorted
...
## 1. Problem Statement
Master the topic: **Remove duplicates sorted**.
...
This problem exercises **Remove duplicates sorted** — a core concept in Linked Lists.
...
Think of **Remove duplicates sorted** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Remove duplicates sorted
int solve_remove_duplicates_sorted(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_remove_duplicates_sorted(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Remove duplicates sorted
int solve_remove_duplicates_sorted(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Remove duplicates sorted
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Remove duplicates sorted — chain of nodes via pointers.
