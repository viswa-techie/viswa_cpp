...
This problem exercises **List splice** — a core concept in Linked Lists.
...
Think of **List splice** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: List splice
int solve_list_splice(std::vector<int> v) {
    int result = 0;
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: List splice
int solve_list_splice(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: List splice
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** List splice — chain of nodes via pointers.
