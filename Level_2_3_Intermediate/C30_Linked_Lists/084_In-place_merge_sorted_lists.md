# Chapter: In-place merge sorted lists
...
## 1. Problem Statement
Master the topic: **In-place merge sorted lists**.
...
This problem exercises **In-place merge sorted lists** — a core concept in Linked Lists.
...
Think of **In-place merge sorted lists** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: In-place merge sorted lists
int solve_in_place_merge_sorted_lists(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_in_place_merge_sorted_lists(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: In-place merge sorted lists
int solve_in_place_merge_sorted_lists(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: In-place merge sorted lists
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** In-place merge sorted lists — chain of nodes via pointers.
