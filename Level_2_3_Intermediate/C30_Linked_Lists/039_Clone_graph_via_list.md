...
## 1. Problem Statement
Master the topic: **Clone graph via list**.
...
This problem exercises **Clone graph via list** — a core concept in Linked Lists.
...
Think of **Clone graph via list** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Clone graph via list
int solve_clone_graph_via_list(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_clone_graph_via_list(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Clone graph via list
int solve_clone_graph_via_list(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Clone graph via list
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Clone graph via list — chain of nodes via pointers.
