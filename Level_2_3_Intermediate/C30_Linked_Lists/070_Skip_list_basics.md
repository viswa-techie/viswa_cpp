...
## 1. Problem Statement
Master the topic: **Skip list basics**.
...
This problem exercises **Skip list basics** — a core concept in Linked Lists.
...
Think of **Skip list basics** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Skip list basics
int solve_skip_list_basics(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_skip_list_basics(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Skip list basics
int solve_skip_list_basics(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Skip list basics
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Skip list basics — chain of nodes via pointers.
