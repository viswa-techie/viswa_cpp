...
## 1. Problem Statement
Master the topic: **Check if palindrome**.
...
This problem exercises **Check if palindrome** — a core concept in Linked Lists.
...
Think of **Check if palindrome** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Check if palindrome
int solve_check_if_palindrome(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_check_if_palindrome(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Check if palindrome
int solve_check_if_palindrome(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Check if palindrome
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Check if palindrome — chain of nodes via pointers.
