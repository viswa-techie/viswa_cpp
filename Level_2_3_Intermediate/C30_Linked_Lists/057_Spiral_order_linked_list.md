# Chapter: Spiral order linked list
...
## 1. Problem Statement
Master the topic: **Spiral order linked list**.
...
This problem exercises **Spiral order linked list** — a core concept in Linked Lists.
...
Think of **Spiral order linked list** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Spiral order linked list
int solve_spiral_order_linked_list(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_spiral_order_linked_list(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Spiral order linked list
int solve_spiral_order_linked_list(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Spiral order linked list
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Spiral order linked list — chain of nodes via pointers.
