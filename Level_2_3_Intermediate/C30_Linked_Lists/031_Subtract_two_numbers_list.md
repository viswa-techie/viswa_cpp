# Chapter: Subtract two numbers list
...
## 1. Problem Statement
Master the topic: **Subtract two numbers list**.
...
This problem exercises **Subtract two numbers list** — a core concept in Linked Lists.
...
Think of **Subtract two numbers list** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Subtract two numbers list
int solve_subtract_two_numbers_list(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_subtract_two_numbers_list(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Subtract two numbers list
int solve_subtract_two_numbers_list(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Subtract two numbers list
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Subtract two numbers list — chain of nodes via pointers.
