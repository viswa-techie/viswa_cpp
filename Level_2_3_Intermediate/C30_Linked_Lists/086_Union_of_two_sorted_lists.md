# Chapter: Union of two sorted lists
...
## 1. Problem Statement
Master the topic: **Union of two sorted lists**.
...
This problem exercises **Union of two sorted lists** — a core concept in Linked Lists.
...
Think of **Union of two sorted lists** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Union of two sorted lists
int solve_union_of_two_sorted_lists(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_union_of_two_sorted_lists(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Union of two sorted lists
int solve_union_of_two_sorted_lists(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Union of two sorted lists
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Union of two sorted lists — chain of nodes via pointers.
