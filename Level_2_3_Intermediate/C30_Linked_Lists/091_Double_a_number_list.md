...
## 1. Problem Statement
Master the topic: **Double a number list**.
...
This problem exercises **Double a number list** — a core concept in Linked Lists.
...
Think of **Double a number list** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Double a number list
int solve_double_a_number_list(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_double_a_number_list(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Double a number list
int solve_double_a_number_list(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Double a number list
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Double a number list — chain of nodes via pointers.
