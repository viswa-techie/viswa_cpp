...
## 1. Problem Statement
Master the topic: **List remove remove_if**.
...
This problem exercises **List remove remove_if** — a core concept in Linked Lists.
...
Think of **List remove remove_if** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: List remove remove_if
int solve_list_remove_remove_if(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_list_remove_remove_if(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: List remove remove_if
int solve_list_remove_remove_if(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: List remove remove_if
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** List remove remove_if — chain of nodes via pointers.
