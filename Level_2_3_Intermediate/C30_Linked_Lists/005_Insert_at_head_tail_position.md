# Chapter: Insert at head tail position
...
## 1. Problem Statement
Master the topic: **Insert at head tail position**.
...
This problem exercises **Insert at head tail position** — a core concept in Linked Lists.
...
Think of **Insert at head tail position** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Insert at head tail position
int solve_insert_at_head_tail_position(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_insert_at_head_tail_position(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Insert at head tail position
int solve_insert_at_head_tail_position(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Insert at head tail position
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Insert at head tail position — chain of nodes via pointers.
