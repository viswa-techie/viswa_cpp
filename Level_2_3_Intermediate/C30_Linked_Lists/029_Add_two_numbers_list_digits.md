# Chapter: Add two numbers list digits
...
## 1. Problem Statement
Master the topic: **Add two numbers list digits**.
...
This problem exercises **Add two numbers list digits** — a core concept in Linked Lists.
...
Think of **Add two numbers list digits** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Add two numbers list digits
int solve_add_two_numbers_list_digits(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_add_two_numbers_list_digits(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Add two numbers list digits
int solve_add_two_numbers_list_digits(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Add two numbers list digits
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Add two numbers list digits — chain of nodes via pointers.
