# Chapter: Delete by value position
...
## 1. Problem Statement
Master the topic: **Delete by value position**.
...
This problem exercises **Delete by value position** — a core concept in Linked Lists.
...
Think of **Delete by value position** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Delete by value position
int solve_delete_by_value_position(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_delete_by_value_position(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Delete by value position
int solve_delete_by_value_position(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Delete by value position
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Delete by value position — chain of nodes via pointers.
