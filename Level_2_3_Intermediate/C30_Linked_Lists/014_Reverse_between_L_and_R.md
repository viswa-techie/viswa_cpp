# Chapter: Reverse between L and R
...
## 1. Problem Statement
Master the topic: **Reverse between L and R**.
...
This problem exercises **Reverse between L and R** — a core concept in Linked Lists.
...
Think of **Reverse between L and R** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Reverse between L and R
int solve_reverse_between_l_and_r(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_reverse_between_l_and_r(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Reverse between L and R
int solve_reverse_between_l_and_r(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Reverse between L and R
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Reverse between L and R — chain of nodes via pointers.
