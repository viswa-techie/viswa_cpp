# Chapter: Move zeros to end list
...
## 1. Problem Statement
Master the topic: **Move zeros to end list**.
...
This problem exercises **Move zeros to end list** — a core concept in Linked Lists.
...
Think of **Move zeros to end list** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Move zeros to end list
int solve_move_zeros_to_end_list(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_move_zeros_to_end_list(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Move zeros to end list
int solve_move_zeros_to_end_list(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Move zeros to end list
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Move zeros to end list — chain of nodes via pointers.
