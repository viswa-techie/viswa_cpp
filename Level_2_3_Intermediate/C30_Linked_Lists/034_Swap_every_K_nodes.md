...
## 1. Problem Statement
Master the topic: **Swap every K nodes**.
...
This problem exercises **Swap every K nodes** — a core concept in Linked Lists.
...
Think of **Swap every K nodes** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Swap every K nodes
int solve_swap_every_k_nodes(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_swap_every_k_nodes(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Swap every K nodes
int solve_swap_every_k_nodes(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Swap every K nodes
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Swap every K nodes — chain of nodes via pointers.
