...
## 1. Problem Statement
Master the topic: **Josephus problem list**.
...
This problem exercises **Josephus problem list** — a core concept in Linked Lists.
...
Think of **Josephus problem list** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Josephus problem list
int solve_josephus_problem_list(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_josephus_problem_list(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Josephus problem list
int solve_josephus_problem_list(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Josephus problem list
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Josephus problem list — chain of nodes via pointers.
