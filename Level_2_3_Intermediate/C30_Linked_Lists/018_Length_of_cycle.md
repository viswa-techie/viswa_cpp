...
## 1. Problem Statement
Master the topic: **Length of cycle**.
...
This problem exercises **Length of cycle** — a core concept in Linked Lists.
...
Think of **Length of cycle** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Length of cycle
int solve_length_of_cycle(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_length_of_cycle(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Length of cycle
int solve_length_of_cycle(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Length of cycle
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Length of cycle — chain of nodes via pointers.
