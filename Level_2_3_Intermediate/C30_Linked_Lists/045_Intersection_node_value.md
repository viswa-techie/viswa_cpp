# Chapter: Intersection node value
...
## 1. Problem Statement
Master the topic: **Intersection node value**.
...
This problem exercises **Intersection node value** — a core concept in Linked Lists.
...
Think of **Intersection node value** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Intersection node value
int solve_intersection_node_value(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_intersection_node_value(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Intersection node value
int solve_intersection_node_value(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Intersection node value
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Intersection node value — chain of nodes via pointers.
