# Chapter: Reverse in groups of K
...
## 1. Problem Statement
Master the topic: **Reverse in groups of K**.
...
This problem exercises **Reverse in groups of K** — a core concept in Linked Lists.
...
Think of **Reverse in groups of K** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Reverse in groups of K
int solve_reverse_in_groups_of_k(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_reverse_in_groups_of_k(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Reverse in groups of K
int solve_reverse_in_groups_of_k(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Reverse in groups of K
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Reverse in groups of K — chain of nodes via pointers.
