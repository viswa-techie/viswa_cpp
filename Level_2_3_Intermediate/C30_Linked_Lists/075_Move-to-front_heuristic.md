# Chapter: Move-to-front heuristic
...
## 1. Problem Statement
Master the topic: **Move-to-front heuristic**.
...
This problem exercises **Move-to-front heuristic** — a core concept in Linked Lists.
...
Think of **Move-to-front heuristic** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Move-to-front heuristic
int solve_move_to_front_heuristic(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_move_to_front_heuristic(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Move-to-front heuristic
int solve_move_to_front_heuristic(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Move-to-front heuristic
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Move-to-front heuristic — chain of nodes via pointers.
