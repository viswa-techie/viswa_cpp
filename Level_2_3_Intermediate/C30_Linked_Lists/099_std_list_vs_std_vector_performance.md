# Chapter: std list vs std vector performance
...
## 1. Problem Statement
Master the topic: **std list vs std vector performance**.
...
This problem exercises **std list vs std vector performance** — a core concept in Linked Lists.
...
Think of **std list vs std vector performance** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: std list vs std vector performance
int solve_std_list_vs_std_vector_performance(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_list_vs_std_vector_performance(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std list vs std vector performance
int solve_std_list_vs_std_vector_performance(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std list vs std vector performance
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std list vs std vector performance — chain of nodes via pointers.
