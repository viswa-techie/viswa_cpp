# Chapter: Flatten multilevel list
...
## 1. Problem Statement
Master the topic: **Flatten multilevel list**.
...
This problem exercises **Flatten multilevel list** — a core concept in Linked Lists.
...
Think of **Flatten multilevel list** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Flatten multilevel list
int solve_flatten_multilevel_list(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_flatten_multilevel_list(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Flatten multilevel list
int solve_flatten_multilevel_list(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Flatten multilevel list
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Flatten multilevel list — chain of nodes via pointers.
