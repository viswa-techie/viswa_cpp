# Chapter: List with O(1) random access skip list
...
## 1. Problem Statement
Master the topic: **List with O(1) random access skip list**.
...
This problem exercises **List with O(1) random access skip list** — a core concept in Linked Lists.
...
Think of **List with O(1) random access skip list** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: List with O(1) random access skip list
int solve_list_with_o_1__random_access_skip_list(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_list_with_o_1__random_access_skip_list(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: List with O(1) random access skip list
int solve_list_with_o_1__random_access_skip_list(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: List with O(1) random access skip list
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** List with O(1) random access skip list — chain of nodes via pointers.
