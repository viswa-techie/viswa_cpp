# Chapter: Link nodes at k distance
...
## 1. Problem Statement
Master the topic: **Link nodes at k distance**.
...
This problem exercises **Link nodes at k distance** — a core concept in Linked Lists.
...
Think of **Link nodes at k distance** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Link nodes at k distance
int solve_link_nodes_at_k_distance(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_link_nodes_at_k_distance(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Link nodes at k distance
int solve_link_nodes_at_k_distance(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Link nodes at k distance
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Link nodes at k distance — chain of nodes via pointers.
