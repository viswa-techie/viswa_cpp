# Chapter: Swapping nodes not values
...
## 1. Problem Statement
Master the topic: **Swapping nodes not values**.
...
This problem exercises **Swapping nodes not values** — a core concept in Linked Lists.
...
Think of **Swapping nodes not values** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Swapping nodes not values
int solve_swapping_nodes_not_values(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_swapping_nodes_not_values(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Swapping nodes not values
int solve_swapping_nodes_not_values(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Swapping nodes not values
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Swapping nodes not values — chain of nodes via pointers.
