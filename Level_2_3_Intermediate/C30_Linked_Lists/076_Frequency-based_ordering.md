# Chapter: Frequency-based ordering
...
## 1. Problem Statement
Master the topic: **Frequency-based ordering**.
...
This problem exercises **Frequency-based ordering** — a core concept in Linked Lists.
...
Think of **Frequency-based ordering** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Frequency-based ordering
int solve_frequency_based_ordering(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_frequency_based_ordering(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Frequency-based ordering
int solve_frequency_based_ordering(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Frequency-based ordering
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Frequency-based ordering — chain of nodes via pointers.
