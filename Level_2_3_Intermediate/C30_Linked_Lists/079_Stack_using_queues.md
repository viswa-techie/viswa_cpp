...
## 1. Problem Statement
Master the topic: **Stack using queues**.
...
This problem exercises **Stack using queues** — a core concept in Linked Lists.
...
Think of **Stack using queues** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Stack using queues
int solve_stack_using_queues(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_stack_using_queues(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Stack using queues
int solve_stack_using_queues(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Stack using queues
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Stack using queues — chain of nodes via pointers.
