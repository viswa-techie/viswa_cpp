...
This problem exercises **List reverse** — a core concept in Linked Lists.
...
Think of **List reverse** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: List reverse
int solve_list_reverse(std::vector<int> v) {
    int result = 0;
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: List reverse
int solve_list_reverse(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: List reverse
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** List reverse — chain of nodes via pointers.
