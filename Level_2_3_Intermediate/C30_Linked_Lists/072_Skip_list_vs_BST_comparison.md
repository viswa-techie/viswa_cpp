# Chapter: Skip list vs BST comparison
...
## 1. Problem Statement
Master the topic: **Skip list vs BST comparison**.
...
This problem exercises **Skip list vs BST comparison** — a core concept in Linked Lists.
...
Think of **Skip list vs BST comparison** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Skip list vs BST comparison
int solve_skip_list_vs_bst_comparison(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_skip_list_vs_bst_comparison(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Skip list vs BST comparison
int solve_skip_list_vs_bst_comparison(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Skip list vs BST comparison
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Skip list vs BST comparison — chain of nodes via pointers.
