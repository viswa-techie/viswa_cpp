...
## 1. Problem Statement
Master the topic: **Interleave two lists**.
...
This problem exercises **Interleave two lists** — a core concept in Linked Lists.
...
Think of **Interleave two lists** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Interleave two lists
int solve_interleave_two_lists(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_interleave_two_lists(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Interleave two lists
int solve_interleave_two_lists(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Interleave two lists
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Interleave two lists — chain of nodes via pointers.
