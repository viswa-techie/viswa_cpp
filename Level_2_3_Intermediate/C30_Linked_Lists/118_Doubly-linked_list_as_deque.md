# Chapter: Doubly-linked list as deque
...
## 1. Problem Statement
Master the topic: **Doubly-linked list as deque**.
...
This problem exercises **Doubly-linked list as deque** — a core concept in Linked Lists.
...
Think of **Doubly-linked list as deque** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Doubly-linked list as deque
int solve_doubly_linked_list_as_deque(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_doubly_linked_list_as_deque(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Doubly-linked list as deque
int solve_doubly_linked_list_as_deque(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Doubly-linked list as deque
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Doubly-linked list as deque — chain of nodes via pointers.
