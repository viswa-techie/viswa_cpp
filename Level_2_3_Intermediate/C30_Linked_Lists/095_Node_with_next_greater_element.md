# Chapter: Node with next greater element
...
## 1. Problem Statement
Master the topic: **Node with next greater element**.
...
This problem exercises **Node with next greater element** — a core concept in Linked Lists.
...
Think of **Node with next greater element** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Node with next greater element
int solve_node_with_next_greater_element(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_node_with_next_greater_element(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Node with next greater element
int solve_node_with_next_greater_element(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Node with next greater element
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Node with next greater element — chain of nodes via pointers.
