# Chapter: Two-pointer approach on lists
...
## 1. Problem Statement
Master the topic: **Two-pointer approach on lists**.
...
This problem exercises **Two-pointer approach on lists** — a core concept in Linked Lists.
...
Think of **Two-pointer approach on lists** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Two-pointer approach on lists
int solve_two_pointer_approach_on_lists(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_two_pointer_approach_on_lists(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Two-pointer approach on lists
int solve_two_pointer_approach_on_lists(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Two-pointer approach on lists
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Two-pointer approach on lists — chain of nodes via pointers.
