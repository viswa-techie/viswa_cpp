# Chapter: Copy list with random pointer
...
## 1. Problem Statement
Master the topic: **Copy list with random pointer**.
...
This problem exercises **Copy list with random pointer** — a core concept in Linked Lists.
...
Think of **Copy list with random pointer** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Copy list with random pointer
int solve_copy_list_with_random_pointer(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_copy_list_with_random_pointer(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Copy list with random pointer
int solve_copy_list_with_random_pointer(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Copy list with random pointer
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Copy list with random pointer — chain of nodes via pointers.
