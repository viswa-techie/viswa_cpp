# Chapter: Dummy head node pattern
...
## 1. Problem Statement
Master the topic: **Dummy head node pattern**.
...
This problem exercises **Dummy head node pattern** — a core concept in Linked Lists.
...
Think of **Dummy head node pattern** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Dummy head node pattern
int solve_dummy_head_node_pattern(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_dummy_head_node_pattern(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Dummy head node pattern
int solve_dummy_head_node_pattern(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Dummy head node pattern
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Dummy head node pattern — chain of nodes via pointers.
