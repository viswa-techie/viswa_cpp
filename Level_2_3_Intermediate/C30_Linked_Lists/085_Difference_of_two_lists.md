# Chapter: Difference of two lists
...
## 1. Problem Statement
Master the topic: **Difference of two lists**.
...
This problem exercises **Difference of two lists** — a core concept in Linked Lists.
...
Think of **Difference of two lists** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Difference of two lists
int solve_difference_of_two_lists(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_difference_of_two_lists(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Difference of two lists
int solve_difference_of_two_lists(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Difference of two lists
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Difference of two lists — chain of nodes via pointers.
