...
## 1. Problem Statement
Master the topic: **Next larger node**.
...
This problem exercises **Next larger node** — a core concept in Linked Lists.
...
Think of **Next larger node** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Next larger node
int solve_next_larger_node(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_next_larger_node(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Next larger node
int solve_next_larger_node(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Next larger node
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Next larger node — chain of nodes via pointers.
