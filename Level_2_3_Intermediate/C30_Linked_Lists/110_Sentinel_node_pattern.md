...
## 1. Problem Statement
Master the topic: **Sentinel node pattern**.
...
This problem exercises **Sentinel node pattern** — a core concept in Linked Lists.
...
Think of **Sentinel node pattern** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Sentinel node pattern
int solve_sentinel_node_pattern(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_sentinel_node_pattern(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Sentinel node pattern
int solve_sentinel_node_pattern(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Sentinel node pattern
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Sentinel node pattern — chain of nodes via pointers.
