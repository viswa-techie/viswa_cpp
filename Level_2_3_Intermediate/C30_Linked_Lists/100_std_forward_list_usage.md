# Chapter: std forward_list usage
...
## 1. Problem Statement
Master the topic: **std forward_list usage**.
...
This problem exercises **std forward_list usage** — a core concept in Linked Lists.
...
Think of **std forward_list usage** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: std forward_list usage
int solve_std_forward_list_usage(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_forward_list_usage(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std forward_list usage
int solve_std_forward_list_usage(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std forward_list usage
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std forward_list usage — chain of nodes via pointers.
