...
## 1. Problem Statement
Master the topic: **Deque from scratch**.
...
This problem exercises **Deque from scratch** — a core concept in Linked Lists.
...
Think of **Deque from scratch** as a chain of train cars, each holding cargo and a coupling to the next.
...
#include <vector>
#include <string>
// Brute-force template for: Deque from scratch
int solve_deque_from_scratch(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_deque_from_scratch(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Deque from scratch
int solve_deque_from_scratch(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Deque from scratch
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Deque from scratch — chain of nodes via pointers.
