# Chapter: Sum of subarray maximums
...
## 1. Problem Statement
Master the topic: **Sum of subarray maximums**.
...
This problem exercises **Sum of subarray maximums** — a core concept in Stacks & Queues.
...
Think of **Sum of subarray maximums** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Sum of subarray maximums
int solve_sum_of_subarray_maximums(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_sum_of_subarray_maximums(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Sum of subarray maximums
int solve_sum_of_subarray_maximums(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Sum of subarray maximums
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Sum of subarray maximums — order-of-arrival/departure matters.
