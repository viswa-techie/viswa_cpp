# Chapter: Design bounded blocking queue
...
## 1. Problem Statement
Master the topic: **Design bounded blocking queue**.
...
This problem exercises **Design bounded blocking queue** — a core concept in Stacks & Queues.
...
Think of **Design bounded blocking queue** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Design bounded blocking queue
int solve_design_bounded_blocking_queue(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_design_bounded_blocking_queue(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Design bounded blocking queue
int solve_design_bounded_blocking_queue(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Design bounded blocking queue
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Design bounded blocking queue — order-of-arrival/departure matters.
