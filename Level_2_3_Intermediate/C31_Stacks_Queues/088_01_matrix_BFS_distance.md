# Chapter: 01 matrix BFS distance
...
## 1. Problem Statement
Master the topic: **01 matrix BFS distance**.
...
This problem exercises **01 matrix BFS distance** — a core concept in Stacks & Queues.
...
Think of **01 matrix BFS distance** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: 01 matrix BFS distance
int solve_01_matrix_bfs_distance(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_01_matrix_bfs_distance(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: 01 matrix BFS distance
int solve_01_matrix_bfs_distance(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: 01 matrix BFS distance
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** 01 matrix BFS distance — order-of-arrival/departure matters.
