...
## 1. Problem Statement
Master the topic: **Jump game BFS variant**.
...
This problem exercises **Jump game BFS variant** — a core concept in Stacks & Queues.
...
Think of **Jump game BFS variant** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Jump game BFS variant
int solve_jump_game_bfs_variant(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_jump_game_bfs_variant(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Jump game BFS variant
int solve_jump_game_bfs_variant(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Jump game BFS variant
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Jump game BFS variant — order-of-arrival/departure matters.
