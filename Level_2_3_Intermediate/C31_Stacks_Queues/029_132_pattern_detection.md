...
## 1. Problem Statement
Master the topic: **132 pattern detection**.
...
This problem exercises **132 pattern detection** — a core concept in Stacks & Queues.
...
Think of **132 pattern detection** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: 132 pattern detection
int solve_132_pattern_detection(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_132_pattern_detection(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: 132 pattern detection
int solve_132_pattern_detection(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: 132 pattern detection
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** 132 pattern detection — order-of-arrival/departure matters.
