# Chapter: Print binary numbers 1 to N
...
## 1. Problem Statement
Master the topic: **Print binary numbers 1 to N**.
...
This problem exercises **Print binary numbers 1 to N** — a core concept in Stacks & Queues.
...
Think of **Print binary numbers 1 to N** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Print binary numbers 1 to N
int solve_print_binary_numbers_1_to_n(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_print_binary_numbers_1_to_n(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Print binary numbers 1 to N
int solve_print_binary_numbers_1_to_n(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Print binary numbers 1 to N
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Print binary numbers 1 to N — order-of-arrival/departure matters.
