# Chapter: Minimum cost to sort with stack
...
## 1. Problem Statement
Master the topic: **Minimum cost to sort with stack**.
...
This problem exercises **Minimum cost to sort with stack** — a core concept in Stacks & Queues.
...
Think of **Minimum cost to sort with stack** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Minimum cost to sort with stack
int solve_minimum_cost_to_sort_with_stack(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_minimum_cost_to_sort_with_stack(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Minimum cost to sort with stack
int solve_minimum_cost_to_sort_with_stack(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Minimum cost to sort with stack
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Minimum cost to sort with stack — order-of-arrival/departure matters.
