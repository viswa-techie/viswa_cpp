...
## 1. Problem Statement
Master the topic: **Circular buffer**.
...
This problem exercises **Circular buffer** — a core concept in Stacks & Queues.
...
Think of **Circular buffer** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Circular buffer
int solve_circular_buffer(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_circular_buffer(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Circular buffer
int solve_circular_buffer(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Circular buffer
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Circular buffer — order-of-arrival/departure matters.
