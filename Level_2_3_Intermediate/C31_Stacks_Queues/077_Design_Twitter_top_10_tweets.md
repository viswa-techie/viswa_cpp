# Chapter: Design Twitter top 10 tweets
...
## 1. Problem Statement
Master the topic: **Design Twitter top 10 tweets**.
...
This problem exercises **Design Twitter top 10 tweets** — a core concept in Stacks & Queues.
...
Think of **Design Twitter top 10 tweets** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Design Twitter top 10 tweets
int solve_design_twitter_top_10_tweets(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_design_twitter_top_10_tweets(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Design Twitter top 10 tweets
int solve_design_twitter_top_10_tweets(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Design Twitter top 10 tweets
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Design Twitter top 10 tweets — order-of-arrival/departure matters.
