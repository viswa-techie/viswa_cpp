...
This problem exercises **0-1 BFS** — a core concept in Stacks & Queues.
...
Think of **0-1 BFS** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: 0-1 BFS
int solve_0_1_bfs(std::vector<int> v) {
    int result = 0;
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: 0-1 BFS
int solve_0_1_bfs(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: 0-1 BFS
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** 0-1 BFS — order-of-arrival/departure matters.
