# Chapter: Minimum steps to reach target
...
## 1. Problem Statement
Master the topic: **Minimum steps to reach target**.
...
This problem exercises **Minimum steps to reach target** — a core concept in Stacks & Queues.
...
Think of **Minimum steps to reach target** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Minimum steps to reach target
int solve_minimum_steps_to_reach_target(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_minimum_steps_to_reach_target(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Minimum steps to reach target
int solve_minimum_steps_to_reach_target(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Minimum steps to reach target
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Minimum steps to reach target — order-of-arrival/departure matters.
