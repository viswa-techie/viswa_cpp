...
## 1. Problem Statement
Master the topic: **Deque using std deque**.
...
This problem exercises **Deque using std deque** — a core concept in Stacks & Queues.
...
Think of **Deque using std deque** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Deque using std deque
int solve_deque_using_std_deque(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_deque_using_std_deque(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Deque using std deque
int solve_deque_using_std_deque(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Deque using std deque
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Deque using std deque — order-of-arrival/departure matters.
