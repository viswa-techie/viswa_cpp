# Chapter: Pacific Atlantic water flow
...
## 1. Problem Statement
Master the topic: **Pacific Atlantic water flow**.
...
This problem exercises **Pacific Atlantic water flow** — a core concept in Stacks & Queues.
...
Think of **Pacific Atlantic water flow** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Pacific Atlantic water flow
int solve_pacific_atlantic_water_flow(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_pacific_atlantic_water_flow(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Pacific Atlantic water flow
int solve_pacific_atlantic_water_flow(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Pacific Atlantic water flow
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Pacific Atlantic water flow — order-of-arrival/departure matters.
