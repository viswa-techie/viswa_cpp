# Chapter: Populating next right pointers
...
## 1. Problem Statement
Master the topic: **Populating next right pointers**.
...
This problem exercises **Populating next right pointers** — a core concept in Stacks & Queues.
...
Think of **Populating next right pointers** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Populating next right pointers
int solve_populating_next_right_pointers(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_populating_next_right_pointers(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Populating next right pointers
int solve_populating_next_right_pointers(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Populating next right pointers
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Populating next right pointers — order-of-arrival/departure matters.
