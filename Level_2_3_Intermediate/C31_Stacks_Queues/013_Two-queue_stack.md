...
## 1. Problem Statement
Master the topic: **Two-queue stack**.
...
This problem exercises **Two-queue stack** — a core concept in Stacks & Queues.
...
Think of **Two-queue stack** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Two-queue stack
int solve_two_queue_stack(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_two_queue_stack(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Two-queue stack
int solve_two_queue_stack(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Two-queue stack
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Two-queue stack — order-of-arrival/departure matters.
