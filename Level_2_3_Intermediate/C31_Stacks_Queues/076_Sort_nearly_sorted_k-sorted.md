# Chapter: Sort nearly sorted k-sorted
...
## 1. Problem Statement
Master the topic: **Sort nearly sorted k-sorted**.
...
This problem exercises **Sort nearly sorted k-sorted** — a core concept in Stacks & Queues.
...
Think of **Sort nearly sorted k-sorted** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Sort nearly sorted k-sorted
int solve_sort_nearly_sorted_k_sorted(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_sort_nearly_sorted_k_sorted(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Sort nearly sorted k-sorted
int solve_sort_nearly_sorted_k_sorted(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Sort nearly sorted k-sorted
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Sort nearly sorted k-sorted — order-of-arrival/departure matters.
