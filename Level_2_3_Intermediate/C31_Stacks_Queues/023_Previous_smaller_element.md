# Chapter: Previous smaller element
...
## 1. Problem Statement
Master the topic: **Previous smaller element**.
...
This problem exercises **Previous smaller element** — a core concept in Stacks & Queues.
...
Think of **Previous smaller element** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Previous smaller element
int solve_previous_smaller_element(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_previous_smaller_element(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Previous smaller element
int solve_previous_smaller_element(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Previous smaller element
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Previous smaller element — order-of-arrival/departure matters.
