...
## 1. Problem Statement
Master the topic: **Word ladder I**.
...
This problem exercises **Word ladder I** — a core concept in Stacks & Queues.
...
Think of **Word ladder I** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Word ladder I
int solve_word_ladder_i(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_word_ladder_i(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Word ladder I
int solve_word_ladder_i(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Word ladder I
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Word ladder I — order-of-arrival/departure matters.
