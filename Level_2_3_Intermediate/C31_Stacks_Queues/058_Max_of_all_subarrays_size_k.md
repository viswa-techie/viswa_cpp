# Chapter: Max of all subarrays size k
...
## 1. Problem Statement
Master the topic: **Max of all subarrays size k**.
...
This problem exercises **Max of all subarrays size k** — a core concept in Stacks & Queues.
...
Think of **Max of all subarrays size k** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Max of all subarrays size k
int solve_max_of_all_subarrays_size_k(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_max_of_all_subarrays_size_k(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Max of all subarrays size k
int solve_max_of_all_subarrays_size_k(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Max of all subarrays size k
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Max of all subarrays size k — order-of-arrival/departure matters.
