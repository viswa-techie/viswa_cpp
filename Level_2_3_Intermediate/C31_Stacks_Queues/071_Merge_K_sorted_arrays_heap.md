# Chapter: Merge K sorted arrays heap
...
## 1. Problem Statement
Master the topic: **Merge K sorted arrays heap**.
...
This problem exercises **Merge K sorted arrays heap** — a core concept in Stacks & Queues.
...
Think of **Merge K sorted arrays heap** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Merge K sorted arrays heap
int solve_merge_k_sorted_arrays_heap(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_merge_k_sorted_arrays_heap(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Merge K sorted arrays heap
int solve_merge_k_sorted_arrays_heap(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Merge K sorted arrays heap
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Merge K sorted arrays heap — order-of-arrival/departure matters.
