# Chapter: Shortest path binary matrix
...
## 1. Problem Statement
Master the topic: **Shortest path binary matrix**.
...
This problem exercises **Shortest path binary matrix** — a core concept in Stacks & Queues.
...
Think of **Shortest path binary matrix** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Shortest path binary matrix
int solve_shortest_path_binary_matrix(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_shortest_path_binary_matrix(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Shortest path binary matrix
int solve_shortest_path_binary_matrix(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Shortest path binary matrix
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Shortest path binary matrix — order-of-arrival/departure matters.
