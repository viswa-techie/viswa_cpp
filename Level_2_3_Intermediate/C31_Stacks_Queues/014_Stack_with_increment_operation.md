# Chapter: Stack with increment operation
...
## 1. Problem Statement
Master the topic: **Stack with increment operation**.
...
This problem exercises **Stack with increment operation** — a core concept in Stacks & Queues.
...
Think of **Stack with increment operation** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Stack with increment operation
int solve_stack_with_increment_operation(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_stack_with_increment_operation(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Stack with increment operation
int solve_stack_with_increment_operation(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Stack with increment operation
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Stack with increment operation — order-of-arrival/departure matters.
