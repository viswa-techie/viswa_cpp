# Chapter: Snakes and ladders BFS
...
## 1. Problem Statement
Master the topic: **Snakes and ladders BFS**.
...
This problem exercises **Snakes and ladders BFS** — a core concept in Stacks & Queues.
...
Think of **Snakes and ladders BFS** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Snakes and ladders BFS
int solve_snakes_and_ladders_bfs(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_snakes_and_ladders_bfs(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Snakes and ladders BFS
int solve_snakes_and_ladders_bfs(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Snakes and ladders BFS
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Snakes and ladders BFS — order-of-arrival/departure matters.
