...
## 1. Problem Statement
Master the topic: **Top K frequent words**.
...
This problem exercises **Top K frequent words** — a core concept in Stacks & Queues.
...
Think of **Top K frequent words** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Top K frequent words
int solve_top_k_frequent_words(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_top_k_frequent_words(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Top K frequent words
int solve_top_k_frequent_words(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Top K frequent words
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Top K frequent words — order-of-arrival/departure matters.
