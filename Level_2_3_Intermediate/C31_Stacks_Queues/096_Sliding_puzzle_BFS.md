...
## 1. Problem Statement
Master the topic: **Sliding puzzle BFS**.
...
This problem exercises **Sliding puzzle BFS** — a core concept in Stacks & Queues.
...
Think of **Sliding puzzle BFS** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Sliding puzzle BFS
int solve_sliding_puzzle_bfs(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_sliding_puzzle_bfs(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Sliding puzzle BFS
int solve_sliding_puzzle_bfs(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Sliding puzzle BFS
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Sliding puzzle BFS — order-of-arrival/departure matters.
