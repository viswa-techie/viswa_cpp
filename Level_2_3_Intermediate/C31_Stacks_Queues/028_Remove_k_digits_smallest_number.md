# Chapter: Remove k digits smallest number
...
## 1. Problem Statement
Master the topic: **Remove k digits smallest number**.
...
This problem exercises **Remove k digits smallest number** — a core concept in Stacks & Queues.
...
Think of **Remove k digits smallest number** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Remove k digits smallest number
int solve_remove_k_digits_smallest_number(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_remove_k_digits_smallest_number(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Remove k digits smallest number
int solve_remove_k_digits_smallest_number(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Remove k digits smallest number
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Remove k digits smallest number — order-of-arrival/departure matters.
