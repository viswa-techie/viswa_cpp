# Chapter: Tower of Hanoi with explicit stack
...
## 1. Problem Statement
Master the topic: **Tower of Hanoi with explicit stack**.
...
This problem exercises **Tower of Hanoi with explicit stack** — a core concept in Stacks & Queues.
...
Think of **Tower of Hanoi with explicit stack** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Tower of Hanoi with explicit stack
int solve_tower_of_hanoi_with_explicit_stack(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_tower_of_hanoi_with_explicit_stack(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Tower of Hanoi with explicit stack
int solve_tower_of_hanoi_with_explicit_stack(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Tower of Hanoi with explicit stack
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Tower of Hanoi with explicit stack — order-of-arrival/departure matters.
