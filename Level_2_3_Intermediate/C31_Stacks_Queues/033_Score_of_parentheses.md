...
## 1. Problem Statement
Master the topic: **Score of parentheses**.
...
This problem exercises **Score of parentheses** — a core concept in Stacks & Queues.
...
Think of **Score of parentheses** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Score of parentheses
int solve_score_of_parentheses(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_score_of_parentheses(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Score of parentheses
int solve_score_of_parentheses(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Score of parentheses
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Score of parentheses — order-of-arrival/departure matters.
