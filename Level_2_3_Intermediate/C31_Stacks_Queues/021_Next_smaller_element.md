...
## 1. Problem Statement
Master the topic: **Next smaller element**.
...
This problem exercises **Next smaller element** — a core concept in Stacks & Queues.
...
Think of **Next smaller element** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Next smaller element
int solve_next_smaller_element(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_next_smaller_element(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Next smaller element
int solve_next_smaller_element(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Next smaller element
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Next smaller element — order-of-arrival/departure matters.
