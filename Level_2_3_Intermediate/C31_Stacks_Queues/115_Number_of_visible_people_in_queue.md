# Chapter: Number of visible people in queue
...
## 1. Problem Statement
Master the topic: **Number of visible people in queue**.
...
This problem exercises **Number of visible people in queue** — a core concept in Stacks & Queues.
...
Think of **Number of visible people in queue** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Number of visible people in queue
int solve_number_of_visible_people_in_queue(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_number_of_visible_people_in_queue(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Number of visible people in queue
int solve_number_of_visible_people_in_queue(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Number of visible people in queue
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Number of visible people in queue — order-of-arrival/departure matters.
