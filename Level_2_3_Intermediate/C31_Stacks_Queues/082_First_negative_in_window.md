# Chapter: First negative in window
...
## 1. Problem Statement
Master the topic: **First negative in window**.
...
This problem exercises **First negative in window** — a core concept in Stacks & Queues.
...
Think of **First negative in window** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: First negative in window
int solve_first_negative_in_window(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_first_negative_in_window(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: First negative in window
int solve_first_negative_in_window(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: First negative in window
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** First negative in window — order-of-arrival/departure matters.
