...
## 1. Problem Statement
Master the topic: **Infix to postfix conversion**.
...
This problem exercises **Infix to postfix conversion** — a core concept in Stacks & Queues.
...
Think of **Infix to postfix conversion** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Infix to postfix conversion
int solve_infix_to_postfix_conversion(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_infix_to_postfix_conversion(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Infix to postfix conversion
int solve_infix_to_postfix_conversion(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Infix to postfix conversion
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Infix to postfix conversion — order-of-arrival/departure matters.
