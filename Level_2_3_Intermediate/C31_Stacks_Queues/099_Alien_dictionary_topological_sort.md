# Chapter: Alien dictionary topological sort
...
## 1. Problem Statement
Master the topic: **Alien dictionary topological sort**.
...
This problem exercises **Alien dictionary topological sort** — a core concept in Stacks & Queues.
...
Think of **Alien dictionary topological sort** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Alien dictionary topological sort
int solve_alien_dictionary_topological_sort(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_alien_dictionary_topological_sort(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Alien dictionary topological sort
int solve_alien_dictionary_topological_sort(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Alien dictionary topological sort
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Alien dictionary topological sort — order-of-arrival/departure matters.
