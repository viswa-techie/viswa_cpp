...
## 1. Problem Statement
Master the topic: **Level averages**.
...
This problem exercises **Level averages** — a core concept in Stacks & Queues.
...
Think of **Level averages** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Level averages
int solve_level_averages(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_level_averages(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Level averages
int solve_level_averages(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Level averages
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Level averages — order-of-arrival/departure matters.
