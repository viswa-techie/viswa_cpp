...
## 1. Problem Statement
Master the topic: **Multi-source BFS**.
...
This problem exercises **Multi-source BFS** — a core concept in Stacks & Queues.
...
Think of **Multi-source BFS** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Multi-source BFS
int solve_multi_source_bfs(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_multi_source_bfs(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Multi-source BFS
int solve_multi_source_bfs(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Multi-source BFS
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Multi-source BFS — order-of-arrival/departure matters.
