...
## 1. Problem Statement
Master the topic: **Asteroid collision**.
...
This problem exercises **Asteroid collision** — a core concept in Stacks & Queues.
...
Think of **Asteroid collision** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Asteroid collision
int solve_asteroid_collision(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_asteroid_collision(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Asteroid collision
int solve_asteroid_collision(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Asteroid collision
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Asteroid collision — order-of-arrival/departure matters.
