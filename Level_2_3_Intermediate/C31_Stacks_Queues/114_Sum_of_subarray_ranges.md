# Chapter: Sum of subarray ranges
...
## 1. Problem Statement
Master the topic: **Sum of subarray ranges**.
...
This problem exercises **Sum of subarray ranges** — a core concept in Stacks & Queues.
...
Think of **Sum of subarray ranges** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Sum of subarray ranges
int solve_sum_of_subarray_ranges(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_sum_of_subarray_ranges(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Sum of subarray ranges
int solve_sum_of_subarray_ranges(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Sum of subarray ranges
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Sum of subarray ranges — order-of-arrival/departure matters.
