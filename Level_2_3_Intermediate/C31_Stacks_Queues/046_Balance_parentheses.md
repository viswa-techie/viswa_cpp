...
## 1. Problem Statement
Master the topic: **Balance parentheses**.
...
This problem exercises **Balance parentheses** — a core concept in Stacks & Queues.
...
Think of **Balance parentheses** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Balance parentheses
int solve_balance_parentheses(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_balance_parentheses(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Balance parentheses
int solve_balance_parentheses(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Balance parentheses
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Balance parentheses — order-of-arrival/departure matters.
