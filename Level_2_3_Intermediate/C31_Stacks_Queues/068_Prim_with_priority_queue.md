# Chapter: Prim with priority queue
...
## 1. Problem Statement
Master the topic: **Prim with priority queue**.
...
This problem exercises **Prim with priority queue** — a core concept in Stacks & Queues.
...
Think of **Prim with priority queue** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Prim with priority queue
int solve_prim_with_priority_queue(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_prim_with_priority_queue(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Prim with priority queue
int solve_prim_with_priority_queue(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Prim with priority queue
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Prim with priority queue — order-of-arrival/departure matters.
