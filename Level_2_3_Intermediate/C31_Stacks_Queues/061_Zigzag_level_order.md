...
## 1. Problem Statement
Master the topic: **Zigzag level order**.
...
This problem exercises **Zigzag level order** — a core concept in Stacks & Queues.
...
Think of **Zigzag level order** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Zigzag level order
int solve_zigzag_level_order(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_zigzag_level_order(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Zigzag level order
int solve_zigzag_level_order(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Zigzag level order
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Zigzag level order — order-of-arrival/departure matters.
