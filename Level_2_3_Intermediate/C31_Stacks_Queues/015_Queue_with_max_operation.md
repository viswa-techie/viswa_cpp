# Chapter: Queue with max operation
...
## 1. Problem Statement
Master the topic: **Queue with max operation**.
...
This problem exercises **Queue with max operation** — a core concept in Stacks & Queues.
...
Think of **Queue with max operation** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Queue with max operation
int solve_queue_with_max_operation(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_queue_with_max_operation(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Queue with max operation
int solve_queue_with_max_operation(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Queue with max operation
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Queue with max operation — order-of-arrival/departure matters.
