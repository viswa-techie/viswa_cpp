# Chapter: Previous greater element
...
## 1. Problem Statement
Master the topic: **Previous greater element**.
...
This problem exercises **Previous greater element** — a core concept in Stacks & Queues.
...
Think of **Previous greater element** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Previous greater element
int solve_previous_greater_element(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_previous_greater_element(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Previous greater element
int solve_previous_greater_element(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Previous greater element
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Previous greater element — order-of-arrival/departure matters.
