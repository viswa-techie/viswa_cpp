# Chapter: Decode string k bracket str
...
## 1. Problem Statement
Master the topic: **Decode string k bracket str**.
...
This problem exercises **Decode string k bracket str** — a core concept in Stacks & Queues.
...
Think of **Decode string k bracket str** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Decode string k bracket str
int solve_decode_string_k_bracket_str(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_decode_string_k_bracket_str(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Decode string k bracket str
int solve_decode_string_k_bracket_str(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Decode string k bracket str
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Decode string k bracket str — order-of-arrival/departure matters.
