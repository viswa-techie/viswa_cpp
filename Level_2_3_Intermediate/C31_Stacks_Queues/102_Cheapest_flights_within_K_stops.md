# Chapter: Cheapest flights within K stops
...
## 1. Problem Statement
Master the topic: **Cheapest flights within K stops**.
...
This problem exercises **Cheapest flights within K stops** — a core concept in Stacks & Queues.
...
Think of **Cheapest flights within K stops** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Cheapest flights within K stops
int solve_cheapest_flights_within_k_stops(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_cheapest_flights_within_k_stops(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Cheapest flights within K stops
int solve_cheapest_flights_within_k_stops(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Cheapest flights within K stops
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Cheapest flights within K stops — order-of-arrival/departure matters.
