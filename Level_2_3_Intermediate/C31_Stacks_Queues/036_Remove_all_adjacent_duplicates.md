# Chapter: Remove all adjacent duplicates
...
## 1. Problem Statement
Master the topic: **Remove all adjacent duplicates**.
...
This problem exercises **Remove all adjacent duplicates** — a core concept in Stacks & Queues.
...
Think of **Remove all adjacent duplicates** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Remove all adjacent duplicates
int solve_remove_all_adjacent_duplicates(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_remove_all_adjacent_duplicates(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Remove all adjacent duplicates
int solve_remove_all_adjacent_duplicates(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Remove all adjacent duplicates
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Remove all adjacent duplicates — order-of-arrival/departure matters.
