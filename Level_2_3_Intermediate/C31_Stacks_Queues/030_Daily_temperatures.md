...
## 1. Problem Statement
Master the topic: **Daily temperatures**.
...
This problem exercises **Daily temperatures** — a core concept in Stacks & Queues.
...
Think of **Daily temperatures** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Daily temperatures
int solve_daily_temperatures(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_daily_temperatures(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Daily temperatures
int solve_daily_temperatures(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Daily temperatures
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Daily temperatures — order-of-arrival/departure matters.
