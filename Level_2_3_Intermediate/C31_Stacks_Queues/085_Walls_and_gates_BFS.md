...
## 1. Problem Statement
Master the topic: **Walls and gates BFS**.
...
This problem exercises **Walls and gates BFS** — a core concept in Stacks & Queues.
...
Think of **Walls and gates BFS** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Walls and gates BFS
int solve_walls_and_gates_bfs(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_walls_and_gates_bfs(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Walls and gates BFS
int solve_walls_and_gates_bfs(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Walls and gates BFS
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Walls and gates BFS — order-of-arrival/departure matters.
