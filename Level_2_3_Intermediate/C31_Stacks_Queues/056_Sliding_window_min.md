...
## 1. Problem Statement
Master the topic: **Sliding window min**.
...
This problem exercises **Sliding window min** — a core concept in Stacks & Queues.
...
Think of **Sliding window min** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Sliding window min
int solve_sliding_window_min(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_sliding_window_min(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Sliding window min
int solve_sliding_window_min(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Sliding window min
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Sliding window min — order-of-arrival/departure matters.
