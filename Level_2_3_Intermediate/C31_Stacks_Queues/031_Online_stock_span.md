...
## 1. Problem Statement
Master the topic: **Online stock span**.
...
This problem exercises **Online stock span** — a core concept in Stacks & Queues.
...
Think of **Online stock span** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Online stock span
int solve_online_stock_span(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_online_stock_span(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Online stock span
int solve_online_stock_span(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Online stock span
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Online stock span — order-of-arrival/departure matters.
