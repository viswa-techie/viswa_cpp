# Chapter: Word ladder II all shortest paths
...
## 1. Problem Statement
Master the topic: **Word ladder II all shortest paths**.
...
This problem exercises **Word ladder II all shortest paths** — a core concept in Stacks & Queues.
...
Think of **Word ladder II all shortest paths** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Word ladder II all shortest paths
int solve_word_ladder_ii_all_shortest_paths(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_word_ladder_ii_all_shortest_paths(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Word ladder II all shortest paths
int solve_word_ladder_ii_all_shortest_paths(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Word ladder II all shortest paths
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Word ladder II all shortest paths — order-of-arrival/departure matters.
