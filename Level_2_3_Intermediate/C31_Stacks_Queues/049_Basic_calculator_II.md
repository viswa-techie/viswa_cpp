...
## 1. Problem Statement
Master the topic: **Basic calculator II**.
...
This problem exercises **Basic calculator II** — a core concept in Stacks & Queues.
...
Think of **Basic calculator II** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Basic calculator II
int solve_basic_calculator_ii(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_basic_calculator_ii(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Basic calculator II
int solve_basic_calculator_ii(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Basic calculator II
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Basic calculator II — order-of-arrival/departure matters.
