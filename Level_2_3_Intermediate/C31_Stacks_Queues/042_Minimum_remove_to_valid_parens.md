# Chapter: Minimum remove to valid parens
...
## 1. Problem Statement
Master the topic: **Minimum remove to valid parens**.
...
This problem exercises **Minimum remove to valid parens** — a core concept in Stacks & Queues.
...
Think of **Minimum remove to valid parens** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Minimum remove to valid parens
int solve_minimum_remove_to_valid_parens(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_minimum_remove_to_valid_parens(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Minimum remove to valid parens
int solve_minimum_remove_to_valid_parens(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Minimum remove to valid parens
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Minimum remove to valid parens — order-of-arrival/departure matters.
