# Chapter: Ring buffer for streaming data
...
## 1. Problem Statement
Master the topic: **Ring buffer for streaming data**.
...
This problem exercises **Ring buffer for streaming data** — a core concept in Stacks & Queues.
...
Think of **Ring buffer for streaming data** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Ring buffer for streaming data
int solve_ring_buffer_for_streaming_data(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_ring_buffer_for_streaming_data(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Ring buffer for streaming data
int solve_ring_buffer_for_streaming_data(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Ring buffer for streaming data
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Ring buffer for streaming data — order-of-arrival/departure matters.
