...
## 1. Problem Statement
Master the topic: **Rotten oranges BFS**.
...
This problem exercises **Rotten oranges BFS** — a core concept in Stacks & Queues.
...
Think of **Rotten oranges BFS** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Rotten oranges BFS
int solve_rotten_oranges_bfs(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_rotten_oranges_bfs(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Rotten oranges BFS
int solve_rotten_oranges_bfs(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Rotten oranges BFS
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Rotten oranges BFS — order-of-arrival/departure matters.
