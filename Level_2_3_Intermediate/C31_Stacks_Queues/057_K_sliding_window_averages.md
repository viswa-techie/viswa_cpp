# Chapter: K sliding window averages
...
## 1. Problem Statement
Master the topic: **K sliding window averages**.
...
This problem exercises **K sliding window averages** — a core concept in Stacks & Queues.
...
Think of **K sliding window averages** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: K sliding window averages
int solve_k_sliding_window_averages(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_k_sliding_window_averages(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: K sliding window averages
int solve_k_sliding_window_averages(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: K sliding window averages
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** K sliding window averages — order-of-arrival/departure matters.
