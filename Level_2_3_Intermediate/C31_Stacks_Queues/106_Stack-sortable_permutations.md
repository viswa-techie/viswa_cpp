# Chapter: Stack-sortable permutations
...
## 1. Problem Statement
Master the topic: **Stack-sortable permutations**.
...
This problem exercises **Stack-sortable permutations** — a core concept in Stacks & Queues.
...
Think of **Stack-sortable permutations** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Stack-sortable permutations
int solve_stack_sortable_permutations(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_stack_sortable_permutations(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Stack-sortable permutations
int solve_stack_sortable_permutations(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Stack-sortable permutations
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Stack-sortable permutations — order-of-arrival/departure matters.
