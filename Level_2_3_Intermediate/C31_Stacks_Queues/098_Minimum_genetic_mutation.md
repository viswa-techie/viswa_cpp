# Chapter: Minimum genetic mutation
...
## 1. Problem Statement
Master the topic: **Minimum genetic mutation**.
...
This problem exercises **Minimum genetic mutation** — a core concept in Stacks & Queues.
...
Think of **Minimum genetic mutation** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Minimum genetic mutation
int solve_minimum_genetic_mutation(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_minimum_genetic_mutation(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Minimum genetic mutation
int solve_minimum_genetic_mutation(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Minimum genetic mutation
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Minimum genetic mutation — order-of-arrival/departure matters.
