...
## 1. Problem Statement
Master the topic: **Sliding window median**.
...
This problem exercises **Sliding window median** — a core concept in Stacks & Queues.
...
Think of **Sliding window median** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Sliding window median
int solve_sliding_window_median(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_sliding_window_median(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Sliding window median
int solve_sliding_window_median(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Sliding window median
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Sliding window median — order-of-arrival/departure matters.
