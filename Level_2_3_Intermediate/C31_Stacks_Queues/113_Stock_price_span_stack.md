# Chapter: Stock price span stack
...
## 1. Problem Statement
Master the topic: **Stock price span stack**.
...
This problem exercises **Stock price span stack** — a core concept in Stacks & Queues.
...
Think of **Stock price span stack** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Stock price span stack
int solve_stock_price_span_stack(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_stock_price_span_stack(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Stock price span stack
int solve_stock_price_span_stack(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Stock price span stack
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Stock price span stack — order-of-arrival/departure matters.
