# Chapter: Priority queue with lazy deletion
...
## 1. Problem Statement
Master the topic: **Priority queue with lazy deletion**.
...
This problem exercises **Priority queue with lazy deletion** — a core concept in Stacks & Queues.
...
Think of **Priority queue with lazy deletion** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Priority queue with lazy deletion
int solve_priority_queue_with_lazy_deletion(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_priority_queue_with_lazy_deletion(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Priority queue with lazy deletion
int solve_priority_queue_with_lazy_deletion(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Priority queue with lazy deletion
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Priority queue with lazy deletion — order-of-arrival/departure matters.
