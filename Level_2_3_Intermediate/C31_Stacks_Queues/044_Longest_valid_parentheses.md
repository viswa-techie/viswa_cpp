# Chapter: Longest valid parentheses
...
## 1. Problem Statement
Master the topic: **Longest valid parentheses**.
...
This problem exercises **Longest valid parentheses** — a core concept in Stacks & Queues.
...
Think of **Longest valid parentheses** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Longest valid parentheses
int solve_longest_valid_parentheses(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_longest_valid_parentheses(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Longest valid parentheses
int solve_longest_valid_parentheses(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Longest valid parentheses
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Longest valid parentheses — order-of-arrival/departure matters.
