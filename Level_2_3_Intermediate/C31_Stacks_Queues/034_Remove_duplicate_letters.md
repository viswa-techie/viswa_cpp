# Chapter: Remove duplicate letters
...
## 1. Problem Statement
Master the topic: **Remove duplicate letters**.
...
This problem exercises **Remove duplicate letters** — a core concept in Stacks & Queues.
...
Think of **Remove duplicate letters** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Remove duplicate letters
int solve_remove_duplicate_letters(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_remove_duplicate_letters(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Remove duplicate letters
int solve_remove_duplicate_letters(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Remove duplicate letters
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Remove duplicate letters — order-of-arrival/departure matters.
