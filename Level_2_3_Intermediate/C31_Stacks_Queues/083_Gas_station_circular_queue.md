# Chapter: Gas station circular queue
...
## 1. Problem Statement
Master the topic: **Gas station circular queue**.
...
This problem exercises **Gas station circular queue** — a core concept in Stacks & Queues.
...
Think of **Gas station circular queue** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Gas station circular queue
int solve_gas_station_circular_queue(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_gas_station_circular_queue(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Gas station circular queue
int solve_gas_station_circular_queue(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Gas station circular queue
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Gas station circular queue — order-of-arrival/departure matters.
