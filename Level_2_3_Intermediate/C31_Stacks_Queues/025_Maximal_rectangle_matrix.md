# Chapter: Maximal rectangle matrix
...
## 1. Problem Statement
Master the topic: **Maximal rectangle matrix**.
...
This problem exercises **Maximal rectangle matrix** — a core concept in Stacks & Queues.
...
Think of **Maximal rectangle matrix** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Maximal rectangle matrix
int solve_maximal_rectangle_matrix(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_maximal_rectangle_matrix(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Maximal rectangle matrix
int solve_maximal_rectangle_matrix(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Maximal rectangle matrix
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Maximal rectangle matrix — order-of-arrival/departure matters.
