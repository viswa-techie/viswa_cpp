# Chapter: Sum of subarray minimums
...
## 1. Problem Statement
Master the topic: **Sum of subarray minimums**.
...
This problem exercises **Sum of subarray minimums** — a core concept in Stacks & Queues.
...
Think of **Sum of subarray minimums** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Sum of subarray minimums
int solve_sum_of_subarray_minimums(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_sum_of_subarray_minimums(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Sum of subarray minimums
int solve_sum_of_subarray_minimums(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Sum of subarray minimums
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Sum of subarray minimums — order-of-arrival/departure matters.
