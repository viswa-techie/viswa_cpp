# Chapter: Minimum add to make valid
...
## 1. Problem Statement
Master the topic: **Minimum add to make valid**.
...
This problem exercises **Minimum add to make valid** — a core concept in Stacks & Queues.
...
Think of **Minimum add to make valid** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Minimum add to make valid
int solve_minimum_add_to_make_valid(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_minimum_add_to_make_valid(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Minimum add to make valid
int solve_minimum_add_to_make_valid(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Minimum add to make valid
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Minimum add to make valid — order-of-arrival/departure matters.
