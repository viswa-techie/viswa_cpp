# Chapter: Expression tree construction
...
## 1. Problem Statement
Master the topic: **Expression tree construction**.
...
This problem exercises **Expression tree construction** — a core concept in Stacks & Queues.
...
Think of **Expression tree construction** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Expression tree construction
int solve_expression_tree_construction(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_expression_tree_construction(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Expression tree construction
int solve_expression_tree_construction(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Expression tree construction
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Expression tree construction — order-of-arrival/departure matters.
