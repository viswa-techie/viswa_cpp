...
## 1. Problem Statement
Master the topic: **Task scheduler queue**.
...
This problem exercises **Task scheduler queue** — a core concept in Stacks & Queues.
...
Think of **Task scheduler queue** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Task scheduler queue
int solve_task_scheduler_queue(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_task_scheduler_queue(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Task scheduler queue
int solve_task_scheduler_queue(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Task scheduler queue
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Task scheduler queue — order-of-arrival/departure matters.
