...
## 1. Problem Statement
Master the topic: **BFS level order tree**.
...
This problem exercises **BFS level order tree** — a core concept in Stacks & Queues.
...
Think of **BFS level order tree** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: BFS level order tree
int solve_bfs_level_order_tree(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_bfs_level_order_tree(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: BFS level order tree
int solve_bfs_level_order_tree(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: BFS level order tree
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** BFS level order tree — order-of-arrival/departure matters.
