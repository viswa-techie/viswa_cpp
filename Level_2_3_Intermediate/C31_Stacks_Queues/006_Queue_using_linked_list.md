# Chapter: Queue using linked list
...
## 1. Problem Statement
Master the topic: **Queue using linked list**.
...
This problem exercises **Queue using linked list** — a core concept in Stacks & Queues.
...
Think of **Queue using linked list** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Queue using linked list
int solve_queue_using_linked_list(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_queue_using_linked_list(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Queue using linked list
int solve_queue_using_linked_list(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Queue using linked list
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Queue using linked list — order-of-arrival/departure matters.
