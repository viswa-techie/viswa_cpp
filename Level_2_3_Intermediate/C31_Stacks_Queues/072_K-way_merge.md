...
This problem exercises **K-way merge** — a core concept in Stacks & Queues.
...
Think of **K-way merge** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: K-way merge
int solve_k_way_merge(std::vector<int> v) {
    int result = 0;
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: K-way merge
int solve_k_way_merge(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: K-way merge
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** K-way merge — order-of-arrival/departure matters.
