...
This problem exercises **Max-stack** — a core concept in Stacks & Queues.
...
Think of **Max-stack** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Max-stack
int solve_max_stack(std::vector<int> v) {
    int result = 0;
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Max-stack
int solve_max_stack(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Max-stack
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Max-stack — order-of-arrival/departure matters.
