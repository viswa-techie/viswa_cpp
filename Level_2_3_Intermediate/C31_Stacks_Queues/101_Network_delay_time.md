...
## 1. Problem Statement
Master the topic: **Network delay time**.
...
This problem exercises **Network delay time** — a core concept in Stacks & Queues.
...
Think of **Network delay time** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Network delay time
int solve_network_delay_time(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_network_delay_time(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Network delay time
int solve_network_delay_time(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Network delay time
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Network delay time — order-of-arrival/departure matters.
