# Chapter: Stack using std vector
...
## 1. Problem Statement
Master the topic: **Stack using std vector**.
...
This problem exercises **Stack using std vector** — a core concept in Stacks & Queues.
...
Think of **Stack using std vector** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Stack using std vector
int solve_stack_using_std_vector(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_stack_using_std_vector(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Stack using std vector
int solve_stack_using_std_vector(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Stack using std vector
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Stack using std vector — order-of-arrival/departure matters.
