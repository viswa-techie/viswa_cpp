...
## 1. Problem Statement
Master the topic: **Course schedule BFS**.
...
This problem exercises **Course schedule BFS** — a core concept in Stacks & Queues.
...
Think of **Course schedule BFS** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Course schedule BFS
int solve_course_schedule_bfs(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_course_schedule_bfs(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Course schedule BFS
int solve_course_schedule_bfs(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Course schedule BFS
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Course schedule BFS — order-of-arrival/departure matters.
