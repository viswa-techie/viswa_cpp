...
## 1. Problem Statement
Master the topic: **Circular queue design**.
...
This problem exercises **Circular queue design** — a core concept in Stacks & Queues.
...
Think of **Circular queue design** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Circular queue design
int solve_circular_queue_design(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_circular_queue_design(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Circular queue design
int solve_circular_queue_design(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Circular queue design
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Circular queue design — order-of-arrival/departure matters.
