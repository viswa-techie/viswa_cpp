...
## 1. Problem Statement
Master the topic: **Number of islands BFS**.
...
This problem exercises **Number of islands BFS** — a core concept in Stacks & Queues.
...
Think of **Number of islands BFS** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Number of islands BFS
int solve_number_of_islands_bfs(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_number_of_islands_bfs(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Number of islands BFS
int solve_number_of_islands_bfs(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Number of islands BFS
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Number of islands BFS — order-of-arrival/departure matters.
