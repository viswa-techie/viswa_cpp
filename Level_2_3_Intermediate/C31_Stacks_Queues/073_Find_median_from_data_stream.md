...
This problem exercises **Find median from data stream** — a core concept in Stacks & Queues.
...
Think of **Find median from data stream** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Find median from data stream
int solve_find_median_from_data_stream(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_find_median_from_data_stream(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Find median from data stream
int solve_find_median_from_data_stream(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Find median from data stream
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Find median from data stream — order-of-arrival/departure matters.
