# Chapter: Queue using array circular
...
## 1. Problem Statement
Master the topic: **Queue using array circular**.
...
This problem exercises **Queue using array circular** — a core concept in Stacks & Queues.
...
Think of **Queue using array circular** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Queue using array circular
int solve_queue_using_array_circular(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_queue_using_array_circular(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Queue using array circular
int solve_queue_using_array_circular(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Queue using array circular
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Queue using array circular — order-of-arrival/departure matters.
