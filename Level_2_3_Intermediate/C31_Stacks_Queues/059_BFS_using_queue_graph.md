...
## 1. Problem Statement
Master the topic: **BFS using queue graph**.
...
This problem exercises **BFS using queue graph** — a core concept in Stacks & Queues.
...
Think of **BFS using queue graph** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: BFS using queue graph
int solve_bfs_using_queue_graph(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_bfs_using_queue_graph(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: BFS using queue graph
int solve_bfs_using_queue_graph(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: BFS using queue graph
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** BFS using queue graph — order-of-arrival/departure matters.
