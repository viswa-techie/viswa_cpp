# Chapter: Check redundant brackets
...
## 1. Problem Statement
Master the topic: **Check redundant brackets**.
...
This problem exercises **Check redundant brackets** — a core concept in Stacks & Queues.
...
Think of **Check redundant brackets** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Check redundant brackets
int solve_check_redundant_brackets(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_check_redundant_brackets(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Check redundant brackets
int solve_check_redundant_brackets(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Check redundant brackets
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Check redundant brackets — order-of-arrival/departure matters.
