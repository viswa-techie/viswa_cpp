...
## 1. Problem Statement
Master the topic: **Two-stack queue**.
...
This problem exercises **Two-stack queue** — a core concept in Stacks & Queues.
...
Think of **Two-stack queue** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Two-stack queue
int solve_two_stack_queue(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_two_stack_queue(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Two-stack queue
int solve_two_stack_queue(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Two-stack queue
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Two-stack queue — order-of-arrival/departure matters.
