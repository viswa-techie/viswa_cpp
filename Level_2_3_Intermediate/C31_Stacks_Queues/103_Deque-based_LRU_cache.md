...
## 1. Problem Statement
Master the topic: **Deque-based LRU cache**.
...
This problem exercises **Deque-based LRU cache** — a core concept in Stacks & Queues.
...
Think of **Deque-based LRU cache** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Deque-based LRU cache
int solve_deque_based_lru_cache(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_deque_based_lru_cache(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Deque-based LRU cache
int solve_deque_based_lru_cache(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Deque-based LRU cache
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Deque-based LRU cache — order-of-arrival/departure matters.
