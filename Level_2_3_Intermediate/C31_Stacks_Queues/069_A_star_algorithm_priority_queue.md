# Chapter: A star algorithm priority queue
...
## 1. Problem Statement
Master the topic: **A star algorithm priority queue**.
...
This problem exercises **A star algorithm priority queue** — a core concept in Stacks & Queues.
...
Think of **A star algorithm priority queue** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: A star algorithm priority queue
int solve_a_star_algorithm_priority_queue(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_a_star_algorithm_priority_queue(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: A star algorithm priority queue
int solve_a_star_algorithm_priority_queue(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: A star algorithm priority queue
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** A star algorithm priority queue — order-of-arrival/departure matters.
