...
## 1. Problem Statement
Master the topic: **Open the lock BFS**.
...
This problem exercises **Open the lock BFS** — a core concept in Stacks & Queues.
...
Think of **Open the lock BFS** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Open the lock BFS
int solve_open_the_lock_bfs(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_open_the_lock_bfs(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Open the lock BFS
int solve_open_the_lock_bfs(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Open the lock BFS
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Open the lock BFS — order-of-arrival/departure matters.
