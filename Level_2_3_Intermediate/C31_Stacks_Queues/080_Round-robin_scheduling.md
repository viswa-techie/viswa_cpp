# Chapter: Round-robin scheduling
...
## 1. Problem Statement
Master the topic: **Round-robin scheduling**.
...
This problem exercises **Round-robin scheduling** — a core concept in Stacks & Queues.
...
Think of **Round-robin scheduling** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Round-robin scheduling
int solve_round_robin_scheduling(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_round_robin_scheduling(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Round-robin scheduling
int solve_round_robin_scheduling(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Round-robin scheduling
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Round-robin scheduling — order-of-arrival/departure matters.
