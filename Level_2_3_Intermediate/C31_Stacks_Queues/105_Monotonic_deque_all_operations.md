# Chapter: Monotonic deque all operations
...
## 1. Problem Statement
Master the topic: **Monotonic deque all operations**.
...
This problem exercises **Monotonic deque all operations** — a core concept in Stacks & Queues.
...
Think of **Monotonic deque all operations** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Monotonic deque all operations
int solve_monotonic_deque_all_operations(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_monotonic_deque_all_operations(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Monotonic deque all operations
int solve_monotonic_deque_all_operations(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Monotonic deque all operations
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Monotonic deque all operations — order-of-arrival/departure matters.
