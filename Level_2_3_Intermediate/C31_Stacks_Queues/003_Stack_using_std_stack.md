...
## 1. Problem Statement
Master the topic: **Stack using std stack**.
...
This problem exercises **Stack using std stack** — a core concept in Stacks & Queues.
...
Think of **Stack using std stack** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Stack using std stack
int solve_stack_using_std_stack(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_stack_using_std_stack(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Stack using std stack
int solve_stack_using_std_stack(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Stack using std stack
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Stack using std stack — order-of-arrival/departure matters.
