...
## 1. Problem Statement
Master the topic: **Queue using std queue**.
...
This problem exercises **Queue using std queue** — a core concept in Stacks & Queues.
...
Think of **Queue using std queue** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Queue using std queue
int solve_queue_using_std_queue(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_queue_using_std_queue(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Queue using std queue
int solve_queue_using_std_queue(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Queue using std queue
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Queue using std queue — order-of-arrival/departure matters.
