...
## 1. Problem Statement
Master the topic: **Simplify Unix path**.
...
This problem exercises **Simplify Unix path** — a core concept in Stacks & Queues.
...
Think of **Simplify Unix path** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Simplify Unix path
int solve_simplify_unix_path(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_simplify_unix_path(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Simplify Unix path
int solve_simplify_unix_path(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Simplify Unix path
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Simplify Unix path — order-of-arrival/departure matters.
