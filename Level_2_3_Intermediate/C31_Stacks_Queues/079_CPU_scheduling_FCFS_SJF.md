...
## 1. Problem Statement
Master the topic: **CPU scheduling FCFS SJF**.
...
This problem exercises **CPU scheduling FCFS SJF** — a core concept in Stacks & Queues.
...
Think of **CPU scheduling FCFS SJF** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: CPU scheduling FCFS SJF
int solve_cpu_scheduling_fcfs_sjf(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_cpu_scheduling_fcfs_sjf(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: CPU scheduling FCFS SJF
int solve_cpu_scheduling_fcfs_sjf(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: CPU scheduling FCFS SJF
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** CPU scheduling FCFS SJF — order-of-arrival/departure matters.
