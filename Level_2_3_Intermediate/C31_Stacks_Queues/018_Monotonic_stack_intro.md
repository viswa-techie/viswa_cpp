...
## 1. Problem Statement
Master the topic: **Monotonic stack intro**.
...
This problem exercises **Monotonic stack intro** — a core concept in Stacks & Queues.
...
Think of **Monotonic stack intro** as a stack of plates (LIFO) or a coffee-shop queue (FIFO).
...
#include <vector>
#include <string>
// Brute-force template for: Monotonic stack intro
int solve_monotonic_stack_intro(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_monotonic_stack_intro(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Monotonic stack intro
int solve_monotonic_stack_intro(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Monotonic stack intro
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Monotonic stack intro — order-of-arrival/departure matters.
