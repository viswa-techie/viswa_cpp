# Chapter: make_heap push_heap pop_heap sort_heap

> **Level:** 2 | **Category:** C22 — STL Containers & Iterators

---

## 1. Problem Statement
Master the topic: **make_heap push_heap pop_heap sort_heap**.
...
This problem exercises **make_heap push_heap pop_heap sort_heap** — a core concept in STL Containers & Iterators.

**Internal mechanics:** Containers manage memory via allocators; iterators model pointer-like traversal with category traits.

**Common misconception:** `size()` and `capacity()` are different; `reserve()` doesn't change `size()`.

---

## 4. Mental Model & Intuition
Think of **make_heap push_heap pop_heap sort_heap** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
**Pattern category:** STL Algorithm + Container
...
#include <vector>
#include <string>
// Brute-force template for: make_heap push_heap pop_heap sort_heap
int solve_make_heap_push_heap_pop_heap_sort_heap(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_make_heap_push_heap_pop_heap_sort_heap(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: make_heap push_heap pop_heap sort_heap
int solve_make_heap_push_heap_pop_heap_sort_heap(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: make_heap push_heap pop_heap sort_heap
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Cache behavior:** vector excellent; map/unordered_map poor
...
`vector` stores 3 pointers (begin, end, cap_end). `push_back` doubles capacity on overflow → amortized O(1). `unordered_map` typical layout: bucket array of linked-list heads.

---

## 11. Pattern Recognition & Generalizations
- **Formal name:** STL Algorithm + Container
- **Similar problems:** see other entries in this category.
- **When you see** _need a container_, **think** _pick the right STL container_.
...
- **Core idea:** make_heap push_heap pop_heap sort_heap — reuse battle-tested containers and algorithms.
