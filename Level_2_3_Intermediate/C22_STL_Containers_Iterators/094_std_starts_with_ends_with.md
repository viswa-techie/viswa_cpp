# Chapter: std starts_with ends_with
...
## 1. Problem Statement
Master the topic: **std starts_with ends_with**.
...
This problem exercises **std starts_with ends_with** — a core concept in STL Containers & Iterators.
...
Think of **std starts_with ends_with** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: std starts_with ends_with
int solve_std_starts_with_ends_with(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_starts_with_ends_with(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std starts_with ends_with
int solve_std_starts_with_ends_with(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std starts_with ends_with
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std starts_with ends_with — reuse battle-tested containers and algorithms.
