# Chapter: Container complexity table
...
## 1. Problem Statement
Master the topic: **Container complexity table**.
...
This problem exercises **Container complexity table** — a core concept in STL Containers & Iterators.
...
Think of **Container complexity table** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: Container complexity table
int solve_container_complexity_table(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_container_complexity_table(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Container complexity table
int solve_container_complexity_table(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Container complexity table
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Container complexity table — reuse battle-tested containers and algorithms.
