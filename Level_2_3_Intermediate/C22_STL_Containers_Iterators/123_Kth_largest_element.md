...
## 1. Problem Statement
Master the topic: **Kth largest element**.
...
This problem exercises **Kth largest element** — a core concept in STL Containers & Iterators.
...
Think of **Kth largest element** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: Kth largest element
int solve_kth_largest_element(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_kth_largest_element(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Kth largest element
int solve_kth_largest_element(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Kth largest element
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Kth largest element — reuse battle-tested containers and algorithms.
