...
## 1. Problem Statement
Master the topic: **std forward_list**.
...
This problem exercises **std forward_list** — a core concept in STL Containers & Iterators.
...
Think of **std forward_list** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: std forward_list
int solve_std_forward_list(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_forward_list(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std forward_list
int solve_std_forward_list(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std forward_list
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std forward_list — reuse battle-tested containers and algorithms.
