# Chapter: std adjacent_difference
...
## 1. Problem Statement
Master the topic: **std adjacent_difference**.
...
This problem exercises **std adjacent_difference** — a core concept in STL Containers & Iterators.
...
Think of **std adjacent_difference** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: std adjacent_difference
int solve_std_adjacent_difference(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_adjacent_difference(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std adjacent_difference
int solve_std_adjacent_difference(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std adjacent_difference
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std adjacent_difference — reuse battle-tested containers and algorithms.
