...
## 1. Problem Statement
Master the topic: **map extract node**.
...
This problem exercises **map extract node** — a core concept in STL Containers & Iterators.
...
Think of **map extract node** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: map extract node
int solve_map_extract_node(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_map_extract_node(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: map extract node
int solve_map_extract_node(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: map extract node
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** map extract node — reuse battle-tested containers and algorithms.
