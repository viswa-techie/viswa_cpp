...
## 1. Problem Statement
Master the topic: **Two-heap median**.
...
This problem exercises **Two-heap median** — a core concept in STL Containers & Iterators.
...
Think of **Two-heap median** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: Two-heap median
int solve_two_heap_median(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_two_heap_median(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Two-heap median
int solve_two_heap_median(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Two-heap median
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Two-heap median — reuse battle-tested containers and algorithms.
