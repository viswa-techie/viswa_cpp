...
## 1. Problem Statement
Master the topic: **std binary_search**.
...
This problem exercises **std binary_search** — a core concept in STL Containers & Iterators.
...
Think of **std binary_search** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: std binary_search
int solve_std_binary_search(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_binary_search(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std binary_search
int solve_std_binary_search(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std binary_search
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std binary_search — reuse battle-tested containers and algorithms.
