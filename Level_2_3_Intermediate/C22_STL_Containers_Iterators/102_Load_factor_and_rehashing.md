# Chapter: Load factor and rehashing
...
## 1. Problem Statement
Master the topic: **Load factor and rehashing**.
...
This problem exercises **Load factor and rehashing** — a core concept in STL Containers & Iterators.
...
Think of **Load factor and rehashing** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: Load factor and rehashing
int solve_load_factor_and_rehashing(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_load_factor_and_rehashing(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Load factor and rehashing
int solve_load_factor_and_rehashing(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Load factor and rehashing
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Load factor and rehashing — reuse battle-tested containers and algorithms.
