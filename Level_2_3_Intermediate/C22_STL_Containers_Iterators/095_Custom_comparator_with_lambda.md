# Chapter: Custom comparator with lambda
...
## 1. Problem Statement
Master the topic: **Custom comparator with lambda**.
...
This problem exercises **Custom comparator with lambda** — a core concept in STL Containers & Iterators.
...
Think of **Custom comparator with lambda** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: Custom comparator with lambda
int solve_custom_comparator_with_lambda(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_custom_comparator_with_lambda(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Custom comparator with lambda
int solve_custom_comparator_with_lambda(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Custom comparator with lambda
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Custom comparator with lambda — reuse battle-tested containers and algorithms.
