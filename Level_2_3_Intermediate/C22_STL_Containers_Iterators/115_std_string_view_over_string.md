# Chapter: std string_view over string
...
## 1. Problem Statement
Master the topic: **std string_view over string**.
...
This problem exercises **std string_view over string** — a core concept in STL Containers & Iterators.
...
Think of **std string_view over string** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: std string_view over string
int solve_std_string_view_over_string(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_string_view_over_string(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std string_view over string
int solve_std_string_view_over_string(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std string_view over string
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std string_view over string — reuse battle-tested containers and algorithms.
