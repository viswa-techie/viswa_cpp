...
## 1. Problem Statement
Master the topic: **take drop enumerate**.
...
This problem exercises **take drop enumerate** — a core concept in STL Containers & Iterators.
...
Think of **take drop enumerate** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: take drop enumerate
int solve_take_drop_enumerate(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_take_drop_enumerate(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: take drop enumerate
int solve_take_drop_enumerate(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: take drop enumerate
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** take drop enumerate — reuse battle-tested containers and algorithms.
