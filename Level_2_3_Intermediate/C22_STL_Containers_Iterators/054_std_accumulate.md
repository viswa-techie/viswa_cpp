...
## 1. Problem Statement
Master the topic: **std accumulate**.
...
This problem exercises **std accumulate** — a core concept in STL Containers & Iterators.
...
Think of **std accumulate** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: std accumulate
int solve_std_accumulate(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_accumulate(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std accumulate
int solve_std_accumulate(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std accumulate
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std accumulate — reuse battle-tested containers and algorithms.
