...
## 1. Problem Statement
Master the topic: **std equal_range**.
...
This problem exercises **std equal_range** — a core concept in STL Containers & Iterators.
...
Think of **std equal_range** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: std equal_range
int solve_std_equal_range(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_equal_range(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std equal_range
int solve_std_equal_range(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std equal_range
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std equal_range — reuse battle-tested containers and algorithms.
