# Chapter: map emplace try_emplace
...
## 1. Problem Statement
Master the topic: **map emplace try_emplace**.
...
This problem exercises **map emplace try_emplace** — a core concept in STL Containers & Iterators.
...
Think of **map emplace try_emplace** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: map emplace try_emplace
int solve_map_emplace_try_emplace(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_map_emplace_try_emplace(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: map emplace try_emplace
int solve_map_emplace_try_emplace(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: map emplace try_emplace
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** map emplace try_emplace — reuse battle-tested containers and algorithms.
