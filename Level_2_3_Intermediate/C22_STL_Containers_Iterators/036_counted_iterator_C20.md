...
## 1. Problem Statement
Master the topic: **counted_iterator C++20**.
...
This problem exercises **counted_iterator C++20** — a core concept in STL Containers & Iterators.
...
Think of **counted_iterator C++20** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: counted_iterator C++20
int solve_counted_iterator_c__20(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_counted_iterator_c__20(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: counted_iterator C++20
int solve_counted_iterator_c__20(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: counted_iterator C++20
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** counted_iterator C++20 — reuse battle-tested containers and algorithms.
