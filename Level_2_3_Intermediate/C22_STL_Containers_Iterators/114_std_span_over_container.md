# Chapter: std span over container
...
## 1. Problem Statement
Master the topic: **std span over container**.
...
This problem exercises **std span over container** — a core concept in STL Containers & Iterators.
...
Think of **std span over container** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: std span over container
int solve_std_span_over_container(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_span_over_container(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std span over container
int solve_std_span_over_container(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std span over container
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std span over container — reuse battle-tested containers and algorithms.
