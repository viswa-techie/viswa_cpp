# Chapter: std string as container
...
## 1. Problem Statement
Master the topic: **std string as container**.
...
This problem exercises **std string as container** — a core concept in STL Containers & Iterators.
...
Think of **std string as container** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: std string as container
int solve_std_string_as_container(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_string_as_container(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std string as container
int solve_std_string_as_container(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std string as container
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std string as container — reuse battle-tested containers and algorithms.
