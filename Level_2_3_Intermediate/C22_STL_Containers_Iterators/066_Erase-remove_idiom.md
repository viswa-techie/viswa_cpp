...
## 1. Problem Statement
Master the topic: **Erase-remove idiom**.
...
This problem exercises **Erase-remove idiom** — a core concept in STL Containers & Iterators.
...
Think of **Erase-remove idiom** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: Erase-remove idiom
int solve_erase_remove_idiom(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_erase_remove_idiom(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Erase-remove idiom
int solve_erase_remove_idiom(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Erase-remove idiom
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Erase-remove idiom — reuse battle-tested containers and algorithms.
