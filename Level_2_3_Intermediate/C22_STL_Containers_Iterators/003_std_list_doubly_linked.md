# Chapter: std list doubly linked
...
## 1. Problem Statement
Master the topic: **std list doubly linked**.
...
This problem exercises **std list doubly linked** — a core concept in STL Containers & Iterators.
...
Think of **std list doubly linked** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: std list doubly linked
int solve_std_list_doubly_linked(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_list_doubly_linked(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std list doubly linked
int solve_std_list_doubly_linked(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std list doubly linked
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std list doubly linked — reuse battle-tested containers and algorithms.
