...
## 1. Problem Statement
Master the topic: **std unique for sorted**.
...
This problem exercises **std unique for sorted** — a core concept in STL Containers & Iterators.
...
Think of **std unique for sorted** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: std unique for sorted
int solve_std_unique_for_sorted(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_unique_for_sorted(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std unique for sorted
int solve_std_unique_for_sorted(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std unique for sorted
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std unique for sorted — reuse battle-tested containers and algorithms.
