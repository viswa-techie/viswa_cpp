# Chapter: Flat map sorted vector pairs
...
## 1. Problem Statement
Master the topic: **Flat map sorted vector pairs**.
...
This problem exercises **Flat map sorted vector pairs** — a core concept in STL Containers & Iterators.
...
Think of **Flat map sorted vector pairs** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: Flat map sorted vector pairs
int solve_flat_map_sorted_vector_pairs(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_flat_map_sorted_vector_pairs(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Flat map sorted vector pairs
int solve_flat_map_sorted_vector_pairs(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Flat map sorted vector pairs
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Flat map sorted vector pairs — reuse battle-tested containers and algorithms.
