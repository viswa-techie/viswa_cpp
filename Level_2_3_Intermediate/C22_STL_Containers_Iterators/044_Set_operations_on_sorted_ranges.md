# Chapter: Set operations on sorted ranges
...
## 1. Problem Statement
Master the topic: **Set operations on sorted ranges**.
...
This problem exercises **Set operations on sorted ranges** — a core concept in STL Containers & Iterators.
...
Think of **Set operations on sorted ranges** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: Set operations on sorted ranges
int solve_set_operations_on_sorted_ranges(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_set_operations_on_sorted_ranges(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Set operations on sorted ranges
int solve_set_operations_on_sorted_ranges(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Set operations on sorted ranges
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Set operations on sorted ranges — reuse battle-tested containers and algorithms.
