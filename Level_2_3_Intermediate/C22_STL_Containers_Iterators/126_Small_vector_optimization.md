# Chapter: Small vector optimization
...
## 1. Problem Statement
Master the topic: **Small vector optimization**.
...
This problem exercises **Small vector optimization** — a core concept in STL Containers & Iterators.
...
Think of **Small vector optimization** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: Small vector optimization
int solve_small_vector_optimization(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_small_vector_optimization(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Small vector optimization
int solve_small_vector_optimization(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Small vector optimization
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Small vector optimization — reuse battle-tested containers and algorithms.
