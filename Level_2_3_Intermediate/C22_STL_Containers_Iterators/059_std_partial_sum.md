...
## 1. Problem Statement
Master the topic: **std partial_sum**.
...
This problem exercises **std partial_sum** — a core concept in STL Containers & Iterators.
...
Think of **std partial_sum** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: std partial_sum
int solve_std_partial_sum(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_partial_sum(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std partial_sum
int solve_std_partial_sum(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std partial_sum
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std partial_sum — reuse battle-tested containers and algorithms.
