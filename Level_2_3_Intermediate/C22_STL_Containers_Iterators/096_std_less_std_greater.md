...
## 1. Problem Statement
Master the topic: **std less std greater**.
...
This problem exercises **std less std greater** — a core concept in STL Containers & Iterators.
...
Think of **std less std greater** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: std less std greater
int solve_std_less_std_greater(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_less_std_greater(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std less std greater
int solve_std_less_std_greater(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std less std greater
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std less std greater — reuse battle-tested containers and algorithms.
