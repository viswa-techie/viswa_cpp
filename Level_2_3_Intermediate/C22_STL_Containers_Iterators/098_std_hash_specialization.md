# Chapter: std hash specialization
...
## 1. Problem Statement
Master the topic: **std hash specialization**.
...
This problem exercises **std hash specialization** — a core concept in STL Containers & Iterators.
...
Think of **std hash specialization** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: std hash specialization
int solve_std_hash_specialization(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_hash_specialization(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std hash specialization
int solve_std_hash_specialization(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std hash specialization
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std hash specialization — reuse battle-tested containers and algorithms.
