# Chapter: set lower_bound upper_bound
...
## 1. Problem Statement
Master the topic: **set lower_bound upper_bound**.
...
This problem exercises **set lower_bound upper_bound** — a core concept in STL Containers & Iterators.
...
Think of **set lower_bound upper_bound** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: set lower_bound upper_bound
int solve_set_lower_bound_upper_bound(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_set_lower_bound_upper_bound(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: set lower_bound upper_bound
int solve_set_lower_bound_upper_bound(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: set lower_bound upper_bound
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** set lower_bound upper_bound — reuse battle-tested containers and algorithms.
