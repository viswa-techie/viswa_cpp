...
## 1. Problem Statement
Master the topic: **std is_permutation**.
...
This problem exercises **std is_permutation** — a core concept in STL Containers & Iterators.
...
Think of **std is_permutation** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: std is_permutation
int solve_std_is_permutation(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_is_permutation(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std is_permutation
int solve_std_is_permutation(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std is_permutation
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std is_permutation — reuse battle-tested containers and algorithms.
