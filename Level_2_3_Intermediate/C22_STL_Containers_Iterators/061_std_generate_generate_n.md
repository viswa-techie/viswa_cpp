# Chapter: std generate generate_n
...
## 1. Problem Statement
Master the topic: **std generate generate_n**.
...
This problem exercises **std generate generate_n** — a core concept in STL Containers & Iterators.
...
Think of **std generate generate_n** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: std generate generate_n
int solve_std_generate_generate_n(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_generate_generate_n(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std generate generate_n
int solve_std_generate_generate_n(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std generate generate_n
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std generate generate_n — reuse battle-tested containers and algorithms.
