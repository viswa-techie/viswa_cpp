...
## 1. Problem Statement
Master the topic: **Custom hash function**.
...
This problem exercises **Custom hash function** — a core concept in STL Containers & Iterators.
...
Think of **Custom hash function** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: Custom hash function
int solve_custom_hash_function(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_custom_hash_function(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Custom hash function
int solve_custom_hash_function(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Custom hash function
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Custom hash function — reuse battle-tested containers and algorithms.
