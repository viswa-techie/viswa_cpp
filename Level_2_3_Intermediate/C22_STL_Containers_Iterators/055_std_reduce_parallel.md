...
## 1. Problem Statement
Master the topic: **std reduce parallel**.
...
This problem exercises **std reduce parallel** — a core concept in STL Containers & Iterators.
...
Think of **std reduce parallel** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: std reduce parallel
int solve_std_reduce_parallel(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_reduce_parallel(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std reduce parallel
int solve_std_reduce_parallel(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std reduce parallel
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std reduce parallel — reuse battle-tested containers and algorithms.
