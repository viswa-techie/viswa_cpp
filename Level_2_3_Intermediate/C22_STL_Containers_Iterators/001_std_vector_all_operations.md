# Chapter: std vector all operations
...
## 1. Problem Statement
Master the topic: **std vector all operations**.
...
This problem exercises **std vector all operations** — a core concept in STL Containers & Iterators.
...
Think of **std vector all operations** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: std vector all operations
int solve_std_vector_all_operations(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_vector_all_operations(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std vector all operations
int solve_std_vector_all_operations(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std vector all operations
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std vector all operations — reuse battle-tested containers and algorithms.
