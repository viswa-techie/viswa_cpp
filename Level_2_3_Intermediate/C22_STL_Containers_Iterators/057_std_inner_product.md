...
## 1. Problem Statement
Master the topic: **std inner_product**.
...
This problem exercises **std inner_product** — a core concept in STL Containers & Iterators.
...
Think of **std inner_product** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: std inner_product
int solve_std_inner_product(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_inner_product(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std inner_product
int solve_std_inner_product(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std inner_product
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std inner_product — reuse battle-tested containers and algorithms.
