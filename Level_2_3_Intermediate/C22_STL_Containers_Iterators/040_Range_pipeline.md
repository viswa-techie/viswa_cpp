...
## 1. Problem Statement
Master the topic: **Range pipeline**.
...
This problem exercises **Range pipeline** — a core concept in STL Containers & Iterators.
...
Think of **Range pipeline** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: Range pipeline
int solve_range_pipeline(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_range_pipeline(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Range pipeline
int solve_range_pipeline(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Range pipeline
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Range pipeline — reuse battle-tested containers and algorithms.
