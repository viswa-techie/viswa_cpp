# Chapter: Bucket count and max load factor
...
## 1. Problem Statement
Master the topic: **Bucket count and max load factor**.
...
This problem exercises **Bucket count and max load factor** — a core concept in STL Containers & Iterators.
...
Think of **Bucket count and max load factor** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: Bucket count and max load factor
int solve_bucket_count_and_max_load_factor(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_bucket_count_and_max_load_factor(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Bucket count and max load factor
int solve_bucket_count_and_max_load_factor(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Bucket count and max load factor
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Bucket count and max load factor — reuse battle-tested containers and algorithms.
