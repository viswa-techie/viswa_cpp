# Chapter: Choosing the right container
...
## 1. Problem Statement
Master the topic: **Choosing the right container**.
...
This problem exercises **Choosing the right container** — a core concept in STL Containers & Iterators.
...
Think of **Choosing the right container** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: Choosing the right container
int solve_choosing_the_right_container(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_choosing_the_right_container(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Choosing the right container
int solve_choosing_the_right_container(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Choosing the right container
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Choosing the right container — reuse battle-tested containers and algorithms.
