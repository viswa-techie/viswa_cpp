# Chapter: Sliding window with deque
...
## 1. Problem Statement
Master the topic: **Sliding window with deque**.
...
This problem exercises **Sliding window with deque** — a core concept in STL Containers & Iterators.
...
Think of **Sliding window with deque** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: Sliding window with deque
int solve_sliding_window_with_deque(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_sliding_window_with_deque(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Sliding window with deque
int solve_sliding_window_with_deque(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Sliding window with deque
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Sliding window with deque — reuse battle-tested containers and algorithms.
