...
## 1. Problem Statement
Master the topic: **istream_iterator**.
...
This problem exercises **istream_iterator** — a core concept in STL Containers & Iterators.
...
Think of **istream_iterator** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: istream_iterator
int solve_istream_iterator(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_istream_iterator(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: istream_iterator
int solve_istream_iterator(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: istream_iterator
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** istream_iterator — reuse battle-tested containers and algorithms.
