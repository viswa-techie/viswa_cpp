...
## 1. Problem Statement
Master the topic: **std next prev**.
...
This problem exercises **std next prev** — a core concept in STL Containers & Iterators.
...
Think of **std next prev** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: std next prev
int solve_std_next_prev(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_next_prev(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std next prev
int solve_std_next_prev(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std next prev
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std next prev — reuse battle-tested containers and algorithms.
