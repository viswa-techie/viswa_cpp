...
## 1. Problem Statement
Master the topic: **Heap operations**.
...
This problem exercises **Heap operations** — a core concept in STL Containers & Iterators.
...
Think of **Heap operations** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: Heap operations
int solve_heap_operations(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_heap_operations(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Heap operations
int solve_heap_operations(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Heap operations
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Heap operations — reuse battle-tested containers and algorithms.
