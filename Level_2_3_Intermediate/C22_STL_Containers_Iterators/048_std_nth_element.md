...
## 1. Problem Statement
Master the topic: **std nth_element**.
...
This problem exercises **std nth_element** — a core concept in STL Containers & Iterators.
...
Think of **std nth_element** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: std nth_element
int solve_std_nth_element(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_nth_element(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std nth_element
int solve_std_nth_element(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std nth_element
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std nth_element — reuse battle-tested containers and algorithms.
