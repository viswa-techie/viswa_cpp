# Chapter: std unordered_multiset
...
## 1. Problem Statement
Master the topic: **std unordered_multiset**.
...
This problem exercises **std unordered_multiset** — a core concept in STL Containers & Iterators.
...
Think of **std unordered_multiset** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: std unordered_multiset
int solve_std_unordered_multiset(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_unordered_multiset(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std unordered_multiset
int solve_std_unordered_multiset(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std unordered_multiset
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std unordered_multiset — reuse battle-tested containers and algorithms.
