...
This problem exercises **std for_each** — a core concept in STL Containers & Iterators.
...
Think of **std for_each** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: std for_each
int solve_std_for_each(std::vector<int> v) {
    int result = 0;
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std for_each
int solve_std_for_each(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std for_each
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std for_each — reuse battle-tested containers and algorithms.
