# Chapter: std inclusive_scan exclusive_scan
...
## 1. Problem Statement
Master the topic: **std inclusive_scan exclusive_scan**.
...
This problem exercises **std inclusive_scan exclusive_scan** — a core concept in STL Containers & Iterators.
...
Think of **std inclusive_scan exclusive_scan** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: std inclusive_scan exclusive_scan
int solve_std_inclusive_scan_exclusive_scan(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_inclusive_scan_exclusive_scan(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std inclusive_scan exclusive_scan
int solve_std_inclusive_scan_exclusive_scan(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std inclusive_scan exclusive_scan
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std inclusive_scan exclusive_scan — reuse battle-tested containers and algorithms.
