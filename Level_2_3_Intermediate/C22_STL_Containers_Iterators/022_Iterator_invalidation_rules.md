# Chapter: Iterator invalidation rules
...
## 1. Problem Statement
Master the topic: **Iterator invalidation rules**.
...
This problem exercises **Iterator invalidation rules** — a core concept in STL Containers & Iterators.
...
Think of **Iterator invalidation rules** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: Iterator invalidation rules
int solve_iterator_invalidation_rules(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_iterator_invalidation_rules(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Iterator invalidation rules
int solve_iterator_invalidation_rules(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Iterator invalidation rules
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Iterator invalidation rules — reuse battle-tested containers and algorithms.
