...
## 1. Problem Statement
Master the topic: **K closest elements**.
...
This problem exercises **K closest elements** — a core concept in STL Containers & Iterators.
...
Think of **K closest elements** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: K closest elements
int solve_k_closest_elements(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_k_closest_elements(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: K closest elements
int solve_k_closest_elements(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: K closest elements
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** K closest elements — reuse battle-tested containers and algorithms.
