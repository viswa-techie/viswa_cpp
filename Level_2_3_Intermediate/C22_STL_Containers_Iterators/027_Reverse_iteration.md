...
## 1. Problem Statement
Master the topic: **Reverse iteration**.
...
This problem exercises **Reverse iteration** — a core concept in STL Containers & Iterators.
...
Think of **Reverse iteration** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: Reverse iteration
int solve_reverse_iteration(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_reverse_iteration(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Reverse iteration
int solve_reverse_iteration(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Reverse iteration
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Reverse iteration — reuse battle-tested containers and algorithms.
