# Chapter: std is_sorted is_sorted_until
...
## 1. Problem Statement
Master the topic: **std is_sorted is_sorted_until**.
...
This problem exercises **std is_sorted is_sorted_until** — a core concept in STL Containers & Iterators.
...
Think of **std is_sorted is_sorted_until** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: std is_sorted is_sorted_until
int solve_std_is_sorted_is_sorted_until(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_is_sorted_is_sorted_until(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std is_sorted is_sorted_until
int solve_std_is_sorted_is_sorted_until(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std is_sorted is_sorted_until
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std is_sorted is_sorted_until — reuse battle-tested containers and algorithms.
