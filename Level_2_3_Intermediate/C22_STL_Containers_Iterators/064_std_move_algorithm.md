...
## 1. Problem Statement
Master the topic: **std move algorithm**.
...
This problem exercises **std move algorithm** — a core concept in STL Containers & Iterators.
...
Think of **std move algorithm** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: std move algorithm
int solve_std_move_algorithm(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_move_algorithm(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std move algorithm
int solve_std_move_algorithm(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std move algorithm
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std move algorithm — reuse battle-tested containers and algorithms.
