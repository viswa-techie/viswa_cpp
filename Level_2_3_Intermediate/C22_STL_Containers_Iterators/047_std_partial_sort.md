...
## 1. Problem Statement
Master the topic: **std partial_sort**.
...
This problem exercises **std partial_sort** — a core concept in STL Containers & Iterators.
...
Think of **std partial_sort** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: std partial_sort
int solve_std_partial_sort(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_partial_sort(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std partial_sort
int solve_std_partial_sort(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std partial_sort
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std partial_sort — reuse battle-tested containers and algorithms.
