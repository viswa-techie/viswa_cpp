...
## 1. Problem Statement
Master the topic: **std ranges view**.
...
This problem exercises **std ranges view** — a core concept in STL Containers & Iterators.
...
Think of **std ranges view** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: std ranges view
int solve_std_ranges_view(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_ranges_view(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std ranges view
int solve_std_ranges_view(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std ranges view
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std ranges view — reuse battle-tested containers and algorithms.
