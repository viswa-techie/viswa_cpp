# Chapter: begin end cbegin rbegin
...
## 1. Problem Statement
Master the topic: **begin end cbegin rbegin**.
...
This problem exercises **begin end cbegin rbegin** — a core concept in STL Containers & Iterators.
...
Think of **begin end cbegin rbegin** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: begin end cbegin rbegin
int solve_begin_end_cbegin_rbegin(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_begin_end_cbegin_rbegin(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: begin end cbegin rbegin
int solve_begin_end_cbegin_rbegin(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: begin end cbegin rbegin
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** begin end cbegin rbegin — reuse battle-tested containers and algorithms.
