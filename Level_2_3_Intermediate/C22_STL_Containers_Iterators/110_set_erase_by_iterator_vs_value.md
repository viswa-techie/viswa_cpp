# Chapter: set erase by iterator vs value
...
## 1. Problem Statement
Master the topic: **set erase by iterator vs value**.
...
This problem exercises **set erase by iterator vs value** — a core concept in STL Containers & Iterators.
...
Think of **set erase by iterator vs value** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: set erase by iterator vs value
int solve_set_erase_by_iterator_vs_value(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_set_erase_by_iterator_vs_value(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: set erase by iterator vs value
int solve_set_erase_by_iterator_vs_value(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: set erase by iterator vs value
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** set erase by iterator vs value — reuse battle-tested containers and algorithms.
