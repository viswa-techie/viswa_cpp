# Chapter: Monotonic stack pattern
...
## 1. Problem Statement
Master the topic: **Monotonic stack pattern**.
...
This problem exercises **Monotonic stack pattern** — a core concept in STL Containers & Iterators.
...
Think of **Monotonic stack pattern** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: Monotonic stack pattern
int solve_monotonic_stack_pattern(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_monotonic_stack_pattern(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Monotonic stack pattern
int solve_monotonic_stack_pattern(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Monotonic stack pattern
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Monotonic stack pattern — reuse battle-tested containers and algorithms.
