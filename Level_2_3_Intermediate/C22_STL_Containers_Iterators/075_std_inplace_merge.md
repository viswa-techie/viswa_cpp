...
## 1. Problem Statement
Master the topic: **std inplace_merge**.
...
This problem exercises **std inplace_merge** — a core concept in STL Containers & Iterators.
...
Think of **std inplace_merge** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: std inplace_merge
int solve_std_inplace_merge(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_inplace_merge(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std inplace_merge
int solve_std_inplace_merge(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std inplace_merge
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std inplace_merge — reuse battle-tested containers and algorithms.
