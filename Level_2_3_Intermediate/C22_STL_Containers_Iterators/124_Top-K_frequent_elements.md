...
This problem exercises **Top-K frequent elements** — a core concept in STL Containers & Iterators.
...
Think of **Top-K frequent elements** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: Top-K frequent elements
int solve_top_k_frequent_elements(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_top_k_frequent_elements(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Top-K frequent elements
int solve_top_k_frequent_elements(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
- **Core idea:** Top-K frequent elements — reuse battle-tested containers and algorithms.
