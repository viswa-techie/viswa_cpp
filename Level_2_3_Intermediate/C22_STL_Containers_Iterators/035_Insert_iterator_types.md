...
## 1. Problem Statement
Master the topic: **Insert iterator types**.
...
This problem exercises **Insert iterator types** — a core concept in STL Containers & Iterators.
...
Think of **Insert iterator types** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: Insert iterator types
int solve_insert_iterator_types(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_insert_iterator_types(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Insert iterator types
int solve_insert_iterator_types(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Insert iterator types
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Insert iterator types — reuse battle-tested containers and algorithms.
