...
## 1. Problem Statement
Master the topic: **std transform**.
...
This problem exercises **std transform** — a core concept in STL Containers & Iterators.
...
Think of **std transform** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: std transform
int solve_std_transform(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_transform(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std transform
int solve_std_transform(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std transform
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std transform — reuse battle-tested containers and algorithms.
