# Chapter: Priority queue with custom struct
...
## 1. Problem Statement
Master the topic: **Priority queue with custom struct**.
...
This problem exercises **Priority queue with custom struct** — a core concept in STL Containers & Iterators.
...
Think of **Priority queue with custom struct** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: Priority queue with custom struct
int solve_priority_queue_with_custom_struct(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_priority_queue_with_custom_struct(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Priority queue with custom struct
int solve_priority_queue_with_custom_struct(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Priority queue with custom struct
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Priority queue with custom struct — reuse battle-tested containers and algorithms.
