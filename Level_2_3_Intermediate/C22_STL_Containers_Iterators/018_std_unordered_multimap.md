# Chapter: std unordered_multimap
...
## 1. Problem Statement
Master the topic: **std unordered_multimap**.
...
This problem exercises **std unordered_multimap** — a core concept in STL Containers & Iterators.
...
Think of **std unordered_multimap** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: std unordered_multimap
int solve_std_unordered_multimap(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_unordered_multimap(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std unordered_multimap
int solve_std_unordered_multimap(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std unordered_multimap
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std unordered_multimap — reuse battle-tested containers and algorithms.
