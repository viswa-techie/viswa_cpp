# Chapter: View adapters filter transform
...
## 1. Problem Statement
Master the topic: **View adapters filter transform**.
...
This problem exercises **View adapters filter transform** — a core concept in STL Containers & Iterators.
...
Think of **View adapters filter transform** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: View adapters filter transform
int solve_view_adapters_filter_transform(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_view_adapters_filter_transform(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: View adapters filter transform
int solve_view_adapters_filter_transform(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: View adapters filter transform
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** View adapters filter transform — reuse battle-tested containers and algorithms.
