...
## 1. Problem Statement
Master the topic: **const_iterator usage**.
...
This problem exercises **const_iterator usage** — a core concept in STL Containers & Iterators.
...
Think of **const_iterator usage** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: const_iterator usage
int solve_const_iterator_usage(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_const_iterator_usage(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: const_iterator usage
int solve_const_iterator_usage(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: const_iterator usage
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** const_iterator usage — reuse battle-tested containers and algorithms.
