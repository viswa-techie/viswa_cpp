...
## 1. Problem Statement
Master the topic: **std sort Introsort**.
...
This problem exercises **std sort Introsort** — a core concept in STL Containers & Iterators.
...
Think of **std sort Introsort** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: std sort Introsort
int solve_std_sort_introsort(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_sort_introsort(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std sort Introsort
int solve_std_sort_introsort(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std sort Introsort
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std sort Introsort — reuse battle-tested containers and algorithms.
