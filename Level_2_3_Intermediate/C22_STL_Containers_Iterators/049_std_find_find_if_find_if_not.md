# Chapter: std find find_if find_if_not
...
## 1. Problem Statement
Master the topic: **std find find_if find_if_not**.
...
This problem exercises **std find find_if find_if_not** — a core concept in STL Containers & Iterators.
...
Think of **std find find_if find_if_not** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: std find find_if find_if_not
int solve_std_find_find_if_find_if_not(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_find_find_if_find_if_not(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std find find_if find_if_not
int solve_std_find_find_if_find_if_not(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std find find_if find_if_not
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std find find_if find_if_not — reuse battle-tested containers and algorithms.
