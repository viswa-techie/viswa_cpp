...
This problem exercises **Lazy ranges** — a core concept in STL Containers & Iterators.
...
Think of **Lazy ranges** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: Lazy ranges
int solve_lazy_ranges(std::vector<int> v) {
    int result = 0;
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Lazy ranges
int solve_lazy_ranges(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Lazy ranges
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Lazy ranges — reuse battle-tested containers and algorithms.
