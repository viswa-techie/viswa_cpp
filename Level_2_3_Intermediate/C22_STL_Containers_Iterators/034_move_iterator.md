...
## 1. Problem Statement
Master the topic: **move_iterator**.
...
This problem exercises **move_iterator** — a core concept in STL Containers & Iterators.
...
Think of **move_iterator** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: move_iterator
int solve_move_iterator(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_move_iterator(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: move_iterator
int solve_move_iterator(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: move_iterator
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** move_iterator — reuse battle-tested containers and algorithms.
