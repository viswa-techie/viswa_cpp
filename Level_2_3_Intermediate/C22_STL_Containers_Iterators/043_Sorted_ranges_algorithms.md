# Chapter: Sorted ranges algorithms
...
## 1. Problem Statement
Master the topic: **Sorted ranges algorithms**.
...
This problem exercises **Sorted ranges algorithms** — a core concept in STL Containers & Iterators.
...
Think of **Sorted ranges algorithms** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: Sorted ranges algorithms
int solve_sorted_ranges_algorithms(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_sorted_ranges_algorithms(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Sorted ranges algorithms
int solve_sorted_ranges_algorithms(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Sorted ranges algorithms
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Sorted ranges algorithms — reuse battle-tested containers and algorithms.
