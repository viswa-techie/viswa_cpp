...
## 1. Problem Statement
Master the topic: **std count count_if**.
...
This problem exercises **std count count_if** — a core concept in STL Containers & Iterators.
...
Think of **std count count_if** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: std count count_if
int solve_std_count_count_if(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_count_count_if(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std count count_if
int solve_std_count_count_if(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std count count_if
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std count count_if — reuse battle-tested containers and algorithms.
