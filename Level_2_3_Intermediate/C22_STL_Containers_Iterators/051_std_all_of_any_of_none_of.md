# Chapter: std all_of any_of none_of
...
## 1. Problem Statement
Master the topic: **std all_of any_of none_of**.
...
This problem exercises **std all_of any_of none_of** — a core concept in STL Containers & Iterators.
...
Think of **std all_of any_of none_of** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: std all_of any_of none_of
int solve_std_all_of_any_of_none_of(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_all_of_any_of_none_of(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std all_of any_of none_of
int solve_std_all_of_any_of_none_of(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std all_of any_of none_of
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std all_of any_of none_of — reuse battle-tested containers and algorithms.
