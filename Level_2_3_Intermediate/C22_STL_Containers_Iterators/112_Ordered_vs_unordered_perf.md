# Chapter: Ordered vs unordered perf
...
## 1. Problem Statement
Master the topic: **Ordered vs unordered perf**.
...
This problem exercises **Ordered vs unordered perf** — a core concept in STL Containers & Iterators.
...
Think of **Ordered vs unordered perf** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: Ordered vs unordered perf
int solve_ordered_vs_unordered_perf(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_ordered_vs_unordered_perf(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Ordered vs unordered perf
int solve_ordered_vs_unordered_perf(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Ordered vs unordered perf
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Ordered vs unordered perf — reuse battle-tested containers and algorithms.
