...
## 1. Problem Statement
Master the topic: **unordered_map reserve**.
...
This problem exercises **unordered_map reserve** — a core concept in STL Containers & Iterators.
...
Think of **unordered_map reserve** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: unordered_map reserve
int solve_unordered_map_reserve(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_unordered_map_reserve(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: unordered_map reserve
int solve_unordered_map_reserve(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: unordered_map reserve
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** unordered_map reserve — reuse battle-tested containers and algorithms.
