# Chapter: priority_queue max-heap
...
## 1. Problem Statement
Master the topic: **priority_queue max-heap**.
...
This problem exercises **priority_queue max-heap** — a core concept in STL Containers & Iterators.
...
Think of **priority_queue max-heap** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: priority_queue max-heap
int solve_priority_queue_max_heap(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_priority_queue_max_heap(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: priority_queue max-heap
int solve_priority_queue_max_heap(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: priority_queue max-heap
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** priority_queue max-heap — reuse battle-tested containers and algorithms.
