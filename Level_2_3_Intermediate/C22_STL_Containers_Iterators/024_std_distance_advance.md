...
## 1. Problem Statement
Master the topic: **std distance advance**.
...
This problem exercises **std distance advance** — a core concept in STL Containers & Iterators.
...
Think of **std distance advance** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: std distance advance
int solve_std_distance_advance(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_distance_advance(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std distance advance
int solve_std_distance_advance(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std distance advance
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std distance advance — reuse battle-tested containers and algorithms.
