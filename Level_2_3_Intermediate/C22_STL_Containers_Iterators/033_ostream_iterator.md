...
## 1. Problem Statement
Master the topic: **ostream_iterator**.
...
This problem exercises **ostream_iterator** — a core concept in STL Containers & Iterators.
...
Think of **ostream_iterator** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: ostream_iterator
int solve_ostream_iterator(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_ostream_iterator(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: ostream_iterator
int solve_ostream_iterator(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: ostream_iterator
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** ostream_iterator — reuse battle-tested containers and algorithms.
