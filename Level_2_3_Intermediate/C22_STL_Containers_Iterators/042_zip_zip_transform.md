...
## 1. Problem Statement
Master the topic: **zip zip_transform**.
...
This problem exercises **zip zip_transform** — a core concept in STL Containers & Iterators.
...
Think of **zip zip_transform** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: zip zip_transform
int solve_zip_zip_transform(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_zip_zip_transform(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: zip zip_transform
int solve_zip_zip_transform(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: zip zip_transform
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** zip zip_transform — reuse battle-tested containers and algorithms.
