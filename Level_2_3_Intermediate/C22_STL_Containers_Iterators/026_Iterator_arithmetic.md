...
## 1. Problem Statement
Master the topic: **Iterator arithmetic**.
...
This problem exercises **Iterator arithmetic** — a core concept in STL Containers & Iterators.
...
Think of **Iterator arithmetic** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: Iterator arithmetic
int solve_iterator_arithmetic(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_iterator_arithmetic(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Iterator arithmetic
int solve_iterator_arithmetic(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Iterator arithmetic
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Iterator arithmetic — reuse battle-tested containers and algorithms.
