...
## 1. Problem Statement
Master the topic: **multimap equal_range**.
...
This problem exercises **multimap equal_range** — a core concept in STL Containers & Iterators.
...
Think of **multimap equal_range** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: multimap equal_range
int solve_multimap_equal_range(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_multimap_equal_range(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: multimap equal_range
int solve_multimap_equal_range(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: multimap equal_range
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** multimap equal_range — reuse battle-tested containers and algorithms.
