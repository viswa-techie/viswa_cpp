# Chapter: Collision handling concept
...
## 1. Problem Statement
Master the topic: **Collision handling concept**.
...
This problem exercises **Collision handling concept** — a core concept in STL Containers & Iterators.
...
Think of **Collision handling concept** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: Collision handling concept
int solve_collision_handling_concept(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_collision_handling_concept(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Collision handling concept
int solve_collision_handling_concept(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Collision handling concept
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Collision handling concept — reuse battle-tested containers and algorithms.
