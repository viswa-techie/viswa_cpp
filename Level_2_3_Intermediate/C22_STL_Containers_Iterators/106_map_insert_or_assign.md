...
## 1. Problem Statement
Master the topic: **map insert_or_assign**.
...
This problem exercises **map insert_or_assign** — a core concept in STL Containers & Iterators.
...
Think of **map insert_or_assign** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: map insert_or_assign
int solve_map_insert_or_assign(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_map_insert_or_assign(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: map insert_or_assign
int solve_map_insert_or_assign(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: map insert_or_assign
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** map insert_or_assign — reuse battle-tested containers and algorithms.
