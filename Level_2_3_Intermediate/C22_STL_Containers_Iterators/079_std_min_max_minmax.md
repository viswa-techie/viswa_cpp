...
## 1. Problem Statement
Master the topic: **std min max minmax**.
...
This problem exercises **std min max minmax** — a core concept in STL Containers & Iterators.
...
Think of **std min max minmax** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: std min max minmax
int solve_std_min_max_minmax(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_min_max_minmax(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std min max minmax
int solve_std_min_max_minmax(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std min max minmax
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std min max minmax — reuse battle-tested containers and algorithms.
