...
## 1. Problem Statement
Master the topic: **Custom iterator class**.
...
This problem exercises **Custom iterator class** — a core concept in STL Containers & Iterators.
...
Think of **Custom iterator class** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: Custom iterator class
int solve_custom_iterator_class(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_custom_iterator_class(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Custom iterator class
int solve_custom_iterator_class(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Custom iterator class
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Custom iterator class — reuse battle-tested containers and algorithms.
