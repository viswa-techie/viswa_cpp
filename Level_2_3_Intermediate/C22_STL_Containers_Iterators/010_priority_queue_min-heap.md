# Chapter: priority_queue min-heap
...
## 1. Problem Statement
Master the topic: **priority_queue min-heap**.
...
This problem exercises **priority_queue min-heap** — a core concept in STL Containers & Iterators.
...
Think of **priority_queue min-heap** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: priority_queue min-heap
int solve_priority_queue_min_heap(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_priority_queue_min_heap(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: priority_queue min-heap
int solve_priority_queue_min_heap(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: priority_queue min-heap
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** priority_queue min-heap — reuse battle-tested containers and algorithms.
