# Chapter: std lexicographical_compare
...
## 1. Problem Statement
Master the topic: **std lexicographical_compare**.
...
This problem exercises **std lexicographical_compare** — a core concept in STL Containers & Iterators.
...
Think of **std lexicographical_compare** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: std lexicographical_compare
int solve_std_lexicographical_compare(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_lexicographical_compare(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std lexicographical_compare
int solve_std_lexicographical_compare(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std lexicographical_compare
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std lexicographical_compare — reuse battle-tested containers and algorithms.
