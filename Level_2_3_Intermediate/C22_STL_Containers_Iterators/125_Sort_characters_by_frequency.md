# Chapter: Sort characters by frequency
...
## 1. Problem Statement
Master the topic: **Sort characters by frequency**.
...
This problem exercises **Sort characters by frequency** — a core concept in STL Containers & Iterators.
...
Think of **Sort characters by frequency** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: Sort characters by frequency
int solve_sort_characters_by_frequency(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_sort_characters_by_frequency(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Sort characters by frequency
int solve_sort_characters_by_frequency(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Sort characters by frequency
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Sort characters by frequency — reuse battle-tested containers and algorithms.
