# Chapter: std swap iter_swap swap_ranges
...
## 1. Problem Statement
Master the topic: **std swap iter_swap swap_ranges**.
...
This problem exercises **std swap iter_swap swap_ranges** — a core concept in STL Containers & Iterators.
...
Think of **std swap iter_swap swap_ranges** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: std swap iter_swap swap_ranges
int solve_std_swap_iter_swap_swap_ranges(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_std_swap_iter_swap_swap_ranges(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std swap iter_swap swap_ranges
int solve_std_swap_iter_swap_swap_ranges(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std swap iter_swap swap_ranges
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std swap iter_swap swap_ranges — reuse battle-tested containers and algorithms.
