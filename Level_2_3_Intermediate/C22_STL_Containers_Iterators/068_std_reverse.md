...
This problem exercises **std reverse** — a core concept in STL Containers & Iterators.
...
Think of **std reverse** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: std reverse
int solve_std_reverse(std::vector<int> v) {
    int result = 0;
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: std reverse
int solve_std_reverse(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: std reverse
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** std reverse — reuse battle-tested containers and algorithms.
