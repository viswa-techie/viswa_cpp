...
## 1. Problem Statement
Master the topic: **Iterator concepts**.
...
This problem exercises **Iterator concepts** — a core concept in STL Containers & Iterators.
...
Think of **Iterator concepts** as a toolbox of pre-built containers, each with specific complexity guarantees.
...
#include <vector>
#include <string>
// Brute-force template for: Iterator concepts
int solve_iterator_concepts(std::vector<int> v) {
    int result = 0;
...
int main() {
    ...
    std::cout << solve_iterator_concepts(v) << "\n";
}
```
**Time:** O(n²)  
**Space:** O(1)  
...
#include <vector>
#include <algorithm>
// Optimized one-pass template for: Iterator concepts
int solve_iterator_concepts(const std::vector<int>& v) {
    if (v.empty()) return 0;
...
#include <numeric>
#include <algorithm>
// STL idiomatic template for: Iterator concepts
int main() {
    std::vector<int> v = {1,2,3,4,5};
...
- **Core idea:** Iterator concepts — reuse battle-tested containers and algorithms.
