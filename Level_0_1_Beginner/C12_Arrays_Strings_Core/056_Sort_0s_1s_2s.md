...
Write a C++ program that solves the Sort 0s 1s 2s problem.
...
### What Is It?
Sort 0s 1s 2s is a classic problem in C++ that appears frequently in interviews and real projects.
...
### Mental Model
Think of sort 0s 1s 2s as a puzzle — break it into smaller pieces and solve each one.
...
/*
 * Sort 0s 1s 2s — STL/Library approach
 */
int main() {
...
/*
 * Sort 0s 1s 2s — Modern C++17/20 approach
 */
int main() {
    std::vector<int> data = {5, 2, 8, 1, 9, 3, 7, 4, 6};
...
- Sort 0s 1s 2s demonstrates loop control and algorithmic thinking
...
2. Three pointers: low (0s boundary), mid (current), high (2s boundary)
3. Only swap and advance — never go backwards
4. Generalizes to any 3-way partition problem
5. Used as subroutine in quicksort (3-way partition)
