...
Write a C++ program that solves the Prefix sum 2D (range sum query) problem.
...
### What Is It?
Prefix sum 2D (range sum query) is a classic problem in C++ that appears frequently in interviews and real projects.
...
### Mental Model
Think of prefix sum 2d (range sum query) as a puzzle — break it into smaller pieces and solve each one.
...
- Prefix sum 2D (range sum query) demonstrates loop control and algorithmic thinking
...
- **Core idea:** Prefix sum 2D (range sum query)
- **Key construct:** Loops
