...
Write a C++ program that solves the Linear search problem.
...
### What Is It?
Linear search is a classic problem in C++ that appears frequently in interviews and real projects.
...
### Mental Model
Think of linear search as a puzzle — break it into smaller pieces and solve each one.
...
int main() {
    std::vector<int> arr = {4, 2, 7, 1, 9, 3, 8};
    ...
    // Not found case
    idx = linearSearch(arr, 99);
    std::cout << "99 found at index " << idx << "";  // -1
    return 0;
}
```
...
/*
 * Linear search — STL/Library approach
 */
int main() {
...
/*
 * Linear search — Modern C++17/20 approach
 */
int main() {
    std::vector<int> data = {5, 2, 8, 1, 9, 3, 7, 4, 6};
...
- Linear search demonstrates loop control and algorithmic thinking
