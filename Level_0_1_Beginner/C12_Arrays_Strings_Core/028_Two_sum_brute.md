...
Write a C++ program that solves the Two sum (brute) problem.
...
### What Is It?
Two sum (brute) is a classic problem in C++ that appears frequently in interviews and real projects.
...
### Mental Model
Think of two sum (brute) as a puzzle — break it into smaller pieces and solve each one.
...
std::pair<int,int> twoSumBrute(const std::vector<int>& nums, int target) {
...
int main() {
    ...
    int target = 9;
    auto [i, j] = twoSumBrute(nums, target);
...
/*
 * Two sum brute — STL/Library approach
 */
int main() {
...
/*
 * Two sum brute — Modern C++17/20 approach
 */
int main() {
    std::vector<int> data = {5, 2, 8, 1, 9, 3, 7, 4, 6};
...
- Two sum (brute) demonstrates loop control and algorithmic thinking
